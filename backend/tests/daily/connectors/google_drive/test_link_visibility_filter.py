...
def test_connector_skips_link_only_files_when_enabled() -> None:
    ...
    with (
        patch(
            "orbyte.connectors.google_drive.connector.run_functions_tuples_in_parallel",
            side_effect=_stub_run_functions,
        ),
        patch(
            "orbyte.connectors.google_drive.connector.convert_drive_item_to_document"
        ) as convert_mock,
        patch(
            "orbyte.connectors.google_drive.connector.GoogleDriveConnector._get_new_ancestors_for_files"
