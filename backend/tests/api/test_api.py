...
from orbyte.main import fetch_versioned_implementation
...
def test_versions_endpoint(client: TestClient) -> None:
    ...
    assert STABLE_VERSION_PATTERN.match(stable["orbyte"]), (
        f"Stable version {stable['orbyte']} doesn't match pattern v(number).(number).(number)"
    )

    # Verify dev configuration
    dev = data["dev"]
    assert "orbyte" in dev
    assert "relational_db" in dev
    assert "index" in dev
    assert "nginx" in dev

    # Verify dev version follows correct pattern (v1.2.3-beta.4)
    assert DEV_VERSION_PATTERN.match(dev["orbyte"]), (
        f"Dev version {dev['orbyte']} doesn't match pattern v(number).(number).(number)-beta.(number)"
    )

    # Verify migration configuration
    migration = data["migration"]
    assert "orbyte" in migration
    assert "relational_db" in migration
    assert "index" in migration
    assert "nginx" in migration

    # Verify migration has expected values
    assert migration["orbyte"] == "airgapped-intfloat-nomic-migration"
