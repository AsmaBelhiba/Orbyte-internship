...
from orbyte.sandbox_proxy import backend as backend_mod
...
def test_build_ca_store_kubernetes_dispatches_k8s_store() -> None:
    expected = MagicMock()
    with (
        patch("orbyte.sandbox_proxy.ca_k8s.K8sSecretCAStore", return_value=expected),
...
def test_build_ca_store_docker_dispatches_file_store() -> None:
    expected = MagicMock()
    with (
        patch("orbyte.sandbox_proxy.ca_docker.FileCAStore", return_value=expected),
...
def test_build_ip_lookup_kubernetes_dispatches_informer() -> None:
    ...
    with (
        patch(
            "orbyte.sandbox_proxy.identity_k8s.K8sInformerLookup", return_value=expected
        ),
        patch.object(backend_mod, "SANDBOX_BACKEND", SandboxBackend.KUBERNETES),
    ):
...
def test_build_ip_lookup_docker_dispatches_events_lookup() -> None:
    ...
    with (
        patch(
            "orbyte.sandbox_proxy.identity_docker.DockerEventsLookup",
