...
_HANDLE_MSG = "orbyte.orbytebot.slack.handlers.handle_message"
_LISTENER = "orbyte.orbytebot.slack.listener"
...
class TestCheckTenantGated:
    ...
    def _call(
        ...
        from orbyte.orbytebot.slack.listener import _check_tenant_gated
...
class TestExtractChannelFromRequest:
    ...
    def test_channel_extraction(
        ...
        from orbyte.orbytebot.slack.listener import _extract_channel_from_request
...
class TestHandleMessageSeatCheck:
    ...
    def test_new_user_blocked_when_seats_exceeded(
        ...
        assert "Orbyte administrator" in mock_respond.call_args[1]["text"]
...
class TestCheckSeatAvailability:
    ...
    def _check(self, used: int, total: int) -> Any:
        from ee.orbyte.db.license import check_seat_availability

        metadata = MagicMock(seats=total)
        with (
            patch("ee.orbyte.db.license.get_used_seats", return_value=used),
            patch("ee.orbyte.db.license.get_license_metadata", return_value=metadata),
        ):
    ...
    def test_no_license_allows_unlimited(self) -> None:
        ...
        with patch("ee.orbyte.db.license.get_license_metadata", return_value=None):
...
class TestGetUsedSeats:
    ...
    @patch("ee.orbyte.db.license.MULTI_TENANT", False)
    @patch("orbyte.db.engine.sql_engine.get_session_with_current_tenant")
