"""Unit tests for the generalized audit-event subsystem (``orbyte.utils.audit``).
...
from orbyte.utils import audit
from orbyte.utils.audit import _OCSF_CLASS_BY_ACTION
from orbyte.utils.audit import actor_from_user
from orbyte.utils.audit import AUDIT_HANDLER_NAME
from orbyte.utils.audit import AUDIT_LOGGER_ROOT
...
from orbyte.utils.audit import OCSFEventClass


def _capture(caplog: pytest.LogCaptureFixture) -> list[dict[str, Any]]:
    """Parse every captured ``orbyte.audit`` JSON message into a dict."""
    events: list[dict[str, Any]] = []
    for record in caplog.records:
        if record.name.startswith("orbyte.audit"):
...
def test_dedup_degrades_to_emit_when_redis_down(
    ...
    import orbyte.redis.redis_pool as redis_pool
...
def test_audit_logger_self_contained_without_root_handler() -> None:
    ...
    assert own_handlers, "orbyte.audit must attach its own handler"
    ...
    assert lines, "audit event must reach orbyte.audit handlers without root"
