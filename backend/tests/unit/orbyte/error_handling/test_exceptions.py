"""Tests for OrbyteError and the global exception handler."""
...
class TestOrbyteError:
    """Unit tests for OrbyteError construction and properties."""

    def test_basic_construction(self) -> None:
        err = OrbyteError(OrbyteErrorCode.NOT_FOUND, "Session not found")
        assert err.error_code is OrbyteErrorCode.NOT_FOUND
        assert err.detail == "Session not found"
        assert err.status_code == 404

    def test_message_defaults_to_code(self) -> None:
        err = OrbyteError(OrbyteErrorCode.UNAUTHENTICATED)
    ...
    def test_no_override_uses_error_code_status(self) -> None:
        err = OrbyteError(OrbyteErrorCode.RATE_LIMITED, "slow down")
        assert err.status_code == 429

    def test_is_exception(self) -> None:
        err = OrbyteError(OrbyteErrorCode.INTERNAL_ERROR)
        assert isinstance(err, Exception)


class TestExceptionHandler:
    """Integration test: OrbyteError → JSON response via FastAPI TestClient."""

    @pytest.fixture()
    def client(self) -> TestClient:
        app = FastAPI()
        register_orbyte_exception_handlers(app)

        @app.get("/boom")
        def _boom() -> None:
            raise OrbyteError(OrbyteErrorCode.NOT_FOUND, "Thing not found")
