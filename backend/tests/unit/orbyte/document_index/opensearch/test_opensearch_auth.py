...
import orbyte.configs.app_configs as app_configs
...
def test_iam_auth_uses_sigv4_signer() -> None:
    ...
    """
    with (
        patch("orbyte.document_index.opensearch.client.OpenSearch") as mock_os,
        patch("orbyte.document_index.opensearch.client.boto3.Session") as mock_session,
    ):
...
def test_basic_auth_uses_tuple() -> None:
    ...
    """
    with patch("orbyte.document_index.opensearch.client.OpenSearch") as mock_os:
        OpenSearchClient(
            host="h",
            auth=("u", "p"),
...
def test_iam_without_credentials_raises() -> None:
    with patch("orbyte.document_index.opensearch.client.boto3.Session") as mock_session:
