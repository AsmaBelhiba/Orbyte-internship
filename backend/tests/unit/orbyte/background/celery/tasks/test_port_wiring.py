...
from orbyte.background.celery.tasks.beat_schedule import beat_task_templates
...
def test_check_for_port_scheduled_in_beat() -> None:
    ...
    if DISABLE_VECTOR_DB:
        ...
        assert OrbyteCeleryTask.CHECK_FOR_PORT not in scheduled
    else:
        assert OrbyteCeleryTask.CHECK_FOR_PORT in scheduled
...
def test_docprocessing_worker_consumes_port_queue() -> None:
    ...
    assert OrbyteCeleryQueues.PORT in _worker_queues("celery_worker_docprocessing")
...
def test_docprocessing_worker_registers_port_tasks() -> None:
    ...
    from orbyte.background.celery.apps.docprocessing import celery_app

    celery_app.loader.import_default_modules()
    registered = set(celery_app.tasks.keys())
    assert OrbyteCeleryTask.RUN_PORT_ATTEMPT in registered
