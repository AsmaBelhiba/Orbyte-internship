...
def _enable_multi_tenant(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "ee.orbyte.background.celery.tasks.tenant_provisioning.tasks.MULTI_TENANT",
        True,
    )
...
def mock_pre_provision(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    monkeypatch.setattr(
        "ee.orbyte.background.celery.tasks.tenant_provisioning.tasks.pre_provision_tenant",
        mock,
    )
    return mock
...
def _mock_available_count(monkeypatch: pytest.MonkeyPatch, count: int) -> None:
    ...
    monkeypatch.setattr(
        "ee.orbyte.background.celery.tasks.tenant_provisioning.tasks.get_session_with_shared_schema",
        lambda: mock_session,
    )
...
class TestCheckAvailableTenants:
    def test_provisions_all_needed_tenants(
        ...
        monkeypatch.setattr(
            "ee.orbyte.background.celery.tasks.tenant_provisioning.tasks.TARGET_AVAILABLE_TENANTS",
            5,
        )
