...
from orbyte.llm.models import LanguageModelInput
...
from orbyte.llm.multi_llm import _parse_anthropic_model_version
from orbyte.llm.multi_llm import LitellmLLM
from orbyte.llm.multi_llm import temporary_env_and_lock
...
def test_claude_adaptive_thinking_uses_output_config(
    ...
    with (
        ...
        patch("orbyte.llm.multi_llm.model_is_reasoning_model", return_value=True),
    ):
        mock_completion.return_value = []
...
def test_openai_auto_reasoning_effort_maps_to_medium() -> None:
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch("orbyte.llm.multi_llm.model_is_reasoning_model", return_value=True),
        patch("orbyte.llm.multi_llm.is_true_openai_model", return_value=True),
    ):
        mock_completion.return_value = []
...
def test_openai_chat_omits_reasoning_params() -> None:
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch(
            "orbyte.llm.multi_llm.model_is_reasoning_model", return_value=True
        ) as mock_is_reasoning,
        patch(
            "orbyte.llm.multi_llm.is_true_openai_model", return_value=True
        ) as mock_is_openai,
    ):
        mock_stream_chunks = [
            litellm.ModelResponse(
...
def test_user_identity_metadata_enabled(default_multi_llm: LitellmLLM) -> None:
    with (
        patch("litellm.completion") as mock_completion,
        patch("orbyte.llm.utils.SEND_USER_METADATA_TO_LLM_PROVIDER", True),
    ):
        mock_stream_chunks = [
            litellm.ModelResponse(
                id="chatcmpl-123",
...
def test_user_identity_metadata_disabled_omits_identity(
    ...
    with (
        patch("litellm.completion") as mock_completion,
        patch("orbyte.llm.utils.SEND_USER_METADATA_TO_LLM_PROVIDER", False),
    ):
        mock_stream_chunks = [
            litellm.ModelResponse(
                id="chatcmpl-123",
...
def test_multithreaded_invoke_without_custom_config_does_not_inject_env() -> None:
    ...
    """
    from orbyte.llm import multi_llm as multi_llm_module
...
def test_messages_contain_tool_content_with_tool_role() -> None:
    from orbyte.llm.multi_llm import _messages_contain_tool_content
...
def test_strip_tool_content_converts_assistant_tool_calls_to_text() -> None:
    from orbyte.llm.multi_llm import _strip_tool_content_from_messages
...
def test_prompt_contains_tool_call_history_true() -> None:
    from orbyte.llm.multi_llm import _prompt_contains_tool_call_history
