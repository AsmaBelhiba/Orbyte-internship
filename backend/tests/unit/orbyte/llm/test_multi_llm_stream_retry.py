...
def test_stream_retries_timeout_before_first_chunk() -> None:
    ...
    with (
        patch("orbyte.llm.multi_llm.LLM_FIRST_CHUNK_MAX_RETRIES", 1),
        patch("orbyte.llm.multi_llm.is_true_openai_model", return_value=False),
        patch(
            "orbyte.llm.model_response.from_litellm_model_response_stream",
            return_value=translated_chunk,
        ),
        patch("orbyte.llm.multi_llm.logger") as mock_logger,
    ):
...
def test_stream_does_not_retry_after_first_chunk() -> None:
    ...
    with (
        patch("orbyte.llm.multi_llm.LLM_FIRST_CHUNK_MAX_RETRIES", 2),
        patch("orbyte.llm.multi_llm.is_true_openai_model", return_value=False),
        patch(
            "orbyte.llm.model_response.from_litellm_model_response_stream",
            return_value=translated_chunk,
