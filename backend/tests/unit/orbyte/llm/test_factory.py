...
from orbyte.llm.factory import _build_provider_extra_headers
...
def test_get_llm_sets_ollama_num_ctx_model_kwarg() -> None:
    with patch("orbyte.llm.factory.LitellmLLM") as mock_litellm_llm:
        get_llm(
...
def test_llm_from_provider_passes_configured_ollama_num_ctx() -> None:
    ...
    with patch("orbyte.llm.factory.get_llm") as mock_get_llm:
        llm_from_provider(
...
def test_llm_from_provider_omits_ollama_num_ctx_when_model_context_unknown() -> None:
    ...
    with (
        patch(
            "orbyte.llm.factory.get_max_input_tokens_from_llm_provider",
            return_value=32000,
        ),
        patch("orbyte.llm.factory.get_llm") as mock_get_llm,
    ):
        llm_from_provider(
            model_name="test-model",
            llm_provider=provider,
