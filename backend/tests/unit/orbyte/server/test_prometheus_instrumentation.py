...
from orbyte.server.metrics.per_tenant import per_tenant_request_callback
from orbyte.server.metrics.prometheus_setup import setup_prometheus_metrics
from orbyte.server.metrics.slow_requests import slow_request_callback
...
def test_slow_request_callback_increments_above_threshold() -> None:
    with patch("orbyte.server.metrics.slow_requests._slow_requests") as mock_counter:
...
def test_setup_attaches_instrumentator_to_app() -> None:
    with patch("orbyte.server.metrics.prometheus_setup.Instrumentator") as mock_cls:
...
def test_per_tenant_callback_increments_with_tenant_id() -> None:
    ...
    with (
        patch(
            "orbyte.server.metrics.per_tenant.CURRENT_TENANT_ID_CONTEXTVAR"
        ) as mock_ctx,
        patch("orbyte.server.metrics.per_tenant._requests_by_tenant") as mock_counter,
    ):
        mock_labels = MagicMock()
        mock_counter.labels.return_value = mock_labels
