...
from orbyte.server.metrics.postgres_connection_pool import _register_pool_events
from orbyte.server.metrics.postgres_connection_pool import PoolStateCollector
from orbyte.server.metrics.postgres_connection_pool import (
    setup_postgres_connection_pool_metrics,
)
from orbyte.utils.middleware import _build_route_map
from orbyte.utils.middleware import _match_route
...
def test_pool_state_collector_reports_pool_stats() -> None:
    ...
    assert metrics["orbyte_db_pool_checked_out:sync"] == 5
    assert metrics["orbyte_db_pool_checked_in:sync"] == 35
    assert metrics["orbyte_db_pool_overflow:sync"] == 2
    assert metrics["orbyte_db_pool_size:sync"] == 40
...
def test_checkout_event_stores_endpoint_and_increments_gauge() -> None:
    ...
    with patch("orbyte.server.metrics.postgres_connection_pool.event") as mock_event:
    ...
    with (
        patch(
            "orbyte.server.metrics.postgres_connection_pool.CURRENT_ENDPOINT_CONTEXTVAR"
        ) as mock_ctx,
        patch(
            "orbyte.server.metrics.postgres_connection_pool.CURRENT_TENANT_ID_CONTEXTVAR"
        ) as mock_tenant_ctx,
        patch(
            "orbyte.server.metrics.postgres_connection_pool._connections_held"
        ) as mock_gauge,
        patch("orbyte.server.metrics.postgres_connection_pool._checkout_total"),
    ):
...
def test_checkin_event_observes_hold_duration() -> None:
    ...
    with (
        ...
        patch(
            "orbyte.server.metrics.postgres_connection_pool._hold_seconds"
        ) as mock_hist,
        patch("orbyte.server.metrics.postgres_connection_pool._checkin_total"),
        patch(
            "orbyte.server.metrics.postgres_connection_pool.time.monotonic",
...
def test_checkin_with_missing_endpoint_uses_unknown() -> None:
    ...
    with (
        ...
        patch("orbyte.server.metrics.postgres_connection_pool._hold_seconds"),
        patch("orbyte.server.metrics.postgres_connection_pool._checkin_total"),
    ):
        mock_labels = MagicMock()
        mock_gauge.labels.return_value = mock_labels
...
def test_setup_skips_null_pool_engines() -> None:
    ...
    with (
        patch("orbyte.server.metrics.postgres_connection_pool.REGISTRY"),
        patch(
            "orbyte.server.metrics.postgres_connection_pool._register_pool_events"
        ) as mock_register,
    ):
