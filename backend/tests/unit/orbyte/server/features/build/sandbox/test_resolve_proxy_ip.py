...
def test_fqdn_resolved_to_clusterip_via_api(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ksm, "SANDBOX_PROXY_HOST", "orbyte-sandbox-proxy.orbyte.svc.cluster.local"
    )
    mgr, core_api = _mgr()
    svc = MagicMock()
    ...
    core_api.read_namespaced_service.assert_called_once_with(
        name="orbyte-sandbox-proxy", namespace="orbyte"
    )


def test_namespace_parsed_from_fqdn(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ksm, "SANDBOX_PROXY_HOST", "orbyte-sandbox-proxy.myrelease.svc.cluster.local"
    )
    monkeypatch.setattr(ksm, "SANDBOX_PROXY_NAMESPACE", "orbyte")
    mgr, core_api = _mgr()
    svc = MagicMock()
    ...
    core_api.read_namespaced_service.assert_called_once_with(
        name="orbyte-sandbox-proxy", namespace="myrelease"
    )
...
def test_namespace_falls_back_to_config_for_bare_host(
    ...
    monkeypatch.setattr(ksm, "SANDBOX_PROXY_HOST", "orbyte-sandbox-proxy")
    ...
    core_api.read_namespaced_service.assert_called_once_with(
        name="orbyte-sandbox-proxy", namespace="proxy-ns"
    )
