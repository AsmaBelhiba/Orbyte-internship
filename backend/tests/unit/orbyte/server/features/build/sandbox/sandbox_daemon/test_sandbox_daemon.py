...
#   backend/tests/unit/orbyte/server/features/build/sandbox/sandbox_daemon/test_sandbox_daemon.py
# so parents[9] is the repo root.
_REPO_ROOT = Path(__file__).resolve().parents[9]
_DAEMON_DIR = (
    _REPO_ROOT / "backend/orbyte/server/features/build/sandbox/image/sandbox_daemon"
)
...
def _point_opencode_paths(
    ...
    opencode_restored_path = (
        sessions_root.parent / "managed" / ".orbyte" / "opencode-history-restored"
    )
...
def configured_sandbox_daemon(
    ...
    monkeypatch.setenv("ORBYTE_SANDBOX_PUSH_PUBLIC_KEY", pub_b64)
...
def test_push_missing_public_key_raises_orbyte_error(
    configured_sandbox_daemon: tuple[ModuleType, ModuleType, Ed25519PrivateKey, Path],
    client: TestClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    Note: the in-pod daemon is standalone Python (no Orbyte imports) so it
    surfaces this via ``HTTPException(status_code=500, ...)`` rather than
    ``OrbyteError``. The behavior asserted here is the operator-facing error
    ...
    monkeypatch.delenv("ORBYTE_SANDBOX_PUSH_PUBLIC_KEY", raising=False)
    monkeypatch.setattr(server_mod, "_public_key", None, raising=False)
