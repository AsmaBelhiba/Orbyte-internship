...
import orbyte.server.features.build.sandbox.docker.docker_sandbox_manager as dsm
...
def test_volume_name_is_per_sandbox_and_short() -> None:
    ...
    assert vol.startswith("orbyte-craft-sandbox-")
...
def test_labels_include_required_fields() -> None:
    ...
    assert labels[LABEL_K8S_MANAGED_BY] == LABEL_K8S_MANAGED_BY_ORBYTE
...
def test_registry_port_untagged_image_refreshes_as_implicit_latest() -> None:
    image = "localhost:5001/orbyte-sandbox"
    mgr, docker = _bare_manager_with_image(image)
...
def kwargs() -> ContainerCreateKwargs:
    ...
    return build_container_create_kwargs(
        ...
        orbyte_pat="pat-redacted",
        api_server_url="http://api_server:8080",
        network="orbyte_craft_sandbox",
        volume_name="orbyte-craft-sandbox-12345678",
        memory_limit="2g",
        cpu_limit=1.0,
...
def proxy_kwargs() -> ContainerCreateKwargs:
    ...
    Note: ``orbyte_pat`` is the proxy-injected placeholder because in proxy mode
    ...
    return build_container_create_kwargs(
        ...
        tenant_id=TENANT_ID,
        image="onyxdotapp/sandbox:test",
        orbyte_pat=SANDBOX_PROXY_INJECTED_PLACEHOLDER,
        api_server_url="https://orbyte.example.com",
        network="orbyte_craft_sandbox",
        volume_name="orbyte-craft-sandbox-12345678",
        memory_limit="2g",
        cpu_limit=1.0,
...
def test_container_kwargs_env_allowlist_excludes_storage_credentials(
    ...
    # Required env
    assert env["ORBYTE_PAT"] == "pat-redacted"
    assert env["ORBYTE_SERVER_URL"] == "http://api_server:8080"
    ...
    forbidden = {
        ...
        "ORBYTE_SANDBOX_PUSH_PRIVATE_KEY",
    }
...
def test_container_kwargs_labels_and_volume(kwargs: ContainerCreateKwargs) -> None:
    ...
    assert "orbyte-craft-sandbox-12345678" in kwargs["volumes"]
    assert (
        kwargs["volumes"]["orbyte-craft-sandbox-12345678"]["bind"]
        == "/workspace/sessions"
    )
...
def test_container_kwargs_uses_sandbox_network(kwargs: ContainerCreateKwargs) -> None:
    ...
    assert kwargs["network"] == "orbyte_craft_sandbox"
...
def test_proxy_kwargs_mounts_ca_volume_read_only(
    ...
    assert "orbyte-craft-sandbox-12345678" in volumes
...
def test_proxy_kwargs_env_contains_proxy_and_ca_keys(
    ...
    # The legacy 4-key core is preserved; ORBYTE_PAT is the proxy placeholder in
    # this posture (real value lives in Postgres, proxy injects on wire).
    assert env["ORBYTE_PAT"] == SANDBOX_PROXY_INJECTED_PLACEHOLDER
    assert env["ORBYTE_SERVER_URL"] == "https://orbyte.example.com"
    assert env["OPENCODE_SERVER_PASSWORD"] == _OPENCODE_PASSWORD
    assert env["OPENCODE_CONFIG_CONTENT"] == _OPENCODE_CONFIG_JSON
