...
import orbyte.server.features.build.sandbox.kubernetes.kubernetes_sandbox_manager as ksm
import orbyte.server.features.build.sandbox.kubernetes.sidecar_client as sidecar
...
# backend/tests/unit/orbyte/server/features/build/sandbox/ -> repo root
_REPO_ROOT = Path(__file__).resolve().parents[8]
_CHART_DIR = _REPO_ROOT / "deployment" / "helm" / "charts" / "orbyte"
...
def _push_key_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(ksm, "SANDBOX_PROXY_HOST", "sandbox-proxy.orbyte.svc")
    monkeypatch.setattr(
...
def _build_pod() -> client.V1Pod:
    ...
    mgr._namespace = "orbyte-sandboxes"  # type: ignore[attr-defined]
...
def test_push_public_key_is_in_sidecar_env_only(pod: client.V1Pod) -> None:
    ...
    assert "ORBYTE_SANDBOX_PUSH_PUBLIC_KEY" in sidecar_env
    assert "ORBYTE_SANDBOX_PUSH_PUBLIC_KEY" not in sandbox_env
...
def test_orbyte_pat_env_is_placeholder_not_real(pod: client.V1Pod) -> None:
...
def test_no_proxy_is_loopback_only(pod: client.V1Pod) -> None:
    """Only loopback may bypass the proxy; the Orbyte API host must route through
