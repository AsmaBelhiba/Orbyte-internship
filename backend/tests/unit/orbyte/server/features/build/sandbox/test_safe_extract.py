...
from orbyte.server.features.build.sandbox.image.sandbox_daemon.extract import (
...
def _patch_allowed_prefix(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    prefix = str(tmp_path) + "/"
    monkeypatch.setattr(
        "orbyte.server.features.build.sandbox.image.sandbox_daemon.extract.ALLOWED_PREFIX",
        prefix,
    )
...
def test_oversized_file_rejected(
    ...
    monkeypatch.setattr(
        "orbyte.server.features.build.sandbox.image.sandbox_daemon.extract.MAX_FILE_BYTES",
        50,
    )
...
def test_oversized_bundle_rejected(
    ...
    monkeypatch.setattr(
        "orbyte.server.features.build.sandbox.image.sandbox_daemon.extract.MAX_BUNDLE_BYTES",
        100,
    )
