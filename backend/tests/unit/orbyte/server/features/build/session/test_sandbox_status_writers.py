...
ALLOWED_REFERENCES: set[str] = {
    ...
    "orbyte/server/features/build/db/sandbox.py",
    # Substrate-invariant transitions + invariants.
    "orbyte/server/features/build/session/sandbox_lifecycle.py",
}
...
def _modules_referencing_status_mutation() -> set[str]:
    ...
    scan_roots = (
        backend_dir / "orbyte" / "server" / "features" / "build",
        backend_dir / "orbyte" / "background",
    )
    found: set[str] = set()
