...
def _stub_recommended_default() -> Iterator[None]:
    with patch(
        "orbyte.server.features.build.session.llm_config."
...
class TestGetLlmConfigFallback:
    ...
    def _patch_providers(
        ...
        return patch(
            "orbyte.server.features.build.session.manager."
    ...
    def test_ignores_unsupported_provider_type(self) -> None:
        ...
        with self._patch_providers(
            ...
            try:
                ...
                raise AssertionError("expected OrbyteError")
    ...
    def test_raises_orbyte_error_when_no_supported_provider(self) -> None:
        with self._patch_providers([]):
            try:
                self._manager().build_llm_configs(self._user(), None, None)
            except OrbyteError as e:
...
class TestProvisionSandboxForwardsAllLlmConfigs:
    ...
    def test_provision_passes_all_llm_configs(self) -> None:
        ...
        with (
            patch(
                "orbyte.server.features.build.session.sandbox_lifecycle."
                "ensure_sandbox_pat",
        ...
        assert kwargs["orbyte_pat"] == "pat-token"
