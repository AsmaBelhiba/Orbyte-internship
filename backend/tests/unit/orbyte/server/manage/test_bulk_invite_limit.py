...
from orbyte.server.manage.users import bulk_invite_users
from orbyte.server.manage.users import remove_invited_user
...
@patch(
    "orbyte.server.manage.users.get_session_with_shared_schema",
    new_callable=lambda: _make_shared_session_mock(next_total=6),
)
@patch("orbyte.server.manage.users.enforce_invite_rate_limit")
@patch("orbyte.server.manage.users.MULTI_TENANT", True)
@patch("orbyte.server.manage.users.is_tenant_on_trial_fn", return_value=True)
...
@patch("orbyte.server.manage.users.NUM_FREE_TRIAL_USER_INVITES", 5)
def test_trial_tenant_cannot_exceed_invite_limit(*_mocks: None) -> None:
    """Post-upsert total of 6 exceeds cap=5 — must raise OrbyteError."""
    emails = [f"user{i}@example.com" for i in range(6)]

    with pytest.raises(OrbyteError) as exc_info:
        bulk_invite_users(emails=emails, current_user=MagicMock())

    assert exc_info.value.error_code == OrbyteErrorCode.TRIAL_INVITE_LIMIT_EXCEEDED
...
@patch("orbyte.server.manage.users.enforce_invite_rate_limit")
@patch("orbyte.server.manage.users.get_redis_client")
@patch("orbyte.server.manage.users.MULTI_TENANT", True)
@patch("orbyte.server.manage.users.DEV_MODE", True)
@patch("orbyte.server.manage.users.ENABLE_EMAIL_INVITES", False)
@patch("orbyte.server.manage.users.is_tenant_on_trial_fn", return_value=True)
@patch("orbyte.server.manage.users.get_current_tenant_id", return_value="test_tenant")
@patch("orbyte.server.manage.users.get_invited_users", return_value=[])
@patch("orbyte.server.manage.users.get_all_users", return_value=[])
@patch("orbyte.server.manage.users.write_invited_users", return_value=3)
@patch("orbyte.server.manage.users.enforce_seat_limit_locked")
@patch("orbyte.server.manage.users.NUM_FREE_TRIAL_USER_INVITES", 5)
@patch(
    "orbyte.server.manage.users.fetch_ee_implementation_or_noop",
...
@patch("orbyte.server.manage.users.get_session_with_shared_schema")
@patch("orbyte.server.manage.users.enforce_invite_rate_limit")
@patch("orbyte.server.manage.users.MULTI_TENANT", True)
@patch("orbyte.server.manage.users.DEV_MODE", True)
@patch("orbyte.server.manage.users.ENABLE_EMAIL_INVITES", False)
@patch("orbyte.server.manage.users.is_tenant_on_trial_fn", return_value=False)
@patch("orbyte.server.manage.users.get_current_tenant_id", return_value="test_tenant")
@patch("orbyte.server.manage.users.get_invited_users", return_value=[])
@patch("orbyte.server.manage.users.get_all_users", return_value=[])
@patch("orbyte.server.manage.users.write_invited_users", return_value=3)
...
def test_bulk_invite_emits_user_create(
    ...
    events: list[dict[str, Any]] = [
        ...
        if r.name.startswith("orbyte.audit")
    ]
    assert len(events) == 1
...
def test_bulk_invite_reinvite_emits_nothing(
    ...
    events = [r for r in caplog.records if r.name.startswith("orbyte.audit")]
    assert events == []
...
@_with_common_patches
@patch("orbyte.server.manage.users.ENABLE_EMAIL_INVITES", True)
@patch("orbyte.server.manage.users.EMAIL_CONFIGURED", False)
...
@_with_common_patches
@patch("orbyte.server.manage.users.ENABLE_EMAIL_INVITES", True)
@patch("orbyte.server.manage.users.EMAIL_CONFIGURED", True)
@patch("orbyte.server.manage.users.send_user_email_invite")
...
@patch("orbyte.server.manage.users.EMAIL_CONFIGURED", True)
@patch(
    "orbyte.server.manage.users.send_user_email_invite",
...
@patch("orbyte.server.manage.users.enforce_remove_invited_rate_limit")
@patch("orbyte.server.manage.users.remove_user_from_invited_users", return_value=0)
@patch("orbyte.server.manage.users.MULTI_TENANT", True)
@patch("orbyte.server.manage.users.DEV_MODE", True)
@patch("orbyte.server.manage.users.is_tenant_on_trial_fn", return_value=False)
@patch("orbyte.server.manage.users.get_current_tenant_id", return_value="test_tenant")
...
def test_cloud_seat_decline_propagates_from_add_users(*_mocks: MagicMock) -> None:
    ...
    the caller as OrbyteError — the generic except must not swallow it."""

    def _declining_add_users(*_args: object) -> None:
        raise OrbyteError(OrbyteErrorCode.SEAT_LIMIT_EXCEEDED, "card declined")
    ...
    assert exc_info.value.error_code == OrbyteErrorCode.SEAT_LIMIT_EXCEEDED
