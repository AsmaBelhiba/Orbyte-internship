...
def test_invite_allows_under_all_tiers() -> None:
    ...
    with (
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_MIN", 5),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_DAY", 50),
        patch(
            "orbyte.server.manage.invite_rate_limit._INVITE_TENANT_PER_DAY",
            500,
        ),
    ):
...
def test_invite_minute_bucket_blocks_request_flood() -> None:
    ...
    with (
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_MIN", 5),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_DAY", 500),
        patch(
            "orbyte.server.manage.invite_rate_limit._INVITE_TENANT_PER_DAY",
            5000,
        ),
    ...
    assert exc_info.value.error_code == OrbyteErrorCode.RATE_LIMITED
...
def test_invite_admin_daily_cap_enforced() -> None:
    ...
    with (
        patch(
            "orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_MIN",
            1000,
        ),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_DAY", 50),
        patch(
...
def test_invite_tenant_daily_cap_enforced_across_admins() -> None:
    ...
    with (
        ...
        patch(
            "orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_DAY",
            1000,
        ),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_TENANT_PER_DAY", 10),
    ):
        enforce_invite_rate_limit(
...
def test_invite_zero_new_invites_still_ticks_minute_bucket() -> None:
    ...
    with (
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_MIN", 2),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_DAY", 500),
        patch(
            "orbyte.server.manage.invite_rate_limit._INVITE_TENANT_PER_DAY",
            5000,
...
def test_invite_limit_zero_disables_tier() -> None:
    ...
    with (
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_MIN", 0),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_DAY", 0),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_TENANT_PER_DAY", 0),
    ):
        for _ in range(100):
            enforce_invite_rate_limit(
                redis_client, user_id, num_invites=10, tenant_id="tenant_a"
...
def test_invite_tenant_bucket_is_isolated_across_tenants() -> None:
    ...
    with (
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_MIN", 1000),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_DAY", 1000),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_TENANT_PER_DAY", 10),
    ):
...
def test_invite_fails_open_when_redis_unavailable() -> None:
    """Orbyte Lite deployments ship without Redis; invite flow must still work."""
    stub = _StubRedis()
    ...
    with (
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_MIN", 1),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_DAY", 1),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_TENANT_PER_DAY", 1),
    ):
        enforce_invite_rate_limit(
...
def test_remove_minute_bucket_blocks_pattern_attack() -> None:
    ...
    with (
        patch(
            "orbyte.server.manage.invite_rate_limit._REMOVE_ADMIN_PER_MIN",
            3,
        ),
        patch(
            "orbyte.server.manage.invite_rate_limit._REMOVE_ADMIN_PER_DAY",
            100,
        ),
    ):
        for _ in range(3):
...
def test_ttls_set_on_first_increment_and_not_reset() -> None:
    ...
    with (
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_MIN", 100),
        patch("orbyte.server.manage.invite_rate_limit._INVITE_ADMIN_PER_DAY", 500),
        patch(
            "orbyte.server.manage.invite_rate_limit._INVITE_TENANT_PER_DAY",
            5000,
