"""Unit tests for the seat-counted-status helpers in orbyte.auth.users.
...
canonical EE predicate ``ee/orbyte/db/license.py:user_counts_toward_seats``;
...
from ee.orbyte.db.license import user_counts_toward_seats as _canonical
from orbyte.auth.users import _upgrade_will_add_seat
from orbyte.auth.users import _user_currently_counts_toward_seats
...
def _patch_canonical_predicate() -> Iterator[None]:
    ...
    with patch(
        "orbyte.auth.users.fetch_ee_implementation_or_noop", side_effect=fake_fetch
    ):
        yield
...
class TestUpgradeWillAddSeat:
    ...
    def test_multi_tenant_upgrade_is_seat_neutral(self) -> None:
        ...
        with patch("orbyte.auth.users.MULTI_TENANT", True):
            assert not _upgrade_will_add_seat(before, will_become_active=True)
