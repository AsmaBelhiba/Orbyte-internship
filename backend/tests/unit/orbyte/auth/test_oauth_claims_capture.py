...
import orbyte.auth.oauth_claims_capture as claims_capture
from orbyte.auth.oauth_claims_capture import capture_oauth_login_claims
from orbyte.auth.oauth_claims_capture import get_captured_oauth_claims
from orbyte.auth.oauth_claims_capture import get_idp_profile_fields
from orbyte.auth.oauth_claims_capture import get_idp_profile_placeholder_values
...
async def test_capture_stores_id_token_claims_and_token_meta() -> None:
    ...
    with patch(
        "orbyte.auth.oauth_claims_capture.get_async_redis_connection",
        return_value=redis,
    ):
...
async def test_capture_fetches_userinfo_when_endpoint_available() -> None:
    ...
    with (
        ...
        patch(
            "orbyte.auth.oauth_claims_capture._fetch_userinfo",
...
async def test_capture_fetches_directory_profile_for_entra() -> None:
    ...
    with (
        ...
        patch(
            "orbyte.auth.oauth_claims_capture._fetch_ms_graph_profile",
...
def test_get_idp_profile_fields_maps_directory_profile() -> None:
    ...
    with patch("orbyte.redis.redis_pool.get_raw_redis_client", return_value=redis):
