...
class TestDisposableEmailValidation:
    ...
    @pytest.mark.asyncio
    @patch("orbyte.auth.users.is_disposable_email")
    @patch("orbyte.auth.users.fetch_ee_implementation_or_noop")
    @patch("orbyte.auth.users.get_async_session_context_manager")
    @patch("orbyte.auth.users.get_user_count", new_callable=AsyncMock)
    ...
    @pytest.mark.asyncio
    @patch("orbyte.auth.users.is_disposable_email")
    @patch("orbyte.auth.users.verify_email_domain")
    @patch("orbyte.auth.users.fetch_ee_implementation_or_noop")
    @patch("orbyte.auth.users.get_async_session_context_manager")
    @patch("orbyte.auth.users.get_user_count", new_callable=AsyncMock)
    @patch("orbyte.auth.users.MULTI_TENANT", False)
...
class TestMultiTenantInviteLogic:
    ...
    @patch("orbyte.auth.users.SQLAlchemyUserAdminDB")
    @patch("orbyte.auth.users.is_disposable_email", return_value=False)
    @patch("orbyte.auth.users.verify_email_domain")
    @patch("orbyte.auth.users.fetch_ee_implementation_or_noop")
    @patch("orbyte.auth.users.get_async_session_context_manager")
    @patch("orbyte.auth.users.get_user_count", new_callable=AsyncMock)
    @patch("orbyte.auth.users.verify_email_is_invited")
    @patch("orbyte.auth.users.MULTI_TENANT", True)
    @patch("orbyte.auth.users.CURRENT_TENANT_ID_CONTEXTVAR")
    @pytest.mark.asyncio
...
class TestSAMLOIDCBehavior:
    ...
    @patch("orbyte.auth.users.get_invited_users")
    @patch("orbyte.auth.users.workspace_invite_only_enabled", return_value=True)
    @patch("orbyte.auth.users.AUTH_TYPE")
    def test_sso_bypasses_whitelist(
        ...
        from orbyte.auth.users import verify_email_is_invited
        ...
        with patch("orbyte.auth.users.AUTH_TYPE", auth_type):
            verify_email_is_invited("newuser@example.com")  # Should not raise

    @patch("orbyte.auth.users.get_invited_users")
    @patch("orbyte.auth.users.workspace_invite_only_enabled", return_value=True)
    @patch("orbyte.auth.users.AUTH_TYPE", AuthType.BASIC)
...
class TestWhitelistBehavior:
    ...
    @patch("orbyte.auth.users.workspace_invite_only_enabled", return_value=False)
    @patch("orbyte.auth.users.get_invited_users")
    @patch("orbyte.auth.users.AUTH_TYPE", AuthType.BASIC)
...
class TestSeatLimitEnforcement:
    ...
    def test_adding_user_fails_when_seats_full(self) -> None:
        from orbyte.auth.users import enforce_seat_limit

        seat_result = MagicMock(available=False, error_message="Seat limit reached")
        with patch(
            "orbyte.auth.users.fetch_ee_implementation_or_noop",
    ...
    def test_cloud_locked_variant_forwards_session_to_billing(self) -> None:
        ...
        with (
            patch("orbyte.auth.users.MULTI_TENANT", True),
            patch(
                "orbyte.auth.users.get_current_tenant_id",
...
class TestOAuthDottedGmail:
    ...
    @pytest.mark.asyncio
    @patch("orbyte.auth.users.MULTI_TENANT", False)
    @patch("orbyte.auth.users.verify_email_in_whitelist")
    @patch("orbyte.auth.users.verify_email_domain")
    @patch("orbyte.auth.users.fetch_ee_implementation_or_noop")
    @patch("orbyte.auth.users.get_async_session_context_manager")
    @patch("orbyte.auth.users.remove_user_from_invited_users")
