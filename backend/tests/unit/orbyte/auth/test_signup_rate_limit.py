...
from orbyte.auth import signup_rate_limit as rl
from orbyte.auth.signup_rate_limit import _bucket_key
from orbyte.auth.signup_rate_limit import _client_ip
from orbyte.auth.signup_rate_limit import _PER_IP_PER_HOUR
...
async def test_rejects_when_over_limit() -> None:
    """Strictly above the cap → OrbyteError.RATE_LIMITED (HTTP 429)."""
    req = _make_request(xff="1.2.3.4, 10.0.0.1")
