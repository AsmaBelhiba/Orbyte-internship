...
from orbyte.natural_language_processing.search_nlp_models import CloudEmbedding
...
async def test_cohere_embed_supports_v3_response_format(
    ...
    with patch(
        "orbyte.natural_language_processing.search_nlp_models.CohereAsyncClient"
    ) as mock_cohere:
        mock_client = AsyncMock()
...
async def test_rate_limit_handling() -> None:
    with patch(
        "orbyte.natural_language_processing.search_nlp_models.CloudEmbedding.embed"
    ) as mock_embed:
...
_SEARCH_NLP_MODULE = "orbyte.natural_language_processing.search_nlp_models"
