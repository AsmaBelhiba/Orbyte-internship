...
from orbyte.connectors.salesforce.doc_conversion import _extract_section
from orbyte.connectors.salesforce.doc_conversion import ID_PREFIX
from orbyte.connectors.salesforce.orbyte_salesforce import OrbyteSalesforce
from orbyte.connectors.salesforce.salesforce_calls import _bulk_retrieve_from_salesforce
from orbyte.connectors.salesforce.salesforce_calls import _make_time_filter_for_sf_type
from orbyte.connectors.salesforce.salesforce_calls import _make_time_filtered_query
from orbyte.connectors.salesforce.salesforce_calls import get_object_by_id_query
from orbyte.connectors.salesforce.sqlite_functions import OrbyteSalesforceSQLite
...
from orbyte.connectors.salesforce.utils import USER_OBJECT_TYPE
from orbyte.utils.logger import setup_logger

# from orbyte.connectors.salesforce.orbyte_salesforce_type import OrbyteSalesforceType
# from orbyte.connectors.salesforce.salesforce_calls import get_children_of_sf_type
...
def _create_csv_with_example_data(sf_db: OrbyteSalesforceSQLite) -> None:
    """
...
def _test_query(sf_db: OrbyteSalesforceSQLite) -> None:
    """
...
def _test_upsert(sf_db: OrbyteSalesforceSQLite) -> None:
    """
...
def _test_relationships(sf_db: OrbyteSalesforceSQLite) -> None:
    """
...
def _test_account_with_children(sf_db: OrbyteSalesforceSQLite) -> None:
    """
...
def _test_relationship_updates(sf_db: OrbyteSalesforceSQLite) -> None:
    """
...
def _test_get_affected_parent_ids(sf_db: OrbyteSalesforceSQLite) -> None:
    """
...
def test_salesforce_sqlite() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        sf_db = OrbyteSalesforceSQLite(filename)
        sf_db.connect()
        sf_db.apply_schema()
...
def test_salesforce_bulk_retrieve() -> None:
    ...
    # orbyte_sf_type = OrbyteSalesforceType("Contact", sf_client)
...
def test_normalize_record() -> None:
    ...
    with open(current_dir / "test_account.csv", "r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            normalized_record, parent_ids = OrbyteSalesforceSQLite.normalize_record(row)
...
def test_salesforce_connector_single() -> None:
    ...
    # orbyte_parent_sf_type = OrbyteSalesforceType(parent_type, sf_client)
    ...
    for parent_type_working in parent_types:
        ...
        for child_type, child_relationship in child_types_working.items():
            # orbyte_sf_type = OrbyteSalesforceType(child_type, sf_client)
    ...
    normalized_record, _ = OrbyteSalesforceSQLite.normalize_record(record)
