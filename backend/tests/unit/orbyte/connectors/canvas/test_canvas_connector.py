...
from ee.orbyte.external_permissions.canvas.access import get_announcement_permissions
from ee.orbyte.external_permissions.canvas.access import get_assignment_permissions
from ee.orbyte.external_permissions.canvas.access import get_page_permissions
from ee.orbyte.external_permissions.canvas.group_sync import (
...
from orbyte.connectors.canvas.client import CanvasApiClient
from orbyte.connectors.canvas.connector import _in_time_window
from orbyte.connectors.canvas.connector import _parse_canvas_dt
from orbyte.connectors.canvas.connector import _unix_to_canvas_time
...
from orbyte.connectors.canvas.connector import CanvasAnnouncementSection
from orbyte.connectors.canvas.connector import CanvasAssignment
from orbyte.connectors.canvas.connector import CanvasAssignmentOverride
from orbyte.connectors.canvas.connector import CanvasConnector
from orbyte.connectors.canvas.connector import CanvasConnectorCheckpoint
from orbyte.connectors.canvas.connector import CanvasPage
from orbyte.connectors.canvas.connector import CanvasStage
...
def _build_connector(base_url: str = FAKE_BASE_URL) -> CanvasConnector:
    ...
    with patch("orbyte.connectors.canvas.client.rl_requests") as mock_req:
...
class TestGet:
    ...
    @patch("orbyte.connectors.canvas.client.rl_requests")
    ...
    def test_invalid_json_on_success_raises(self, mock_requests: MagicMock) -> None:
        """Invalid JSON on a 2xx response raises OrbyteError."""
        resp = MagicMock()
        ...
        with pytest.raises(OrbyteError, match="Invalid JSON"):
            self.client.get("courses")
...
class TestParseNextLink:
    ...
    def test_rejects_host_mismatch(self) -> None:
        ...
        with pytest.raises(OrbyteError, match="unexpected host"):
    ...
    def test_rejects_non_https_link(self) -> None:
        ...
        with pytest.raises(OrbyteError, match="must use https"):
            self.client._parse_next_link(header)
...
class TestLoadFromCheckpoint:
    ...
    def test_fatal_auth_failure_during_stage_fetch_propagates(self) -> None:
        ...
        with patch("orbyte.connectors.canvas.client.rl_requests") as mock_requests:
    ...
    def test_security_failure_during_stage_fetch_propagates(self) -> None:
        ...
        with patch.object(
            connector,
            "_fetch_stage_page",
            side_effect=OrbyteError(OrbyteErrorCode.BAD_GATEWAY, "bad next link"),
        ):
            with pytest.raises(OrbyteError, match="bad next link"):
                _run_checkpoint(connector, cp)
