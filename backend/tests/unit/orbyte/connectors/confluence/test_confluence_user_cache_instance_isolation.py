...
import orbyte.connectors.confluence.orbyte_confluence as orbyte_confluence
...
def test_username_email_cache_is_instance_isolated() -> None:
    orbyte_confluence._USER_EMAIL_CACHE.clear()
    username = "jsmith"
    ...
    assert (
        orbyte_confluence.get_user_email_from_username__server(client_a, username)
        == "a@example.com"
    )
    ...
    assert (
        orbyte_confluence.get_user_email_from_username__server(client_b, username)
        == "b@example.com"
    )
...
def test_userkey_email_cache_is_instance_isolated() -> None:
    orbyte_confluence._USER_KEY_TO_EMAIL_CACHE.clear()
    user_key = "ff8080816f"
    client_a = _client("https://a.example.com")
    ...
    assert (
        orbyte_confluence.get_user_email_from_userkey__server(client_a, user_key)
        == "a@example.com"
    )
    assert (
        orbyte_confluence.get_user_email_from_userkey__server(client_b, user_key)
        == "b@example.com"
    )
...
def test_display_name_cache_is_instance_isolated() -> None:
    orbyte_confluence._USER_ID_TO_DISPLAY_NAME_CACHE.clear()
    user_id = "user-1"
    client_a = _client("https://a.example.com")
    ...
    assert orbyte_confluence._get_user(client_a, user_id) == "Alice A"
    # same id on a different instance must not see instance A's display name
    assert orbyte_confluence._get_user(client_b, user_id) == "Bob B"
