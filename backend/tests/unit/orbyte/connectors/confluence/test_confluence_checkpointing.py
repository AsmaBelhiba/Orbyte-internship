...
from orbyte.connectors.confluence.connector import ConfluenceCheckpoint
...
from tests.unit.orbyte.connectors.utils import (
...
def confluence_connector(
    confluence_base_url: str, space_key: str, mock_confluence_client: OrbyteConfluence
    ...
    with patch("orbyte.connectors.confluence.connector._SLIM_DOC_BATCH_SIZE", 2):
        yield connector
...
def test_load_from_checkpoint_with_page_processing_error(
    ...
    with patch(
        "orbyte.connectors.confluence.connector.ConfluenceConnector._convert_page_to_document",
...
def test_validate_connector_settings_errors(
    ...
    with patch(
        "orbyte.connectors.confluence.orbyte_confluence.OrbyteConfluence.retrieve_confluence_spaces"
    ) as mock_retrieve:
