...
def test_access_key_auth_passes_region(region_name: str | None) -> None:
    ...
    with patch("orbyte.connectors.blob.connector.boto3.Session") as mock_session_cls:
...
def test_instance_role_auth_passes_region(region_name: str | None) -> None:
    ...
    connector = _make_connector(region_name)
    with patch("orbyte.connectors.blob.connector.boto3.client") as mock_client:
        with patch.object(BlobStorageConnector, "_detect_bucket_region"):
            connector.load_credentials(credentials)
...
def test_iam_role_auth_passes_region_to_sts_and_s3() -> None:
    credentials: dict[str, Any] = {
        ...
        "aws_role_arn": "arn:aws-us-gov:iam::123456789012:role/orbyte-test",
    }
    ...
    with (
        patch(
            "orbyte.connectors.blob.connector.boto3.client", return_value=mock_sts
        ) as mock_client,
        patch("orbyte.connectors.blob.connector.boto3.Session") as mock_session_cls,
