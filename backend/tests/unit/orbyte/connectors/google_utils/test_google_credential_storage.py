...
from orbyte.configs.constants import KV_CRED_KEY
...
from orbyte.server.documents.models import GoogleAppCredentials
from orbyte.server.documents.models import GoogleAppWebCredentials
from orbyte.server.documents.models import GoogleServiceAccountKey
...
def test_get_auth_url_accepts_dict_and_legacy_string(
    ...
    monkeypatch.setattr(
        "orbyte.connectors.google_utils.google_kv.fetch_credential_by_id_for_user",
        _fetch_credential_by_id_for_user,
    )
    monkeypatch.setattr(
        "orbyte.connectors.google_utils.google_kv.update_credential_json",
        _update_credential_json,
    )
    monkeypatch.setattr(
        "orbyte.connectors.google_utils.google_kv.upsert_encrypted_kv",
        _upsert_encrypted_kv,
    )
    monkeypatch.setattr(
        "orbyte.connectors.google_utils.google_kv.InstalledAppFlow.from_client_config",
        _from_client_config,
    )
...
def test_update_credential_access_tokens_restores_pkce_verifier(
    ...
    monkeypatch.setattr(
        "orbyte.connectors.google_utils.google_kv.load_encrypted_kv",
        _load_encrypted_kv,
    )
    monkeypatch.setattr(
        "orbyte.connectors.google_utils.google_kv.delete_encrypted_kv",
    ...
    monkeypatch.setattr(
        "orbyte.connectors.google_utils.google_kv._get_current_oauth_user",
...
def test_verify_csrf_rejects_and_drops_expired_state(monkeypatch: Any) -> None:
    ...
    assert exc_info.value.error_code == OrbyteErrorCode.CSRF_FAILURE
