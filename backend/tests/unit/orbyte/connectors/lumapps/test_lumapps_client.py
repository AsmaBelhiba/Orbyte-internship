...
def _client_with_token() -> tuple[OrbyteLumApps, MagicMock]:
    client = OrbyteLumApps(
...
def _no_sleep(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "orbyte.connectors.lumapps.client.time.sleep", lambda _seconds: None
    )
...
def test_no_backoff_sleep_on_final_attempt(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("orbyte.connectors.lumapps.client.time.sleep", sleep_mock)
    client, session = _client_with_token()
    session.request.side_effect = [_response(503) for _ in range(5)]
