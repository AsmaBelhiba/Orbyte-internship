...
from orbyte.connectors.gong.connector import GongConnectorCheckpoint
...
class TestGongConnectorCheckpoint:
    ...
    def test_missing_call_details_yields_failure(
        ...
        with patch(
            "orbyte.connectors.gong.connector.time.time", side_effect=_advance_clock
        ):
    ...
    def test_pending_retry_does_not_block_on_time_sleep(
        ...
        with (
            patch("orbyte.connectors.gong.connector.time.sleep") as mock_sleep,
            patch(
                "orbyte.connectors.gong.connector.time.time", side_effect=_advance_clock
            ),
        ):
