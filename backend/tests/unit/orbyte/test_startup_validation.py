...
class TestValidateNoVectorDbSettings:
    @patch("orbyte.main.DISABLE_VECTOR_DB", False)
    def test_no_error_when_vector_db_enabled(self) -> None:
        from orbyte.main import validate_no_vector_db_settings

        validate_no_vector_db_settings()

    @patch("orbyte.main.DISABLE_VECTOR_DB", True)
    @patch("orbyte.main.MULTI_TENANT", False)
    @patch("orbyte.server.features.build.configs.ENABLE_CRAFT", False)
    ...
    @patch("orbyte.main.DISABLE_VECTOR_DB", True)
    @patch("orbyte.main.MULTI_TENANT", True)
    ...
    @patch("orbyte.main.DISABLE_VECTOR_DB", True)
    @patch("orbyte.main.MULTI_TENANT", False)
    @patch("orbyte.server.features.build.configs.ENABLE_CRAFT", True)
