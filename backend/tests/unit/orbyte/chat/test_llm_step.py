...
from orbyte.chat import llm_step as llm_step_module
from orbyte.chat.llm_step import _extract_tool_call_kickoffs
from orbyte.chat.llm_step import _increment_turns
from orbyte.chat.llm_step import _LeadingToolCallLeakFilter
from orbyte.chat.llm_step import _parse_tool_args_to_dict
from orbyte.chat.llm_step import _resolve_tool_arguments
from orbyte.chat.llm_step import _XmlToolCallContentFilter
from orbyte.chat.llm_step import extract_tool_calls_from_response_text
...
from orbyte.chat.models import ToolCallSimple
...
from orbyte.llm.well_known_providers.constants import AZURE_PROVIDER_NAME
from orbyte.llm.well_known_providers.constants import OPENAI_PROVIDER_NAME
...
<parameter name="queries" string="false">["Orbyte documentation", "Orbyte docs", "Orbyte platform"]</parameter>
</invoke>
</function_calls>
"""
        tool_calls = extract_tool_calls_from_response_text(
        ...
        assert tool_calls[0].tool_args == {
            "queries": ["Orbyte documentation", "Orbyte docs", "Orbyte platform"]
        }
...
<parameter name="queries" string="false">["Orbyte docs"]</parameter>
</invoke>
</function_calls>
"""
        tool_calls = extract_tool_calls_from_response_text(
...
class TestXmlToolCallContentFilter:
    ...
    def test_strips_function_calls_block_split_across_chunks(self) -> None:
        ...
        chunks = [
            ...
            '<parameter name="queries" string="false">["Orbyte docs"]',
...
def _run_leak_filter(
    chunks: list[str], tool_defs: list[dict] | None = None
) -> tuple[str, _LeadingToolCallLeakFilter]:
    f = _LeadingToolCallLeakFilter(tool_defs or [])
    out = "".join(f.process(chunk) for chunk in chunks) + f.flush()
    return out, f
...
class TestLeadingToolCallLeakFilter:
    """Tests for the JSON/code-fence tool-call leak guard. Complements
    TestXmlToolCallContentFilter above, which covers the Anthropic-style XML
    leak this filter doesn't handle.
    """

    def test_plain_prose_passes_through_untouched(self) -> None:
        text = "Simon Dippenaar is a South African-based attorney known for..."
        out, f = _run_leak_filter(_chunk(text, 3))
        assert out == text
        assert f.matched_tool_call is None
        assert f.suppressed_raw_text is None

    def test_json_matching_real_tool_becomes_tool_call(self) -> None:
        json_call = (
            '{"name": "internal_search", "arguments": {"queries": ["divorce law"]}}'
        )
        out, f = _run_leak_filter(_chunk(json_call, 5), [_SEARCH_TOOL_DEF])
        assert out == ""
        assert f.matched_tool_call == (
            "internal_search",
            {"queries": ["divorce law"]},
        )
        assert f.suppressed_raw_text is None

    def test_json_naming_unoffered_tool_is_suppressed(self) -> None:
        leaked = '{"name": "open_url", "parameters": {"urls": ["https://example.com"]}}'
        out, f = _run_leak_filter(_chunk(leaked, 7), [_SEARCH_TOOL_DEF])
        assert out == ""
        assert f.matched_tool_call is None
        assert f.suppressed_raw_text == leaked

    def test_legitimate_json_data_is_shown_unchanged(self) -> None:
        # A plain JSON record (e.g. the user asked for JSON output) must never
        # be treated as a tool-call attempt just because it starts with '{'.
        data = '{"name": "John Smith", "age": 30}'
        out, f = _run_leak_filter(_chunk(data, 4), [_SEARCH_TOOL_DEF])
        assert out == data
        assert f.suppressed_raw_text is None
        assert f.matched_tool_call is None

    def test_real_code_fence_is_shown_unchanged(self) -> None:
        code = "```python\nfor i in range(5):\n    print(i)\n```"
        out, f = _run_leak_filter(_chunk(code, 6))
        assert out == code
        assert f.suppressed_raw_text is None
        assert f.matched_tool_call is None

    def test_print_wrapped_answer_is_unwrapped_to_plain_text(self) -> None:
        wrapped = (
            "```python\n"
            'print("Uncontested divorce is a divorce in which both spouses '
            'agree on all major issues.")\n'
            "```"
        )
        out, _ = _run_leak_filter(_chunk(wrapped, 9))
        assert out == (
            "Uncontested divorce is a divorce in which both spouses agree "
            "on all major issues."
        )

    def test_multiple_print_calls_are_joined(self) -> None:
        wrapped = '```\nprint("Part one.")\nprint("Part two.")\n```'
        out, _ = _run_leak_filter(_chunk(wrapped, 5))
        assert out == "Part one.\n\nPart two."

    def test_non_json_prose_starting_with_brace_passes_through(self) -> None:
        text = "{this is just a sentence that happens to start with a brace} and continues."
        out, f = _run_leak_filter(_chunk(text, 4))
        assert out == text
        assert f.suppressed_raw_text is None

    def test_truncated_hallucination_suppressed_on_flush(self) -> None:
        truncated = '{"name": "open_url", "parameters": {"urls": ["https://exam'
        out, f = _run_leak_filter(_chunk(truncated, 6), [_SEARCH_TOOL_DEF])
        assert out == ""
        assert f.suppressed_raw_text == truncated

    def test_truncated_non_tool_json_fails_open(self) -> None:
        truncated = '{"just": "some data that never clo'
        out, f = _run_leak_filter(_chunk(truncated, 6), [_SEARCH_TOOL_DEF])
        assert out == truncated
        assert f.suppressed_raw_text is None

    def test_mid_answer_code_fence_never_touched(self) -> None:
        # Once real prose has already streamed, the guard must be fully
        # resolved and never intercept a later, legitimate code block - even
        # one that would otherwise match the print-unwrap heuristic.
        f = _LeadingToolCallLeakFilter([])
        chunks = ["Here is an example: ", '```\nprint("hello")\n```', " done."]
        out = "".join(f.process(c) for c in chunks) + f.flush()
        assert out == "Here is an example: " + '```\nprint("hello")\n```' + " done."

    def test_json_fenced_hallucination_is_suppressed(self) -> None:
        leaked = '```json\n{"name": "python", "parameters": {"code": "print(1)"}}\n```'
        out, f = _run_leak_filter(_chunk(leaked, 8), [_SEARCH_TOOL_DEF])
        assert out == ""
        assert f.suppressed_raw_text is not None
...
class TestEmptyAnswerRecovery:
    ...
    def _run(
        ...
        *,
        ...
        from orbyte.chat import llm_step as _llm_step_module
        from orbyte.chat.citation_processor import CitationMode
        from orbyte.chat.citation_processor import DynamicCitationProcessor
        from orbyte.chat.llm_step import run_llm_step_pkt_generator
