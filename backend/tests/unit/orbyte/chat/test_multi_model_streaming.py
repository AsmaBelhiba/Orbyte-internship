...
from orbyte.server.query_and_chat.streaming_models import ChatHeartbeat
...
def _restore_ee_version() -> Generator[None, None, None]:
    ...
    Importing orbyte.chat.process_message triggers set_is_ee_based_on_env_variable()
...
def _run_models_collect(setup: MagicMock) -> list:
    ...
    from orbyte.chat.process_message import _run_models
...
class TestRunModels:
    ...
    def test_n1_overall_stop_from_llm_loop_passes_through(self) -> None:
        ...
        with (
            patch("orbyte.chat.process_message.run_llm_loop", side_effect=emit_stop),
            patch("orbyte.chat.process_message.run_deep_research_llm_loop"),
            patch("orbyte.chat.process_message.construct_tools", return_value={}),
            patch("orbyte.chat.process_message.llm_loop_completion_handle"),
            patch(
                "orbyte.chat.process_message.get_llm_token_counter",
    ...
    def test_idle_gap_emits_chat_heartbeat(self) -> None:
        ...
        with (
            patch(
                "orbyte.chat.process_message.CHAT_HEARTBEAT_INTERVAL_S",
                0.05,
            ),
            patch(
                "orbyte.chat.process_message.run_llm_loop",
    ...
    def test_n1_emitted_packet_has_model_index_zero(self) -> None:
        ...
        with (
            patch("orbyte.chat.process_message.run_llm_loop", side_effect=emit_one),
            patch("orbyte.chat.process_message.run_deep_research_llm_loop"),
            patch("orbyte.chat.process_message.construct_tools", return_value={}),
            patch("orbyte.chat.process_message.llm_loop_completion_handle"),
            patch(
    ...
    def test_model_error_yields_streaming_error(self) -> None:
        ...
        with (
            patch("orbyte.chat.process_message.run_llm_loop", side_effect=always_fail),
            patch("orbyte.chat.process_message.run_deep_research_llm_loop"),
            patch("orbyte.chat.process_message.construct_tools", return_value={}),
            patch("orbyte.chat.process_message.llm_loop_completion_handle"),
            patch(
    ...
    def test_context_window_overflow_surfaces_as_context_too_long(self) -> None:
        ...
        with (
            patch("orbyte.chat.process_message.run_llm_loop", side_effect=overflow),
            patch("orbyte.chat.process_message.run_deep_research_llm_loop"),
            patch("orbyte.chat.process_message.construct_tools", return_value={}),
            patch("orbyte.chat.process_message.llm_loop_completion_handle"),
            patch(
    ...
    def test_cancellation_yields_user_cancelled_stop(self) -> None:
        ...
        with (
            patch("orbyte.chat.process_message.run_llm_loop", side_effect=slow_llm),
            patch("orbyte.chat.process_message.run_deep_research_llm_loop"),
            patch("orbyte.chat.process_message.construct_tools", return_value={}),
            patch(
                "orbyte.chat.process_message.llm_loop_completion_handle",
    ...
    def test_completion_handle_called_for_each_successful_model(self) -> None:
        ...
        with (
            patch("orbyte.chat.process_message.run_llm_loop"),
            patch("orbyte.chat.process_message.run_deep_research_llm_loop"),
            patch("orbyte.chat.process_message.construct_tools", return_value={}),
            patch(
                "orbyte.chat.process_message.llm_loop_completion_handle"
    ...
    def test_http_disconnect_error_saves_message_once(self) -> None:
        ...
        with (
            ...
            patch(
                "orbyte.chat.process_message.get_session_with_current_tenant",
    ...
    def test_stop_button_does_not_call_completion_for_errored_model(self) -> None:
        ...
        with (
            patch("orbyte.chat.process_message.run_llm_loop", side_effect=fail_model_0),
            patch("orbyte.chat.process_message.run_deep_research_llm_loop"),
            patch("orbyte.chat.process_message.construct_tools", return_value={}),
            patch(
                "orbyte.chat.process_message.llm_loop_completion_handle",
    ...
    def test_external_state_container_used_for_model_zero(self) -> None:
        ...
        with (
            patch("orbyte.chat.process_message.run_llm_loop") as mock_llm,
            patch("orbyte.chat.process_message.run_deep_research_llm_loop"),
            patch("orbyte.chat.process_message.construct_tools", return_value={}),
            patch("orbyte.chat.process_message.llm_loop_completion_handle"),
            patch(
