...
from orbyte.chat.process_message import _resolve_query_processing_hook_result
...
def test_null_query_raises_query_rejected() -> None:
    ...
    assert exc_info.value.error_code is OrbyteErrorCode.QUERY_REJECTED
...
def test_fallback_rejection_message_when_none() -> None:
    """No rejection_message → generic fallback used in OrbyteError detail."""
    with pytest.raises(OrbyteError) as exc_info:
        _resolve_query_processing_hook_result(
