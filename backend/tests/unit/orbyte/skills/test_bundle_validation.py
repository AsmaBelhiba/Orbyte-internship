...
from orbyte.skills.bundle import _ZIP_UNIX_CREATE_SYSTEM
from orbyte.skills.bundle import compute_bundle_sha256
from orbyte.skills.bundle import parse_skill_md_metadata
from orbyte.skills.bundle import read_custom_bundle_instructions
from orbyte.skills.bundle import rewrite_custom_bundle_skill_md
from orbyte.skills.bundle import slug_from_filename
from orbyte.skills.bundle import slug_from_skill_name
from orbyte.skills.bundle import strip_skill_md_frontmatter
from orbyte.skills.bundle import validate_and_normalize_custom_bundle
...
def test_slug_from_skill_name_rejects_name_without_alphanumerics() -> None:
    with pytest.raises(OrbyteError, match="at least one letter or number"):
...
def test_validate_and_normalize_rejects_non_zip() -> None:
    with pytest.raises(OrbyteError, match="not a valid zip"):
...
def test_validate_and_normalize_rejects_missing_skill_md() -> None:
    ...
    with pytest.raises(OrbyteError, match="SKILL.md missing at bundle root"):
...
def test_normalizer_rejects_files_outside_wrapper_directory() -> None:
    ...
    with pytest.raises(OrbyteError, match="outside the directory"):
        validate_and_normalize_custom_bundle(zip_bytes, slug="hello")
...
def test_normalizer_rejects_duplicate_output_paths() -> None:
    ...
    with pytest.raises(OrbyteError, match="duplicate path 'scripts/run.sh'"):
        validate_and_normalize_custom_bundle(zip_bytes, slug="hello")
...
def test_normalizer_rejects_file_descendant_path_collisions(
    ...
    with pytest.raises(OrbyteError, match="conflicting path"):
        validate_and_normalize_custom_bundle(zip_bytes, slug="hello")
...
def test_validator_rejects_template_file() -> None:
    ...
    with pytest.raises(OrbyteError, match="cannot ship templates"):
        validate_and_normalize_custom_bundle(zip_bytes, slug="hello")
...
def test_validator_rejects_oversized_single_file() -> None:
    ...
    with pytest.raises(OrbyteError, match="exceeds"):
...
def test_normalizer_preserves_size_error_when_entry_close_fails(
    ...
    assert exc_info.value.error_code == OrbyteErrorCode.PAYLOAD_TOO_LARGE
...
def test_validator_rejects_oversized_total() -> None:
    ...
    with pytest.raises(OrbyteError, match="uncompressed"):
        validate_and_normalize_custom_bundle(
            zip_bytes,
            slug="hello",
...
def test_validator_rejects_invalid_slug(bad_slug: str) -> None:
    with pytest.raises(OrbyteError, match="invalid slug"):
...
def test_validator_rejects_reserved_slug() -> None:
    ...
    with pytest.raises(OrbyteError, match="reserved"):
...
def test_rewrite_custom_bundle_skill_md_rejects_oversized_skill_md_before_zip_read(
    ...
    monkeypatch.setattr("orbyte.skills.bundle.DEFAULT_PER_FILE_MAX_BYTES", 128)
...
def test_rewrite_custom_bundle_skill_md_rejects_missing_skill_md() -> None:
    ...
    assert exc_info.value.error_code == OrbyteErrorCode.INTERNAL_ERROR
...
def test_validator_rejects_unsupported_compression() -> None:
    ...
    from zf.open() — we must translate that to OrbyteError, not a 500."""
    zip_bytes = _zip_with_patched_compression_method(VALID_SKILL_MD, method=99)
    with pytest.raises(OrbyteError, match="cannot read"):
        validate_and_normalize_custom_bundle(zip_bytes, slug="hello")
