...
`orbyte.indexing.persistent_indexing`."""
...
def test_record_generic_failure_persists_via_create_index_attempt_error() -> None:
    ...
    with (
        patch(
            "orbyte.indexing.persistent_indexing.get_session_with_current_tenant"
        ) as mock_session_ctx,
        patch(
            "orbyte.indexing.persistent_indexing.create_index_attempt_error"
        ) as mock_create,
    ):
