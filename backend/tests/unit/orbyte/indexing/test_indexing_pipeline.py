...
from orbyte.indexing.indexing_pipeline import _apply_document_ingestion_hook
from orbyte.indexing.indexing_pipeline import add_contextual_summaries
from orbyte.indexing.indexing_pipeline import filter_documents
from orbyte.indexing.indexing_pipeline import get_docs_to_update
from orbyte.indexing.indexing_pipeline import process_image_sections
from orbyte.llm.constants import LlmProviderNames
from orbyte.llm.model_capabilities import get_max_input_tokens
from orbyte.llm.model_response import Choice
from orbyte.llm.model_response import Message
...
def test_filter_documents_exceeding_max_chars() -> None:
    ...
    with patch("orbyte.indexing.indexing_pipeline.MAX_DOCUMENT_CHARS", limit):
        docs, failures = filter_documents([doc])
    assert len(docs) == 0
...
@patch("orbyte.llm.model_capabilities.GEN_AI_MAX_TOKENS", 4096)
...
_PATCH_EXECUTE_HOOK = "orbyte.indexing.indexing_pipeline.execute_hook"
_PATCH_GET_SESSION = "orbyte.indexing.indexing_pipeline.get_session_with_current_tenant"
...
_PATCH_MULTI_TENANT = "orbyte.indexing.indexing_pipeline.MULTI_TENANT"
_PATCH_GET_CC_PAIR = "orbyte.indexing.indexing_pipeline.get_connector_credential_pair"
_PATCH_GET_SESSION_AW = (
    "orbyte.indexing.indexing_pipeline.get_session_with_current_tenant"
)
_PATCH_EXECUTE_HOOK = "orbyte.indexing.indexing_pipeline.execute_hook"
...
def test_document_push_skipped_when_from_beginning() -> None:
    from orbyte.indexing.indexing_pipeline import _maybe_push_documents
...
def test_document_push_skipped_in_multi_tenant_mode() -> None:
    ...
    with (
        ...
        patch(
            "orbyte.indexing.indexing_pipeline.get_document_push_config",
            return_value=None,
        ),
        patch(_PATCH_EXECUTE_HOOK) as mock_hook,
    ):
...
def test_document_push_config_sink_skipped_in_multi_tenant_mode() -> None:
    from orbyte.indexing.indexing_pipeline import _maybe_push_documents
    from orbyte.utils.external_endpoint import ExternalEndpointConfig
    ...
    with (
        ...
        patch(_PATCH_EXECUTE_HOOK) as mock_hook,
        patch(
            "orbyte.indexing.indexing_pipeline.push_document_via_config"
        ) as mock_config_push,
    ):
        _maybe_push_documents(_make_adapter(), [doc], _make_insertion_records(["doc1"]))
...
_PATCH_PREFIX = "orbyte.indexing.indexing_pipeline"
