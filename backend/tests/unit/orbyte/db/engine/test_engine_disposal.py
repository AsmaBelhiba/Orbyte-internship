...
import orbyte.db.engine.async_sql_engine as async_sql_engine
...
async def test_lifespan_shutdown_disposes_all_three_engines() -> None:
    ...
    """
    from orbyte import main as orbyte_main
    ...
    with ExitStack() as stack:
        ...
        reset_async = stack.enter_context(
            patch.object(orbyte_main, "reset_sqlalchemy_async_engine", new=AsyncMock())
        )
        stack.enter_context(
            patch.object(
                orbyte_main,
        ...
        stack.enter_context(patch.object(orbyte_main, "setup_tracing"))
        stack.enter_context(
        ...
        stack.enter_context(patch.object(orbyte_main, "optional_telemetry"))
        stack.enter_context(patch.object(orbyte_main, "MULTI_TENANT", False))
        stack.enter_context(patch.object(orbyte_main, "RATE_LIMITING_ENABLED", False))
        stack.enter_context(patch.object(orbyte_main, "DISABLE_VECTOR_DB", False))
        stack.enter_context(patch.object(orbyte_main, "OAUTH_CLIENT_ID", ""))
        stack.enter_context(patch.object(orbyte_main, "OAUTH_CLIENT_SECRET", ""))
        stack.enter_context(patch.object(orbyte_main, "SYSTEM_RECURSION_LIMIT", None))

        async with orbyte_main.lifespan(MagicMock()):
