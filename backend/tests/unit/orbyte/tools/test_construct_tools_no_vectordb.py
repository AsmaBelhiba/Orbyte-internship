...
OPEN_URL_MODULE = "orbyte.tools.tool_implementations.open_url.open_url_tool"
...
class TestSearchToolAvailability:
    ...
    @patch("orbyte.db.connector.check_user_files_exist", return_value=True)
    @patch(
...
class TestForceAddSearchToolGuard:
    def test_force_add_block_checks_disable_vector_db(self) -> None:
        ...
        from orbyte.tools.tool_constructor import _construct_tools_impl
...
class TestValidateVectorDbKnowledge:
    @patch(
        "orbyte.server.features.persona.api.DISABLE_VECTOR_DB",
        True,
    )
    def test_rejects_document_set_ids(self) -> None:
        from fastapi import HTTPException

        from orbyte.server.features.persona.api import _validate_vector_db_knowledge
