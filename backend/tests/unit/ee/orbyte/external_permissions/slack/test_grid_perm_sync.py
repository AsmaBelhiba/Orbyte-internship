...
from ee.orbyte.external_permissions.slack.channel_access import (
    get_channel_access as ee_get_channel_access,
)
from ee.orbyte.external_permissions.slack.doc_sync import _fetch_channel_permissions
from ee.orbyte.external_permissions.slack.doc_sync import _fetch_workspace_permissions
from ee.orbyte.external_permissions.slack.doc_sync import _get_slack_document_access
from ee.orbyte.external_permissions.slack.utils import fetch_team_user_emails
from ee.orbyte.external_permissions.slack.utils import fetch_user_id_to_email_map
...
class TestFetchUserIdToEmailMap:
    def test_non_grid_calls_users_list_without_team_id(self) -> None:
        client = MagicMock()
        with patch(
            "ee.orbyte.external_permissions.slack.utils.make_paginated_slack_api_call"
        ) as mock_paginate:
...
class TestFetchTeamUserEmails:
    def test_returns_per_team_email_sets(self) -> None:
        client = MagicMock()
        with patch(
            "orbyte.connectors.slack.utils.make_paginated_slack_api_call"
        ) as mock_paginate:
            mock_paginate.side_effect = [
                iter([{"members": [{"id": "U1", "profile": {"email": "u1@x.com"}}]}]),
                iter(
...
class TestFetchChannelPermissionsGrid:
    def test_public_channel_scoped_to_its_workspace_users(self) -> None:
        ...
        with patch(
            "ee.orbyte.external_permissions.slack.doc_sync.get_channels_across_teams"
        ) as mock_get:
    ...
    def test_non_grid_public_channel_not_overridden(self) -> None:
        ...
        with patch(
            "ee.orbyte.external_permissions.slack.doc_sync.get_channels"
        ) as mock_get:
    ...
    def test_channel_filter_limits_private_member_fetches(self) -> None:
        ...
        with (
            ...
            patch(
                "ee.orbyte.external_permissions.slack.doc_sync.make_paginated_slack_api_call"
            ) as mock_paginate,
        ):
...
class TestEEGetChannelAccessGrid:
    ...
    def test_public_channel_grid_falls_back_to_is_public_when_union_exceeds_cap(
        ...
        from orbyte.access.models import ExternalAccess as _EA
    ...
    def test_private_channel_uses_members_path_regardless_of_grid(self) -> None:
        ...
        with (
            patch(
                "ee.orbyte.external_permissions.slack.channel_access.make_paginated_slack_api_call"
            ) as mock_paginate,
            patch(
                "ee.orbyte.external_permissions.slack.channel_access.expert_info_from_slack_id"
            ) as mock_expert,
        ):
