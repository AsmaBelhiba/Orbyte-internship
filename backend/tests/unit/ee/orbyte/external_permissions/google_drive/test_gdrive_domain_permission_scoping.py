...
from ee.orbyte.external_permissions.google_drive.doc_sync import (
...
def test_indexing_path_prefixes_domain_group() -> None:
    ...
    expected = build_ext_group_name_for_orbyte(
...
def _folder_access(
    ...
    with patch(
        "ee.orbyte.external_permissions.google_drive.doc_sync.get_permissions_by_ids",
...
def test_user_acl_carries_synced_domain_group() -> None:
    from ee.orbyte.access.access import _get_acl_for_user

    user = cast(
        User, SimpleNamespace(id="u1", email="Alice@CompanyA.com", is_anonymous=False)
    )
    domain_group = build_ext_group_name_for_orbyte(
        f"domain:{COMPANY_DOMAIN}", DocumentSource.GOOGLE_DRIVE
    )
    with (
        patch("ee.orbyte.access.access.fetch_user_groups_for_user", return_value=[]),
        patch(
            "ee.orbyte.access.access.fetch_external_groups_for_user",
...
def test_group_sync_domain_group_uses_real_roster() -> None:
    from ee.orbyte.external_permissions.google_drive import group_sync
