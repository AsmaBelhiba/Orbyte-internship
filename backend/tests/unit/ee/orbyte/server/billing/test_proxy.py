...
class TestProxySeatUpdate:
    ...
    @pytest.mark.asyncio
    @patch("ee.orbyte.server.tenants.proxy.forward_to_control_plane")
    async def test_proxies_seat_update(
        ...
        from ee.orbyte.server.tenants.proxy import proxy_seat_update
...
class TestVerifyLicenseAuth:
    ...
    @pytest.mark.asyncio
    @patch("ee.orbyte.server.tenants.proxy.LICENSE_ENFORCEMENT_ENABLED", True)
    @patch("ee.orbyte.server.tenants.proxy.verify_license_signature")
    @patch("ee.orbyte.server.tenants.proxy.is_license_valid")
