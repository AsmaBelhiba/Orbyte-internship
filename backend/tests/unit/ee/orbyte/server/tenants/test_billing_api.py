...
class TestGetStripePublishableKey:
    ...
    def setup_method(self) -> None:
        ...
        import ee.orbyte.server.tenants.billing_api as billing_api

        billing_api._stripe_publishable_key_cache = None

    @pytest.mark.asyncio
    @patch("ee.orbyte.server.tenants.billing_api.STRIPE_PUBLISHABLE_KEY_OVERRIDE", None)
    @patch(
        "ee.orbyte.server.tenants.billing_api.STRIPE_PUBLISHABLE_KEY_URL",
    ...
    async def test_fetches_from_s3_when_no_override(self) -> None:
        ...
        from ee.orbyte.server.tenants.billing_api import get_stripe_publishable_key
    ...
    @pytest.mark.asyncio
    @patch(
        "ee.orbyte.server.tenants.billing_api.STRIPE_PUBLISHABLE_KEY_OVERRIDE",
        "pk_test_override123",
    )
    ...
    @pytest.mark.asyncio
    @patch("ee.orbyte.server.tenants.billing_api.STRIPE_PUBLISHABLE_KEY_OVERRIDE", None)
    @patch("ee.orbyte.server.tenants.billing_api.STRIPE_PUBLISHABLE_KEY_URL", None)
...
class TestCreateSubscriptionSession:
    ...
    async def test_returns_session_id_and_url_for_new_checkout(self) -> None:
        ...
        from ee.orbyte.server.tenants.billing_api import create_subscription_session
        from ee.orbyte.server.tenants.models import StripeCheckoutSessionResult
        from shared_configs.contextvars import CURRENT_TENANT_ID_CONTEXTVAR

        token = CURRENT_TENANT_ID_CONTEXTVAR.set("tenant_123")
        try:
            with patch(
                "ee.orbyte.server.tenants.billing_api.fetch_stripe_checkout_session",
...
class TestFetchStripeCheckoutSession:
    ...
    def test_raises_when_control_plane_returns_no_url(self) -> None:
        ...
        with (
            patch(
                "ee.orbyte.server.tenants.billing.generate_data_plane_token",
                return_value="cp_token",
            ),
            patch(
                "ee.orbyte.server.tenants.billing.requests.post",
                return_value=mock_response,
            ),
        ):
