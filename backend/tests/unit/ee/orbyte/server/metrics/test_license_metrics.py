...
from ee.orbyte.server.metrics.license_metrics import LicenseMetricsCollector
...
class TestLicenseMetricsCollector:
    def test_emits_seat_and_expiry_gauges(self) -> None:
        ...
        with (
            patch("ee.orbyte.server.metrics.license_metrics.MULTI_TENANT", False),
            patch("ee.orbyte.server.metrics.license_metrics.get_session_with_tenant"),
            patch(
                "ee.orbyte.server.metrics.license_metrics.get_license_metadata",
                return_value=_metadata(10, ApplicationStatus.ACTIVE),
            ),
            patch(
                "ee.orbyte.server.metrics.license_metrics.get_used_seats",
                return_value=7,
            ),
        ):
            values = _values(collector.collect())

        assert values["orbyte_license_seats_total"] == 10
        assert values["orbyte_license_seats_used"] == 7
        assert values["orbyte_license_seats_available"] == 3
        assert values["orbyte_license_expires_timestamp_seconds"] == (
            _EXPIRES_AT.timestamp()
        )
        assert values["orbyte_license_active"] == 1.0
    ...
    def test_active_one_during_grace_period(self) -> None:
        ...
        assert values["orbyte_license_active"] == 1.0
        assert values["orbyte_license_seats_available"] == 1
    ...
    def test_active_zero_when_gated(self) -> None:
        ...
        assert values["orbyte_license_active"] == 0.0
        assert values["orbyte_license_seats_available"] == 0
    ...
    def test_empty_when_multi_tenant(self) -> None:
        collector = LicenseMetricsCollector(cache_ttl=0)
        with patch("ee.orbyte.server.metrics.license_metrics.MULTI_TENANT", True):
