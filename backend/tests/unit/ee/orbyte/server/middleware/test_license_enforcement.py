...
from ee.orbyte.server.middleware.license_enforcement import _is_path_allowed
...
class TestLicenseEnforcementMiddleware:
    ...
    @pytest.mark.asyncio
    @patch(
        "ee.orbyte.server.middleware.license_enforcement.LICENSE_ENFORCEMENT_ENABLED",
        True,
    )
    ...
    @patch(
        "ee.orbyte.server.middleware.license_enforcement.get_session_with_current_tenant"
    )
