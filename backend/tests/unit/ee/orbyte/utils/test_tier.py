"""Unit tests for `ee.orbyte.utils.tier`: self-hosted tier resolution
...
from ee.orbyte.server.license.models import CustomerTier
...
@patch("ee.orbyte.utils.tier.LICENSE_ENFORCEMENT_ENABLED", True)
@patch("ee.orbyte.utils.tier.MULTI_TENANT", False)
class TestSelfHostedTierCacheFailure:
    ...
    @patch("ee.orbyte.utils.tier.get_cached_license_metadata")
    ...
    @patch("ee.orbyte.utils.tier.refresh_license_cache")
    @patch("ee.orbyte.utils.tier.get_session_with_current_tenant")
    @patch("ee.orbyte.utils.tier.get_cached_license_metadata")
...
class TestSelfHostedTierLegacyBypass:
    ...
    @patch("ee.orbyte.utils.tier.LICENSE_ENFORCEMENT_ENABLED", False)
    @patch("ee.orbyte.utils.tier.global_version")
    @patch("ee.orbyte.utils.tier.get_cached_license_metadata")
...
class TestTierFromLicenseMetadata:
    ...
    def test_none_metadata_returns_community(self) -> None:
        from ee.orbyte.utils.tier import tier_from_license_metadata
...
class TestRequireBusinessTierForSyncAccess:
    ...
    @patch("ee.orbyte.utils.tier.LICENSE_ENFORCEMENT_ENABLED", True)
    @patch("ee.orbyte.utils.tier.get_tier")
    def test_non_sync_access_passes_without_checking_tier(
        self, mock_get_tier: MagicMock, access_type: AccessType
    ) -> None:
        from ee.orbyte.utils.tier import require_business_tier_for_sync_access
    ...
    def test_sync_at_community_raises_feature_not_available(
        ...
        assert exc_info.value.error_code == OrbyteErrorCode.FEATURE_NOT_AVAILABLE
...
class TestRequireBusinessTierForMultiSSO:
    ...
    def test_below_business_raises(self, mock_get_tier: MagicMock) -> None:
        from ee.orbyte.utils.tier import require_business_tier_for_multi_sso
