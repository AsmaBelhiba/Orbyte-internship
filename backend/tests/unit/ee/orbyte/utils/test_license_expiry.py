...
from ee.orbyte.utils.license_expiry import get_grace_period_end
from ee.orbyte.utils.license_expiry import LICENSE_GRACE_PERIOD_DAYS

NOW = datetime(2026, 5, 1, 12, 0, 0, tzinfo=timezone.utc)


def _patch_now() -> object:
    p = patch("ee.orbyte.utils.license_expiry.datetime")
    mock = p.start()
...
def test_get_expiry_warning_stage_boundaries(
    ...
    with patch("ee.orbyte.utils.license_expiry.datetime") as dt:
...
def _status_with_default_grace(expires: datetime) -> str:
    ...
    from ee.orbyte.utils.license import get_license_status

    payload = MagicMock()
    payload.expires_at = expires
    grace_end = get_grace_period_end(expires)
    with patch("ee.orbyte.utils.license.datetime") as dt_mock:
