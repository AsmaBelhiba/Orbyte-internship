...
from ee.orbyte.server.enterprise_settings.api import fetch_custom_analytics_script
from orbyte.configs.constants import KV_CUSTOM_ANALYTICS_SCRIPT_KEY
...
def test_returns_none_when_tenant_unresolved__multi_tenant() -> None:
    ...
    try:
        with (
            patch("ee.orbyte.server.enterprise_settings.api.MULTI_TENANT", True),
            patch(
                "ee.orbyte.server.enterprise_settings.api.POSTGRES_DEFAULT_SCHEMA",
...
def test_returns_none_when_no_script_set__single_tenant(
    ...
    token = CURRENT_TENANT_ID_CONTEXTVAR.set(POSTGRES_DEFAULT_SCHEMA_STANDARD_VALUE)
    try:
        with patch("ee.orbyte.server.enterprise_settings.api.MULTI_TENANT", False):
            assert fetch_custom_analytics_script() is None
    finally:
        CURRENT_TENANT_ID_CONTEXTVAR.reset(token)
