...
from orbyte.background.celery.tasks.scheduled_tasks.tasks import (
...
def test_timeout_error_event_marks_run_failed_with_timeout_class(
    ...
    monkeypatch.setattr(
        "orbyte.server.features.build.session.manager.build_user_skills_payload",
        lambda *_: ("", "", {}),
    )
...
def test_run_fails_when_wake_fails(
    ...
    monkeypatch.setattr(
        "orbyte.server.features.build.scheduled_tasks.executor.PROVISIONING_WAIT_SECONDS",
        0,
    )
