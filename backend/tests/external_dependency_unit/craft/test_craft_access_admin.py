...
from orbyte.server.features.build import utils as build_utils
from orbyte.server.features.build.api import require_orbyte_craft_enabled
...
from orbyte.server.manage.users import set_user_craft_access
...
def test_admin_batch_disable_and_reset_to_default(
    ...
    with pytest.raises(OrbyteError) as exc_info:
        require_orbyte_craft_enabled(user=target_a)
    ...
    assert is_craft_enabled_for_user(target_a) is True
    assert require_orbyte_craft_enabled(user=target_a) is target_a
