...
from orbyte.background.celery.tasks.shared.tasks import document_by_cc_pair_cleanup_task
...
from orbyte.server.orbyte_api.ingestion import delete_ingestion_doc
...
class TestDeleteIngestionDoc:
    ...
    def test_cleans_up_file_for_ingestion_api_doc(
        ...
        # index integration.
        with patch(
            "orbyte.server.orbyte_api.ingestion.get_all_document_indices",
            return_value=[],
        ):
...
class TestDocumentByCcPairCleanupTask:
    ...
    def test_count_1_branch_cleans_up_file(
        ...
        with patch(
            "orbyte.background.celery.tasks.shared.tasks.get_all_document_indices",
            return_value=[],
        ):
