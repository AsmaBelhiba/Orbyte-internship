...
from orbyte.configs.app_configs import REDIS_PASSWORD
from orbyte.configs.app_configs import REDIS_PORT
from orbyte.configs.app_configs import REDIS_SSL
...
from orbyte.redis.redis_pool import RedisPool
...
def orbyte_redis(
    ...
    if command == OrbyteRedisCommand.purge_connectorsync_taskset:
    ...
    elif command == OrbyteRedisCommand.purge_documentset_taskset:
        return purge_by_match_and_type(
            "*documentset_taskset*", "set", batch, dry_run, r
        )
    elif command == OrbyteRedisCommand.purge_usergroup_taskset:
        return purge_by_match_and_type("*usergroup_taskset*", "set", batch, dry_run, r)
    elif command == OrbyteRedisCommand.purge_locks_blocking_deletion:
        ...
        return 0
    elif command == OrbyteRedisCommand.purge_vespa_syncing:
        return purge_by_match_and_type(
            "*connectorsync:vespa_syncing*", "string", batch, dry_run, r
        )
    elif command == OrbyteRedisCommand.purge_pidbox:
        return purge_by_match_and_type(
            "*reply.celery.pidbox", "list", batch, dry_run, r
        )
    elif command == OrbyteRedisCommand.get_list_element:
        ...
        print(f"{result}")
        return 0
    elif command == OrbyteRedisCommand.get_user_token:
        if not user_email:
        ...
        if token_key:
            ...
            return 2
    elif command == OrbyteRedisCommand.delete_user_token:
        if not user_email:
        ...
        if delete_user_token_from_redis(r, user_email, dry_run):
            ...
            return 2
    elif command == OrbyteRedisCommand.add_invited_user:
        if not user_email:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Orbyte Redis Manager")
    parser.add_argument(
        "--command",
        type=OrbyteRedisCommand,
        help="The command to run",
        choices=list(OrbyteRedisCommand),
        required=True,
    )
