...
Example launch.json entry:
    ...
    args: ["-A", "orbyte.background.celery.versioned_apps.primary", "worker",
...
# Mirrors BUILTIN_SKILLS_PATH (orbyte/skills/built_in.py); kept inline to avoid
# importing orbyte.db into the reloader supervisor just for a path.
_BACKEND_DIR = Path(__file__).resolve().parents[1]
_SKILLS_BUNDLE_DIR = str(_BACKEND_DIR / "orbyte" / "skills" / "builtin")
_SANDBOX_IMAGE_DIR = str(
    _BACKEND_DIR / "orbyte" / "server" / "features" / "build" / "sandbox" / "image"
)
...
if __name__ == "__main__":
    ...
    watch_paths = [p for p in ("./orbyte", "./ee") if os.path.isdir(p)]
    run_process(
        *watch_paths,
        target=_run,
        args=(celery_argv,),
