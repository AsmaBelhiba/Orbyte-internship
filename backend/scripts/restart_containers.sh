...
stop_and_remove_containers() {
  docker stop orbyte_postgres orbyte_vespa orbyte_redis orbyte_minio orbyte_code_interpreter 2>/dev/null || true
  docker rm orbyte_postgres orbyte_vespa orbyte_redis orbyte_minio orbyte_code_interpreter 2>/dev/null || true
...
if [[ -n "$POSTGRES_VOLUME" ]]; then
    docker run -p 5432:5432 --name orbyte_postgres -e POSTGRES_PASSWORD=password -d -v "$POSTGRES_VOLUME":/var/lib/postgresql/data postgres -c max_connections=250
else
    docker run -p 5432:5432 --name orbyte_postgres -e POSTGRES_PASSWORD=password -d postgres -c max_connections=250
fi

# Start the Vespa container with optional volume
echo "Starting Vespa container..."
if [[ -n "$VESPA_VOLUME" ]]; then
    docker run --detach --name orbyte_vespa --hostname vespa-container --publish 8081:8081 --publish 19071:19071 -v "$VESPA_VOLUME":/opt/vespa/var vespaengine/vespa:8
else
    docker run --detach --name orbyte_vespa --hostname vespa-container --publish 8081:8081 --publish 19071:19071 vespaengine/vespa:8
fi
...
if [[ "$KEEP_OPENSEARCH_DATA" == "false" ]]; then
    ...
    docker volume rm orbyte_opensearch-data 2>/dev/null || true
fi
...
if [[ -n "$REDIS_VOLUME" ]]; then
    docker run --detach --name orbyte_redis --publish 6379:6379 -v "$REDIS_VOLUME":/data redis
else
    docker run --detach --name orbyte_redis --publish 6379:6379 redis
fi
...
if [[ -n "$MINIO_VOLUME" ]]; then
    docker run --detach --name orbyte_minio --publish 9004:9000 --publish 9005:9001 -e MINIO_ROOT_USER=minioadmin -e MINIO_ROOT_PASSWORD=minioadmin -v "$MINIO_VOLUME":/data minio/minio server /data --console-address ":9001"
else
    docker run --detach --name orbyte_minio --publish 9004:9000 --publish 9005:9001 -e MINIO_ROOT_USER=minioadmin -e MINIO_ROOT_PASSWORD=minioadmin minio/minio server /data --console-address ":9001"
fi

# Start the Code Interpreter container
echo "Starting Code Interpreter container..."
docker run --detach --name orbyte_code_interpreter --publish 8000:8000 --user root -v /var/run/docker.sock:/var/run/docker.sock onyxdotapp/code-interpreter:latest bash ./entrypoint.sh code-interpreter-api
