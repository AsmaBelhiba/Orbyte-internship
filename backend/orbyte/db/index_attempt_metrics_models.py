...
a leaf module — it imports nothing from the rest of the Orbyte codebase —
so it can be safely imported by both ``orbyte.db.models`` (which needs the
enum as a column type on ``IndexAttemptStageMetric``) and
``orbyte.db.index_attempt_metrics`` (which uses the taxonomy when writing
