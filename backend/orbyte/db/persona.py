...
from orbyte.configs.app_configs import CURATORS_CANNOT_VIEW_OR_EDIT_NON_OWNED_ASSISTANTS
...
from orbyte.db.document_access import get_accessible_documents_by_ids
from orbyte.db.document_set import filter_document_set_ids_by_user_access
from orbyte.db.enums import AccountType
from orbyte.db.enums import PersonaSharePermission
from orbyte.db.hierarchy import filter_accessible_hierarchy_node_ids
...
from orbyte.db.models import PersonaLabel
from orbyte.db.models import StarterMessage
...
from orbyte.db.notification import create_notification
...
from orbyte.server.features.persona.models import PersonaSharedNotificationData
...
def create_update_persona(
    ...
    try:
        ...
        versioned_update_persona_access = fetch_versioned_implementation(
            "orbyte.db.persona", "update_persona_access"
        )
...
def transfer_persona_ownership(
    ...
    override in ee.orbyte.db.persona)."""
    if new_owner_group_id is not None:
        raise NotImplementedError("Orbyte MIT does not support group ownership")
...
def upsert_persona(
    ...
    if tool_ids is not None:
        ...
        # NOTE: MCP tools have been removed from this deployment, so the
        # per-server access check that used to run here (mcp_server_id is
        # always None now) has been removed as well.
