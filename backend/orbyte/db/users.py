...
from orbyte.configs.constants import NO_AUTH_PLACEHOLDER_USER_EMAIL
...
from orbyte.db.models import SamlAccount
...
def validate_user_role_update(
    ...
    if current_account_type == AccountType.BOT:
        raise HTTPException(
            status_code=400,
            detail="To change a Slack User's role, they must first login to Orbyte via the web app.",
        )

    if current_account_type == AccountType.EXT_PERM_USER:
        raise HTTPException(
            status_code=400,
            detail="To change an External Permissioned User's role, they must first login to Orbyte via the web app.",
        )
    ...
    if requested_role == UserRole.SLACK_USER:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "This role is automatically assigned to users who only use Orbyte via Slack."
            ),
        )
...
def _active_admin_user_stmt() -> Select[tuple[User]]:
    ...
    `orbyte/db/auth.py` so callers that email or surface UI to admins reuse
    the same filter set.
    """
    email_col: KeyedColumnElement[Any] = User.__table__.c.email
    is_active_col: KeyedColumnElement[Any] = User.__table__.c.is_active
