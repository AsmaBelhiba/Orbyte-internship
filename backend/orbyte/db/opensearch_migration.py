...
from orbyte.configs.app_configs import ENABLE_OPENSEARCH_RETRIEVAL_FOR_ORBYTE
...
from orbyte.db.models import OpenSearchDocumentMigrationRecord
from orbyte.db.models import OpenSearchTenantMigrationRecord
...
def get_opensearch_retrieval_state(
    ...
    If ORBYTE_DISABLE_VESPA is True, always returns True.
    """
    if ORBYTE_DISABLE_VESPA:
        return True
    record = db_session.query(OpenSearchTenantMigrationRecord).first()
    if record is None:
        return ENABLE_OPENSEARCH_RETRIEVAL_FOR_ORBYTE
