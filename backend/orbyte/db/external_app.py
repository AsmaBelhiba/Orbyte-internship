...
from orbyte.db.utils import is_set
from orbyte.db.utils import UNSET
from orbyte.db.utils import UnsetType
...
def validate_auth_template(
    ...
    ``OrbyteError(INVALID_INPUT)`` on violation.
    """
...
def get_connectable_apps_for_user(
    ...
    from orbyte.db.skill import all_skills_for_user_incl_external_apps
...
def create_external_app(
    ...
    """
    from orbyte.db.skill import create_built_in_skill_row__no_commit
...
def update_external_app(
    ...
    Raises ``OrbyteError(NOT_FOUND)`` if absent, or ``INVALID_INPUT`` on app_type
    mismatch.
    """
...
def set_external_app_organization_credentials(
    ...
    commits). Used by the Orbyte-managed provisioning/rotation path — deliberately
...
def delete_external_app(
    ...
    FileStore cleanup. Raises ``OrbyteError(NOT_FOUND)`` if absent.
    """
    app = get_external_app_by_id(db_session, external_app_id)
    if app is None:
        raise OrbyteError(
...
def upsert_external_app_user_credential(
    ...
    ``OrbyteError(NOT_FOUND)`` if the app doesn't exist. ``resolve_masked_values``
