...
from orbyte.db.external_app import is_user_authenticated_for_app
...
from orbyte.db.utils import is_fk_violation
from orbyte.db.utils import is_unique_violation
...
def create_built_in_skill_row__no_commit(
    *,
    ...
    ``OrbyteError(DUPLICATE_RESOURCE)``, which is the desired "connect Slack once"
    behaviour.
    """
    skill = Skill(
...
def _flush_shares(db_session: Session, fk_violation_detail: str) -> None:
    try:
        ...
        if is_fk_violation(e):
            raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, fk_violation_detail) from e
        raise
