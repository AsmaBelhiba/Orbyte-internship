...
from orbyte.configs.constants import TokenRateLimitScope
...
from orbyte.db.enums import DefaultAppMode
...
from orbyte.db.enums import HookFailStrategy
...
from orbyte.db.enums import MCPAuthenticationPerformer
from orbyte.db.enums import MCPAuthenticationType
from orbyte.db.enums import MCPOAuthProviderMode
from orbyte.db.enums import MCPServerStatus
from orbyte.db.enums import MCPTransport
from orbyte.db.enums import OpenSearchDocumentMigrationStatus
from orbyte.db.enums import OpenSearchTenantMigrationStatus
...
from orbyte.db.enums import SSOProviderType
...
from orbyte.db.enums import ThemePreference
from orbyte.db.enums import UserFileStatus
from orbyte.db.index_attempt_metrics_models import IndexAttemptStage
from orbyte.db.pydantic_type import PydanticListType
from orbyte.db.pydantic_type import PydanticType
from orbyte.external_apps.url_glob import UrlGlob
from orbyte.file_store.models import FileDescriptor
from orbyte.kg.models import KGEntityTypeAttributes
from orbyte.kg.models import KGStage
from orbyte.llm.override_models import LLMOverride
from orbyte.llm.override_models import PromptOverride
from orbyte.server.security.models import SSRFProtectionLevel
from orbyte.tools.tool_implementations.web_search.models import WebContentProviderConfig
from orbyte.utils.encryption import decrypt_bytes_to_string
from orbyte.utils.encryption import encrypt_string_to_bytes
from orbyte.utils.headers import HeaderItemDict
from orbyte.utils.logger import setup_logger
from orbyte.utils.sensitive import SensitiveValue
from orbyte.utils.special_types import JSON_ro
...
class OpenSearchDocumentMigrationRecord(Base):
    ...
    This table can be dropped when the migration is complete for all Orbyte
    instances.
    """
...
class ChunkStats(Base):
    ...
    # this should correspond to the ID of the document
    # (as is passed around in Orbyte)x
...
class IndexAttemptStageMetric(Base):
    ...
    write-path SQL and `orbyte.db.index_attempt_metrics_models.STAGE_SCOPE`
...
class ChatSession(Base):
    ...
    # This chat created by OrbyteBot
    orbytebot_flow: Mapped[bool] = mapped_column(Boolean, default=False)
...
class Skill(Base):
    ...
    # comes from BUILT_IN_SKILLS in `orbyte.skills.built_in`. When NULL, the
...
These models are only used in Enterprise Edition only features in Orbyte.
They are kept here to simplify the codebase and avoid having different assumptions
on the shape of data being passed around between the MIT and EE versions of Orbyte.

In the MIT version of Orbyte, assume these tables are always empty.
"""
...
class User__ExternalUserGroupId(Base):
    ...
    directly adding the Orbyte user to the doc ACL list"""
...
class ScheduledTask(Base):
    ...
    def pre_approved_app_ids(self) -> list[int]:
        ...
        ``orbyte.db.scheduled_task.set_pre_approved_apps``."""
...
class ScimUserMapping(Base):
    """Maps SCIM externalId from the IdP to an Orbyte User."""
...
class ScimGroupMapping(Base):
    """Maps SCIM externalId from the IdP to an Orbyte UserGroup."""
...
class ExternalApp(Base):
    ...
    # multiple distinct instances within one Orbyte (each with its own
