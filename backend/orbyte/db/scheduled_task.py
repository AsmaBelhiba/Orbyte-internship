...
Mirrors the style of ``backend/orbyte/db/persona.py`` — every function takes
...
from orbyte.db.enums import ScheduledTaskErrorClass
from orbyte.db.enums import ScheduledTaskRunStatus
from orbyte.db.enums import ScheduledTaskSkipReason
...
from orbyte.server.features.build.scheduled_tasks.schedule import compute_next_run_at
...
def create_scheduled_task(
    *,
    ...
    Raises:
        OrbyteError(INVALID_INPUT): if the cron is invalid.
    """
...
def get_scheduled_task(
    *,
    ...
    if task is None:
        raise OrbyteError(OrbyteErrorCode.NOT_FOUND, "Scheduled task not found")
    return task
...
def update_scheduled_task(
    *,
    ...
    Raises:
        OrbyteError(NOT_FOUND): the task does not exist or is not owned by
            the caller.
        OrbyteError(INVALID_INPUT): the new cron is invalid.
    """
...
def mark_run_status(
    *,
    ...
    if run is None:
        raise OrbyteError(OrbyteErrorCode.NOT_FOUND, "Scheduled task run not found")
    run.status = status
...
def get_run(
    *,
    ...
    Raises ``OrbyteError(NOT_FOUND)`` if the run does not exist.
    """
    run = db_session.execute(
        select(ScheduledTaskRun).where(ScheduledTaskRun.id == run_id)
    ).scalar_one_or_none()
