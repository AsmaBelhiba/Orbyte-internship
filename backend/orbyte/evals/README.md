...
This directory contains the evaluation framework for testing and measuring the performance of Orbyte's chat and retrieval systems.

## Overview

The evaluation system uses [Braintrust](https://www.braintrust.dev/) to run automated evaluations against test datasets. It measures the quality of responses generated by Orbyte's chat system and can be used to track performance improvements over time.
...
```bash
orbyte/backend$ python -m dotenv -f .vscode/.env run -- python orbyte/evals/eval_cli.py --remote --api-key <SUPER_CLOUD_USER_API_KEY> --search-permissions-email <email account to reference> --remote --remote-dataset-name Simple
```

You can also run the CLI directly from the command line:

```bash
orbyte$ python -m dotenv -f .vscode/.env run -- python backend/orbyte/evals/eval_cli.py --local-dataset-path backend/orbyte/evals/data/eval.json --search-permissions-email richard@onyx.app
```
Save the env var ORBYTE_EVAL_API_KEY in your .env file so you don't have to specify it every time for triggering remote runs.
