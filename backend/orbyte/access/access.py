...
from orbyte.db.document import get_access_info_for_document
from orbyte.db.document import get_access_info_for_documents
...
def get_access_for_document(
    ...
    versioned_get_access_for_document_fn = fetch_versioned_implementation(
        "orbyte.access.access", "_get_access_for_document"
    )
...
def get_access_for_documents(
    ...
    versioned_get_access_for_documents_fn = fetch_versioned_implementation(
        "orbyte.access.access", "_get_access_for_documents"
    )
...
def get_acl_for_user(user: User, db_session: Session | None = None) -> set[str]:
    versioned_acl_for_user_fn = fetch_versioned_implementation(
        "orbyte.access.access", "_get_acl_for_user"
    )
...
def get_access_for_user_files(
    ...
    versioned_fn = fetch_versioned_implementation(
        "orbyte.access.access", "get_access_for_user_files_impl"
    )
...
def build_access_for_user_files(
    ...
    versioned_fn = fetch_versioned_implementation(
        "orbyte.access.access", "build_access_for_user_files_impl"
    )
...
def _documents_from_file_connector_config(
    ...
    `orbyte/connectors/file/connector.py`). The `@>` lookup also can't use
