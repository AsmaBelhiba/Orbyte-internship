...
from orbyte.configs.app_configs import OPEN_URL_PLAYWRIGHT_FALLBACK_ENABLED
from orbyte.file_processing.html_utils import ParsedHTML
from orbyte.file_processing.html_utils import web_html_cleanup
from orbyte.server.security.models import outbound_allow_private_network
from orbyte.server.security.store import get_security_settings
from orbyte.tools.tool_implementations.open_url.models import WebContent
...
from orbyte.utils.playwright_fetch import fetch_rendered_html
from orbyte.utils.playwright_fetch import looks_like_cloudflare_challenge
from orbyte.utils.playwright_fetch import RenderedPage
from orbyte.utils.url import ssrf_safe_get
from orbyte.utils.url import SSRFException
from orbyte.utils.web_content import decode_html_bytes
from orbyte.utils.web_content import extract_pdf_text
from orbyte.utils.web_content import is_pdf_resource
from orbyte.utils.web_content import title_from_pdf_metadata
from orbyte.utils.web_content import title_from_url

logger = setup_logger()

DEFAULT_READ_TIMEOUT_SECONDS = 15
DEFAULT_CONNECT_TIMEOUT_SECONDS = 5
DEFAULT_USER_AGENT = "OrbyteWebCrawler/1.0 (+https://www.onyx.app)"
...
def _parse_html_to_web_content(url: str, html: str) -> WebContent:
    ...
    try:
        ...
        logger.warning(
            "Orbyte crawler failed to parse %s (%s)", url, exc.__class__.__name__
        )
...
class OrbyteWebCrawler(WebContentProvider):
    """
    ...
    def _fetch_url_safe(self, url: str) -> WebContent:
        ...
        try:
            ...
            logger.warning(
                "Orbyte crawler unexpected error for %s (%s)",
                url,
    ...
    def _fetch_url(self, url: str) -> WebContent:
        try:
            ...
            logger.warning(
                "Orbyte crawler failed to fetch %s (%s)",
                url,
                exc.__class__.__name__,
            )
            return _failed_result(url, FailureReason.NETWORK_ERROR)
        ...
        if response.status_code >= 400:
            ...
            if try_fallback:
                logger.info(
                    "Orbyte crawler got %s for %s; retrying via Playwright "
                    "(cf_signals=%s)",
        ...
        try:
            ...
            logger.warning(
                "Orbyte crawler failed to decode %s (%s)", url, exc.__class__.__name__
            )
