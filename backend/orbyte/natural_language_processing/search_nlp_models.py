...
from orbyte.configs.app_configs import INDEXING_EMBEDDING_MODEL_NUM_THREADS
from orbyte.configs.app_configs import LARGE_CHUNK_RATIO
from orbyte.configs.model_configs import BATCH_SIZE_ENCODE_CHUNKS
from orbyte.configs.model_configs import (
...
from orbyte.natural_language_processing.constants import DEFAULT_COHERE_MODEL
from orbyte.natural_language_processing.constants import DEFAULT_OPENAI_MODEL
from orbyte.natural_language_processing.constants import DEFAULT_VERTEX_MODEL
from orbyte.natural_language_processing.constants import DEFAULT_VOYAGE_MODEL
from orbyte.natural_language_processing.constants import EmbeddingModelTextType
from orbyte.natural_language_processing.exceptions import CohereBillingLimitError
from orbyte.natural_language_processing.exceptions import ModelServerRateLimitError
from orbyte.natural_language_processing.utils import get_tokenizer
from orbyte.natural_language_processing.utils import tokenizer_trim_content
from orbyte.server.metrics.embedding import observe_embedding_client
from orbyte.server.metrics.embedding import track_embedding_in_progress
from orbyte.tracing.flows import LLMFlow
from orbyte.tracing.llm_utils import traced_llm_call
from orbyte.utils.logger import setup_logger
from orbyte.utils.search_nlp_models_utils import pass_aws_key
...
class CloudEmbedding:
    ...
    async def _embed_cohere(
        ...
        for text_batch in batch_list(texts, _COHERE_MAX_INPUT_LEN):
            # Does not use the same tokenizer as the Orbyte API server but it's approximately the same
...
class EmbeddingModel:
    ...
    def _make_model_server_request(
        ...
        def _make_request() -> Response:
            headers = {}
            if tenant_id:
                headers["X-Orbyte-Tenant-ID"] = tenant_id

            if request_id:
                headers["X-Orbyte-Request-ID"] = request_id
