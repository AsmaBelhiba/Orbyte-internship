...
from orbyte.db.document_set import construct_document_id_select_by_docset
from orbyte.redis.redis_object_helper import RedisObjectHelper
...
class RedisDocumentSet(RedisObjectHelper):
    ...
    def set_fence(self, payload: int | None) -> None:
        if payload is None:
            self.redis.srem(OrbyteRedisConstants.ACTIVE_FENCES, self.fence_key)
            self.redis.delete(self.fence_key)
            return

        self.redis.set(self.fence_key, payload, ex=self.FENCE_TTL)
        self.redis.sadd(OrbyteRedisConstants.ACTIVE_FENCES, self.fence_key)
