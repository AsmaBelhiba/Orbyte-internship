...
_active_set_size = Gauge(
    "orbyte_tenant_work_gating_active_set_size",
...
_full_fanout_total = Counter(
    "orbyte_tenant_work_gating_full_fanout_total",
...
def _client() -> TenantRedisClient:
    return get_redis_client(tenant_id=ORBYTE_CLOUD_TENANT_ID)
...
def maybe_mark_tenant_active(tenant_id: str, caller: str = "unknown") -> None:
    ...
    """
    try:
        # Local import to avoid a module-load cycle: OrbyteRuntime imports
        # orbyte.redis.redis_pool, so a top-level import here would wedge on
        # certain startup paths.
        from orbyte.server.runtime.orbyte_runtime import OrbyteRuntime

        if not OrbyteRuntime.get_tenant_work_gating_enabled():
            return
