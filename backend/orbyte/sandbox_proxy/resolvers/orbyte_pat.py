"""Orbyte-API PAT credential resolver.

Claims requests bound for the Orbyte API host (the host of
...
class OrbytePatResolver(CredentialResolver):
    """Injects the sandbox's Orbyte API PAT on requests to the configured API host."""
    ...
    def resolve(self, request: http.Request, ctx: InjectionContext) -> dict[str, str]:
        ...
        logger.debug(
            "orbyte_pat_resolver.resolved sandbox=%s host=%s",
