...
from orbyte.configs.app_configs import CELERY_WORKER_PRIMARY_POOL_OVERFLOW
from orbyte.configs.constants import CELERY_PRIMARY_WORKER_LOCK_TIMEOUT
from orbyte.configs.constants import OrbyteRedisConstants
from orbyte.configs.constants import OrbyteRedisLocks
from orbyte.configs.constants import POSTGRES_CELERY_WORKER_PRIMARY_APP_NAME
...
from orbyte.redis.redis_connector_stop import RedisConnectorStop
...
from orbyte.server.metrics.celery_task_metrics import on_celery_task_postrun
from orbyte.server.metrics.celery_task_metrics import on_celery_task_prerun
from orbyte.server.metrics.celery_task_metrics import on_celery_task_rejected
from orbyte.server.metrics.celery_task_metrics import on_celery_task_retry
from orbyte.server.metrics.celery_task_metrics import on_celery_task_revoked
...
celery_app.config_from_object("orbyte.background.celery.configs.primary")
...
def on_worker_init(sender: Worker, **kwargs: Any) -> None:
    ...
    r.delete(OrbyteRedisLocks.PRIMARY_WORKER)
    ...
    lock: RedisLock = r.lock(
        OrbyteRedisLocks.PRIMARY_WORKER,
    ...
    r.delete(OrbyteRedisLocks.CHECK_VESPA_SYNC_BEAT_LOCK)

    r.delete(OrbyteRedisConstants.ACTIVE_FENCES)
...
celery_app.autodiscover_tasks(
    app_base.filter_task_modules(
        [
            ...
            "orbyte.background.celery.tasks.port",
            "orbyte.background.celery.tasks.evals",
            "orbyte.background.celery.tasks.hierarchyfetching",
            "orbyte.background.celery.tasks.pruning",
            "orbyte.background.celery.tasks.scheduled_tasks",
            "orbyte.background.celery.tasks.shared",
            "orbyte.background.celery.tasks.vespa",
            "orbyte.background.celery.tasks.llm_model_update",
            "orbyte.background.celery.tasks.user_file_processing",
        ]
    )
)
