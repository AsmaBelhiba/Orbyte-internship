...
from orbyte.background.celery.apps.task_formatters import CeleryTaskColoredFormatter
from orbyte.background.celery.apps.task_formatters import CeleryTaskJsonFormatter
from orbyte.background.celery.apps.task_formatters import CeleryTaskPlainFormatter
from orbyte.background.celery.celery_utils import celery_is_worker_primary
from orbyte.background.celery.celery_utils import make_probe_path
...
from orbyte.db.engine.sql_engine import get_sqlalchemy_engine
from orbyte.document_index.opensearch.client import wait_for_opensearch_with_timeout
from orbyte.document_index.vespa.shared_utils.utils import wait_for_vespa_with_timeout
from orbyte.httpx.httpx_pool import HttpxPool
from orbyte.redis.redis_connector import RedisConnector
from orbyte.redis.redis_connector_delete import RedisConnectorDelete
...
from orbyte.utils.logger import ColoredFormatter
from orbyte.utils.logger import get_json_formatter
from orbyte.utils.logger import get_log_level_from_str
from orbyte.utils.logger import LoggerContextVars
from orbyte.utils.logger import PlainFormatter
...
def on_task_postrun(
    ...
    **kwds: Any,  # noqa: ARG001
    ...
    if task.name.startswith(ORBYTE_CLOUD_CELERY_TASK_PREFIX):
...
def on_secondary_worker_init(sender: Any, **kwargs: Any) -> None:  # noqa: ARG001
    ...
    while True:
        if r.exists(OrbyteRedisLocks.PRIMARY_WORKER):
            break
...
class TenantContextFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if tenant_id:
            # Match the 8 character tenant abbreviation used in OrbyteLoggingAdapter
...
def wait_for_document_index_or_shutdown() -> None:
    ...
    if ENABLE_OPENSEARCH_INDEXING_FOR_ORBYTE:
...
_VECTOR_DB_TASK_MODULES: set[str] = {
    "orbyte.background.celery.tasks.connector_deletion",
    "orbyte.background.celery.tasks.docprocessing",
    "orbyte.background.celery.tasks.docfetching",
    "orbyte.background.celery.tasks.pruning",
    "orbyte.background.celery.tasks.vespa",
    "orbyte.background.celery.tasks.opensearch_migration",
    "orbyte.background.celery.tasks.doc_permission_syncing",
    "orbyte.background.celery.tasks.hierarchyfetching",
...
# NOTE: "orbyte.background.celery.tasks.shared" is intentionally NOT in the set
