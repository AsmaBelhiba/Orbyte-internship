...
from orbyte.configs.constants import POSTGRES_CELERY_BEAT_APP_NAME
...
celery_app.config_from_object("orbyte.background.celery.configs.beat")
    ...
    def _generate_schedule(
        ...
        if MULTI_TENANT:
            ...
            get_cloud_tasks_to_schedule = fetch_versioned_implementation(
                "orbyte.background.celery.tasks.beat_schedule",
        ...
        get_tasks_to_schedule = fetch_versioned_implementation(
            "orbyte.background.celery.tasks.beat_schedule", "get_tasks_to_schedule"
        )
