...
from orbyte.background.celery.configs.base import CELERY_SEPARATOR
...
def celery_get_queue_length(queue: str, r: Redis) -> int:
    ...
    total_length = 0
    for i in range(len(OrbyteCeleryPriority)):
        queue_name = queue
        if i > 0:
...
def celery_find_task(task_id: str, queue: str, r: Redis) -> int:
    ...
    """
    for priority in range(len(OrbyteCeleryPriority)):
