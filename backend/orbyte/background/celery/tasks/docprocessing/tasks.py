...
from orbyte.background.celery.celery_redis import celery_get_broker_client
from orbyte.background.celery.celery_redis import celery_get_queued_task_ids
from orbyte.background.celery.celery_redis import celery_get_unacked_task_ids
from orbyte.background.celery.celery_utils import httpx_init_vespa_pool
from orbyte.background.celery.memory_monitoring import emit_process_memory
from orbyte.background.celery.tasks.beat_schedule import CLOUD_BEAT_MULTIPLIER_DEFAULT
from orbyte.background.celery.tasks.docfetching.task_creation_utils import (
...
from orbyte.background.celery.tasks.docprocessing.targeted_reindex_task import (  # noqa: F401  # registers @shared_task with celery
...
from orbyte.background.indexing.checkpointing_utils import (
    get_index_attempts_with_old_checkpoints,
)
from orbyte.background.indexing.index_attempt_utils import cleanup_index_attempts
from orbyte.background.indexing.index_attempt_utils import get_old_index_attempt_ids
from orbyte.configs.app_configs import AUTH_TYPE
from orbyte.configs.app_configs import MANAGED_VESPA
from orbyte.configs.app_configs import PERSISTENT_INDEXING
from orbyte.configs.app_configs import VESPA_CLOUD_CERT_PATH
from orbyte.configs.app_configs import VESPA_CLOUD_KEY_PATH
from orbyte.configs.constants import AuthType
from orbyte.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from orbyte.configs.constants import CELERY_INDEXING_LOCK_TIMEOUT
from orbyte.configs.constants import DocumentSource
from orbyte.configs.constants import MilestoneRecordType
...
from orbyte.configs.constants import OrbyteRedisSignals
...
from orbyte.db.connector_credential_pair import set_cc_pair_repeated_error_state
from orbyte.db.connector_credential_pair import update_connector_credential_pair_from_id
from orbyte.db.engine.sql_engine import get_session_with_current_tenant
from orbyte.db.engine.time_utils import get_db_current_time
from orbyte.db.enums import ConnectorCredentialPairStatus
from orbyte.db.enums import IndexingMode
from orbyte.db.enums import IndexingStatus
from orbyte.db.enums import SwitchoverType
from orbyte.db.index_attempt import create_index_attempt_error
from orbyte.db.index_attempt import get_index_attempt
from orbyte.db.index_attempt import get_index_attempt_errors_for_cc_pair
from orbyte.db.index_attempt import get_stale_not_started_index_attempts
from orbyte.db.index_attempt import IndexAttemptError
from orbyte.db.index_attempt import mark_attempt_canceled
from orbyte.db.index_attempt import mark_attempt_failed
from orbyte.db.index_attempt import mark_attempt_partially_succeeded
from orbyte.db.index_attempt import mark_attempt_succeeded
from orbyte.db.index_attempt_metrics import IndexAttemptStage
from orbyte.db.index_attempt_metrics import safe_record_single_event
from orbyte.db.index_attempt_metrics import time_stage
from orbyte.db.indexing_coordination import CoordinationStatus
...
from orbyte.db.notification import batch_create_notifications
from orbyte.db.notification import delete_notifications_by_additional_data
from orbyte.db.search_settings import get_current_search_settings
from orbyte.db.search_settings import get_secondary_search_settings
from orbyte.db.swap_index import check_and_perform_index_swap
from orbyte.db.users import get_active_admin_users
from orbyte.document_index.factory import get_all_document_indices
from orbyte.error_handling.exceptions import OrbyteError
from orbyte.file_store.document_batch_storage import DocumentBatchStorage
from orbyte.file_store.document_batch_storage import get_document_batch_storage
from orbyte.file_store.staging import cleanup_staged_files_for_attempt
from orbyte.httpx.httpx_pool import HttpxPool
from orbyte.indexing.adapters.document_indexing_adapter import (
...
from orbyte.indexing.persistent_indexing import build_generic_connector_failure
from orbyte.indexing.persistent_indexing import record_generic_failure
from orbyte.natural_language_processing.search_nlp_models import EmbeddingModel
from orbyte.natural_language_processing.search_nlp_models import warm_up_bi_encoder
from orbyte.redis.redis_connector import RedisConnector
from orbyte.redis.redis_docprocessing import RedisDocprocessing
...
from orbyte.redis.redis_pool import SCAN_ITER_COUNT_DEFAULT
...
from orbyte.utils.telemetry import mt_cloud_telemetry
...
def _get_fence_validation_block_expiration() -> int:
    ...
    try:
        beat_multiplier = OrbyteRuntime.get_beat_multiplier()
    except Exception:
    ...
    with get_session_with_current_tenant() as db_session:
        ...
        if stale_not_started:
            ...
            queued_ids = celery_get_queued_task_ids(
                OrbyteCeleryQueues.CONNECTOR_DOC_FETCHING, redis_celery
            )
...
@shared_task(
    name=OrbyteCeleryTask.CHECK_FOR_INDEXING,
    soft_time_limit=300,
    bind=True,
)
def check_for_indexing(self: Task, *, tenant_id: str) -> int | None:
    ...
    the docfetching task (OrbyteCeleryTask.CONNECTOR_DOC_FETCHING_TASK).
    ...
    lock_beat: RedisLock = redis_client.lock(
        OrbyteRedisLocks.CHECK_INDEXING_BEAT_LOCK,
    ...
    try:
        ...
        if not redis_client.exists(OrbyteRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE):
            ...
            for key_bytes in redis_client_replica.scan_iter(
                ...
                if is_fence(key_bytes) and not redis_client.sismember(
                    OrbyteRedisConstants.ACTIVE_FENCES, key_bytes
                ):
                    logger.warning("Adding %s to the lookup table.", key_bytes)
                    redis_client.sadd(OrbyteRedisConstants.ACTIVE_FENCES, key_bytes)

            redis_client.set(
                OrbyteRedisSignals.BLOCK_BUILD_FENCE_LOOKUP_TABLE,
                1,
                ex=OrbyteRuntime.get_build_fence_lookup_table_interval(),
            )
        ...
        lock_beat.reacquire()
        # we want to run this less frequently than the overall task
        if not redis_client.exists(OrbyteRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES):
            ...
            redis_client.set(
                OrbyteRedisSignals.BLOCK_VALIDATE_INDEXING_FENCES,
                1,
...
# primary
@shared_task(
    name=OrbyteCeleryTask.CHECK_FOR_CHECKPOINT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_checkpoint_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    lock: RedisLock = redis_client.lock(
        OrbyteRedisLocks.CHECK_CHECKPOINT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for attempt in old_attempts:
                ...
                self.app.send_task(
                    OrbyteCeleryTask.CLEANUP_CHECKPOINT,
                    kwargs={
                        "index_attempt_id": attempt.id,
                        "tenant_id": tenant_id,
                    },
                    queue=OrbyteCeleryQueues.CHECKPOINT_CLEANUP,
                    priority=OrbyteCeleryPriority.MEDIUM,
                )
    except Exception:
...
# light worker
@shared_task(
    name=OrbyteCeleryTask.CLEANUP_CHECKPOINT,
    bind=True,
)
...
# primary
@shared_task(
    name=OrbyteCeleryTask.CHECK_FOR_INDEX_ATTEMPT_CLEANUP,
    soft_time_limit=300,
    bind=True,
)
def check_for_index_attempt_cleanup(self: Task, *, tenant_id: str) -> None:
    ...
    redis_client = get_redis_client(tenant_id=tenant_id)
    lock: RedisLock = redis_client.lock(
        OrbyteRedisLocks.CHECK_INDEX_ATTEMPT_CLEANUP_BEAT_LOCK,
        timeout=CELERY_GENERIC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            for i in range(0, len(old_attempt_ids), batch_size):
                ...
                self.app.send_task(
                    OrbyteCeleryTask.CLEANUP_INDEX_ATTEMPT,
                    kwargs={
                        "index_attempt_ids": batch,
                        "tenant_id": tenant_id,
                    },
                    queue=OrbyteCeleryQueues.INDEX_ATTEMPT_CLEANUP,
                    priority=OrbyteCeleryPriority.MEDIUM,
                )
    except Exception:
...
# light worker
@shared_task(
    name=OrbyteCeleryTask.CLEANUP_INDEX_ATTEMPT,
    bind=True,
)
...
@shared_task(
    name=OrbyteCeleryTask.DOCPROCESSING_TASK,
    bind=True,
)
...
def _check_chunk_usage_limit(tenant_id: str) -> None:
    ...
    from orbyte.db.usage import UsageType
    from orbyte.server.usage_limits import check_usage_and_raise
...
def _docprocessing_task(
    ...
    # check_usage_and_raise raises OrbyteError; hitting a trial/paid usage limit
    ...
    # would ship ORBYTE-BACKEND-H6ED to Sentry on every queued batch of an
    # over-limit tenant).
    if USAGE_LIMITS_ENABLED:
        try:
            _check_chunk_usage_limit(tenant_id)
        except OrbyteError as e:
            task_logger.warning(
    ...
    try:
        ...
        with get_session_with_current_tenant() as db_session:
            ...
            index_attempt_metadata = IndexAttemptMetadata(
                ...
                request_id=make_randomized_orbyte_request_id("DIP"),
        ...
        if USAGE_LIMITS_ENABLED and index_pipeline_result.total_chunks > 0:
            try:
                from orbyte.db.usage import increment_usage
                from orbyte.db.usage import UsageType
