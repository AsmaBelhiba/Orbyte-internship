...
from orbyte.db.connector_credential_pair import add_deletion_failure_message
...
from orbyte.db.document_set import delete_document_set_cc_pair_relationship__no_commit
...
from orbyte.db.index_attempt import delete_index_attempts
...
from orbyte.redis.redis_connector_delete import RedisConnectorDeletePayload
...
@shared_task(
    name=OrbyteCeleryTask.CHECK_FOR_CONNECTOR_DELETION,
    ignore_result=True,
...
def check_for_connector_deletion_task(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        OrbyteRedisLocks.CHECK_CONNECTOR_DELETION_BEAT_LOCK,
    ...
    try:
        ...
        lock_beat.reacquire()
        if not r.exists(OrbyteRedisSignals.BLOCK_VALIDATE_CONNECTOR_DELETION_FENCES):
