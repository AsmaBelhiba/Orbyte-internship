...
from orbyte.background.celery.celery_redis import celery_get_queue_length
...
from orbyte.db.engine.tenant_utils import validate_tenant_id
from orbyte.db.engine.time_utils import get_db_current_time
from orbyte.db.enums import IndexingStatus
from orbyte.db.enums import SyncStatus
from orbyte.db.enums import SyncType
...
from orbyte.db.models import SyncRecord
from orbyte.db.models import UserGroup
from orbyte.db.search_settings import get_active_search_settings_list
...
from orbyte.utils.platform_utils import is_running_in_container
from orbyte.utils.platform_utils import is_running_in_kubernetes
...
def _collect_queue_metrics(redis_celery: Redis) -> list[Metric]:
    ...
    metrics = []
    queue_mappings = {
        "celery_queue_length": OrbyteCeleryQueues.PRIMARY,
        "docprocessing_queue_length": OrbyteCeleryQueues.DOCPROCESSING,
        "port_queue_length": OrbyteCeleryQueues.PORT,
        "docfetching_queue_length": OrbyteCeleryQueues.CONNECTOR_DOC_FETCHING,
        "sync_queue_length": OrbyteCeleryQueues.VESPA_METADATA_SYNC,
        "deletion_queue_length": OrbyteCeleryQueues.CONNECTOR_DELETION,
        "pruning_queue_length": OrbyteCeleryQueues.CONNECTOR_PRUNING,
        "permissions_sync_queue_length": OrbyteCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        "external_group_sync_queue_length": OrbyteCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC,
        "permissions_upsert_queue_length": OrbyteCeleryQueues.DOC_PERMISSIONS_UPSERT,
        "hierarchy_fetching_queue_length": OrbyteCeleryQueues.CONNECTOR_HIERARCHY_FETCHING,
        "llm_model_update_queue_length": OrbyteCeleryQueues.LLM_MODEL_UPDATE,
        "checkpoint_cleanup_queue_length": OrbyteCeleryQueues.CHECKPOINT_CLEANUP,
        "index_attempt_cleanup_queue_length": OrbyteCeleryQueues.INDEX_ATTEMPT_CLEANUP,
        "chat_ttl_deletion_queue_length": OrbyteCeleryQueues.CHAT_TTL_DELETION,
        "csv_generation_queue_length": OrbyteCeleryQueues.CSV_GENERATION,
        "user_file_processing_queue_length": OrbyteCeleryQueues.USER_FILE_PROCESSING,
        "user_file_project_sync_queue_length": OrbyteCeleryQueues.USER_FILE_PROJECT_SYNC,
        "user_file_delete_queue_length": OrbyteCeleryQueues.USER_FILE_DELETE,
        "monitoring_queue_length": OrbyteCeleryQueues.MONITORING,
        "sandbox_queue_length": OrbyteCeleryQueues.SANDBOX,
        "opensearch_migration_queue_length": OrbyteCeleryQueues.OPENSEARCH_MIGRATION,
    }
...
@shared_task(
    name=OrbyteCeleryTask.MONITOR_BACKGROUND_PROCESSES,
    ignore_result=True,
    soft_time_limit=_MONITORING_SOFT_TIME_LIMIT,
    time_limit=_MONITORING_TIME_LIMIT,
    queue=OrbyteCeleryQueues.MONITORING,
    bind=True,
)
def monitor_background_processes(self: Task, *, tenant_id: str) -> None:
    ...
    lock_monitoring: RedisLock = r.lock(
        OrbyteRedisLocks.MONITOR_BACKGROUND_PROCESSES_LOCK,
...
@shared_task(
    name=OrbyteCeleryTask.CLOUD_MONITOR_ALEMBIC,
)
def cloud_check_alembic() -> bool | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        OrbyteRedisLocks.CLOUD_CHECK_ALEMBIC_BEAT_LOCK,
...
@shared_task(
    name=OrbyteCeleryTask.CLOUD_MONITOR_CELERY_QUEUES, ignore_result=True, bind=True
)
...
@shared_task(name=OrbyteCeleryTask.MONITOR_CELERY_QUEUES, ignore_result=True, bind=True)
...
def monitor_celery_queues_helper(
    ...
    n_celery = celery_get_queue_length(OrbyteCeleryQueues.PRIMARY, r_celery)
    ...
    n_user_file_processing = celery_get_queue_length(
        OrbyteCeleryQueues.USER_FILE_PROCESSING, r_celery
    )
    n_user_file_project_sync = celery_get_queue_length(
        OrbyteCeleryQueues.USER_FILE_PROJECT_SYNC, r_celery
    )
    ...
    n_external_group_sync = celery_get_queue_length(
        OrbyteCeleryQueues.CONNECTOR_EXTERNAL_GROUP_SYNC, r_celery
    )
    n_permissions_upsert = celery_get_queue_length(
        OrbyteCeleryQueues.DOC_PERMISSIONS_UPSERT, r_celery
    )
    n_hierarchy_fetching = celery_get_queue_length(
        OrbyteCeleryQueues.CONNECTOR_HIERARCHY_FETCHING, r_celery
    )
    n_llm_model_update = celery_get_queue_length(
        OrbyteCeleryQueues.LLM_MODEL_UPDATE, r_celery
    )
    n_checkpoint_cleanup = celery_get_queue_length(
        OrbyteCeleryQueues.CHECKPOINT_CLEANUP, r_celery
    )
    n_index_attempt_cleanup = celery_get_queue_length(
        OrbyteCeleryQueues.INDEX_ATTEMPT_CLEANUP, r_celery
    )
    n_csv_generation = celery_get_queue_length(
        OrbyteCeleryQueues.CSV_GENERATION, r_celery
    )
    n_monitoring = celery_get_queue_length(OrbyteCeleryQueues.MONITORING, r_celery)
    n_sandbox = celery_get_queue_length(OrbyteCeleryQueues.SANDBOX, r_celery)
    n_opensearch_migration = celery_get_queue_length(
        OrbyteCeleryQueues.OPENSEARCH_MIGRATION, r_celery
    )
...
@shared_task(
    name=OrbyteCeleryTask.MONITOR_PROCESS_MEMORY,
    ignore_result=True,
    soft_time_limit=_MONITORING_SOFT_TIME_LIMIT,
    time_limit=_MONITORING_TIME_LIMIT,
    queue=OrbyteCeleryQueues.MONITORING,
...
@shared_task(
    name=OrbyteCeleryTask.CLOUD_MONITOR_CELERY_PIDBOX, ignore_result=True, bind=True
)
