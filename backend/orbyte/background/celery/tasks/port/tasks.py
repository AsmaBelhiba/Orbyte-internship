...
from orbyte.db.port_attempt import count_active_port_attempts
...
from orbyte.db.port_attempt import get_stale_in_progress_port_attempts
...
from orbyte.db.port_attempt import touch_port_progress
from orbyte.db.search_settings import get_current_search_settings
from orbyte.db.search_settings import get_search_settings_by_id
from orbyte.db.search_settings import get_secondary_search_settings
from orbyte.document_index.opensearch.port_copy import PortCopier
...
@shared_task(
    name=OrbyteCeleryTask.RUN_PORT_ATTEMPT,
...
def _enqueue_run_port_attempt(
    ...
    celery_app.send_task(
        OrbyteCeleryTask.RUN_PORT_ATTEMPT,
        kwargs={"port_attempt_id": port_attempt_id, "tenant_id": tenant_id},
        queue=OrbyteCeleryQueues.PORT,
...
def run_check_for_port(tenant_id: str, celery_app: Celery) -> int | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        OrbyteRedisLocks.CHECK_PORT_BEAT_LOCK,
...
@shared_task(
    name=OrbyteCeleryTask.CHECK_FOR_PORT,
    soft_time_limit=300,
    bind=True,
)
