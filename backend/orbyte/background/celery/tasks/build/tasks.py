...
from orbyte.server.features.build.db.sandbox import user_has_stale_active_session
...
from orbyte.server.features.build.session.sandbox_lifecycle import is_sandbox_idle
from orbyte.server.features.build.session.sandbox_lifecycle import sleep_sandbox
...
@shared_task(
    name=OrbyteCeleryTask.CLEANUP_IDLE_SANDBOXES,
...
def cleanup_idle_sandboxes_task(self: Task, *, tenant_id: str) -> None:  # noqa: ARG001
    ...
    lock: RedisLock = redis_client.lock(
        OrbyteRedisLocks.CLEANUP_IDLE_SANDBOXES_BEAT_LOCK,
