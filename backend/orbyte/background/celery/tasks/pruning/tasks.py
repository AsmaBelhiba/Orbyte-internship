...
from orbyte.background.celery.celery_redis import celery_find_task
...
from orbyte.background.celery.tasks.docprocessing.utils import IndexingCallbackBase
from orbyte.configs.app_configs import ALLOW_SIMULTANEOUS_PRUNING
from orbyte.configs.app_configs import JOB_TIMEOUT
from orbyte.configs.constants import CELERY_GENERIC_BEAT_LOCK_TIMEOUT
from orbyte.configs.constants import CELERY_PRUNING_LOCK_TIMEOUT
from orbyte.configs.constants import CELERY_TASK_WAIT_FOR_FENCE_TIMEOUT
from orbyte.configs.constants import DANSWER_REDIS_FUNCTION_LOCK_PREFIX
...
from orbyte.db.connector import mark_ccpair_as_pruned
...
from orbyte.db.document import get_documents_for_connector_credential_pair
...
from orbyte.db.hierarchy import delete_orphaned_hierarchy_nodes
from orbyte.db.hierarchy import remove_stale_hierarchy_node_cc_pair_entries
from orbyte.db.hierarchy import reparent_orphaned_hierarchy_nodes
from orbyte.db.hierarchy import update_document_parent_hierarchy_nodes
from orbyte.db.hierarchy import upsert_hierarchy_node_cc_pair_entries
from orbyte.db.hierarchy import upsert_hierarchy_nodes_batch
from orbyte.db.models import ConnectorCredentialPair
from orbyte.db.models import HierarchyNode as DBHierarchyNode
...
from orbyte.redis.redis_connector_prune import RedisConnectorPrune
from orbyte.redis.redis_connector_prune import RedisConnectorPrunePayload
from orbyte.redis.redis_hierarchy import cache_hierarchy_nodes_batch
from orbyte.redis.redis_hierarchy import ensure_source_node_exists
from orbyte.redis.redis_hierarchy import evict_hierarchy_nodes_from_cache
from orbyte.redis.redis_hierarchy import get_node_id_from_raw_id
from orbyte.redis.redis_hierarchy import get_source_node_id_from_cache
from orbyte.redis.redis_hierarchy import HierarchyNodeCacheEntry
...
from orbyte.server.utils import make_short_id
from orbyte.utils.logger import format_error_for_logging
from orbyte.utils.logger import LoggerContextVars
from orbyte.utils.logger import pruning_ctx
...
@shared_task(
    name=OrbyteCeleryTask.CHECK_FOR_PRUNING,
    ignore_result=True,
...
def check_for_pruning(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        OrbyteRedisLocks.CHECK_PRUNE_BEAT_LOCK,
    ...
    try:
        ...
        if not r.exists(OrbyteRedisSignals.BLOCK_PRUNING):
        ...
        lock_beat.reacquire()
        if not r.exists(OrbyteRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES):
            ...
            r.set(
                OrbyteRedisSignals.BLOCK_VALIDATE_PRUNING_FENCES,
                1,
        ...
        lock_beat.reacquire()
        keys = cast(set[Any], r_replica.smembers(OrbyteRedisConstants.ACTIVE_FENCES))
        for key in keys:
            key_bytes = cast(bytes, key)

            if not r.exists(key_bytes):
                r.srem(OrbyteRedisConstants.ACTIVE_FENCES, key_bytes)
                continue
...
def try_creating_prune_generator_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            OrbyteCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
            kwargs=dict(
            ...
            queue=OrbyteCeleryQueues.CONNECTOR_PRUNING,
            task_id=custom_task_id,
            priority=OrbyteCeleryPriority.LOW,
...
@shared_task(
    name=OrbyteCeleryTask.CONNECTOR_PRUNING_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_pruning_generator_task(
    ...
    lock: RedisLock = r.lock(
        OrbyteRedisLocks.PRUNING_LOCK_PREFIX + f"_{redis_connector.cc_pair_id}",
...
def validate_pruning_fences(
    ...
    queue_len = celery_get_queue_length(OrbyteCeleryQueues.CONNECTOR_DELETION, r_celery)
    ...
    reserved_generator_tasks = celery_get_unacked_task_ids(
        OrbyteCeleryQueues.CONNECTOR_PRUNING, r_celery
    )
...
def validate_pruning_fence(
    ...
    found = celery_find_task(
        ...
        OrbyteCeleryQueues.CONNECTOR_PRUNING,
        r_celery,
    )
    if found:
