...
from orbyte.background.celery.memory_monitoring import start_memory_observer
from orbyte.background.celery.memory_monitoring import stop_memory_observer
from orbyte.background.celery.tasks.docfetching.worker_shutdown import (
...
from orbyte.background.celery.tasks.models import SimpleJobResult
from orbyte.background.indexing.job_client import SimpleJob
from orbyte.background.indexing.job_client import SimpleJobClient
from orbyte.background.indexing.job_client import SimpleJobException
from orbyte.background.indexing.run_docfetching import run_docfetching_entrypoint
from orbyte.configs.app_configs import INDEXING_WORKER_MEMORY_LIMIT_MB
from orbyte.configs.constants import CELERY_INDEXING_WATCHDOG_CONNECTOR_TIMEOUT
from orbyte.configs.constants import CELERY_INDEXING_WATCHDOG_SIGTERM_GRACE_SECONDS
...
@shared_task(
    name=OrbyteCeleryTask.CONNECTOR_DOC_FETCHING_TASK,
    bind=True,
    acks_late=False,
    track_started=True,
)
def docfetching_proxy_task(
    ...
    1)  determines parameters of the indexing attempt (which connector indexing function to run,
        ...
        connectors are responsible for reading data from an outside source and converting it to Orbyte documents.
        At the moment these two steps (reading external data and converting to an Orbyte document)
