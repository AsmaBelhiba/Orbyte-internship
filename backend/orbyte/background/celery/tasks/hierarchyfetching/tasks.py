...
from orbyte.connectors.factory import ConnectorMissingException
...
from orbyte.connectors.interfaces import HierarchyConnector
from orbyte.connectors.models import HierarchyNode as PydanticHierarchyNode
from orbyte.db.connector import mark_cc_pair_as_hierarchy_fetched
...
def _try_creating_hierarchy_fetching_task(
    ...
    try:
        ...
        result = celery_app.send_task(
            OrbyteCeleryTask.CONNECTOR_HIERARCHY_FETCHING_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair.id,
                tenant_id=tenant_id,
            ),
            queue=OrbyteCeleryQueues.CONNECTOR_HIERARCHY_FETCHING,
...
@shared_task(
    name=OrbyteCeleryTask.CHECK_FOR_HIERARCHY_FETCHING,
    soft_time_limit=300,
    bind=True,
)
def check_for_hierarchy_fetching(self: Task, *, tenant_id: str) -> int | None:
    ...
    lock_beat: RedisLock = redis_client.lock(
        OrbyteRedisLocks.CHECK_HIERARCHY_FETCHING_BEAT_LOCK,
...
@shared_task(
    name=OrbyteCeleryTask.CONNECTOR_HIERARCHY_FETCHING_TASK,
