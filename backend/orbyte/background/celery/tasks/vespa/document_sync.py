...
from orbyte.configs.app_configs import DB_YIELD_PER_DEFAULT
from orbyte.configs.constants import CELERY_DOCUMENT_SYNC_TASK_EXPIRES
...
def set_document_sync_fence(r: TenantRedisClient, payload: int | None) -> None:
    ...
    if payload is None:
        r.srem(OrbyteRedisConstants.ACTIVE_FENCES, DOCUMENT_SYNC_FENCE_KEY)
        r.delete(DOCUMENT_SYNC_FENCE_KEY)
        return

    r.set(DOCUMENT_SYNC_FENCE_KEY, payload, ex=FENCE_TTL)
    r.sadd(OrbyteRedisConstants.ACTIVE_FENCES, DOCUMENT_SYNC_FENCE_KEY)
...
def generate_document_sync_tasks(
    ...
    for doc_id, is_secondary_pending in db_session.execute(stmt).yield_per(
        ...
        if is_secondary_pending:
            priority = (
                OrbyteCeleryPriority.LOW if port_running else OrbyteCeleryPriority.HIGH
            )
        ...
        celery_app.send_task(
            OrbyteCeleryTask.DOCUMENT_INDEX_METADATA_SYNC_TASK,
            kwargs=dict(document_id=doc_id, tenant_id=tenant_id),
            queue=OrbyteCeleryQueues.VESPA_METADATA_SYNC,
