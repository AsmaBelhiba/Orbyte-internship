...
from orbyte.configs.constants import ORBYTE_CELERY_BEAT_HEARTBEAT_KEY
from orbyte.configs.constants import OrbyteCeleryTask
from orbyte.db.document import delete_document_by_connector_credential_pair__no_commit
from orbyte.db.document import delete_documents_complete
from orbyte.db.document import fetch_chunk_count_for_document
from orbyte.db.document import get_document
from orbyte.db.document import get_document_connector_count
...
from orbyte.db.relationships import delete_document_references_from_kg
...
class OrbyteCeleryTaskCompletionStatus(str, Enum):
...
@shared_task(
    name=OrbyteCeleryTask.DOCUMENT_BY_CC_PAIR_CLEANUP_TASK,
...
def document_by_cc_pair_cleanup_task(
    ...
    if completion_status != OrbyteCeleryTaskCompletionStatus.SUCCEEDED:
        return False
...
@shared_task(name=OrbyteCeleryTask.CELERY_BEAT_HEARTBEAT, ignore_result=True, bind=True)
def celery_beat_heartbeat(self: Task, *, tenant_id: str) -> None:  # noqa: ARG001
    ...
    r = get_redis_client()
    r.set(ORBYTE_CELERY_BEAT_HEARTBEAT_KEY, 1, ex=600)
