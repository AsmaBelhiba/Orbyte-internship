...
from orbyte.file_processing.enums import HtmlBasedConnectorTransformLinksStrategy
from orbyte.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_SYSTEM_PROMPT
from orbyte.prompts.image_analysis import DEFAULT_IMAGE_SUMMARIZATION_USER_PROMPT
...
# This override allows self-hosting the MCP server with Orbyte Cloud backend.
...
# history APIs + recording stay on (unlike ORBYTE_QUERY_HISTORY_TYPE=disabled).
...
# NOTE: if you are having problems accessing the Orbyte web UI locally (especially
...
# restrict access to Orbyte to only users with emails from those domains.
# E.g. `VALID_EMAIL_DOMAINS=example.com,example.org` will restrict Orbyte
...
# If set, Orbyte will listen to the `expires_at` returned by the identity
...
# of OpenSearch running for the relevant Orbyte instance.
# NOTE: Now enabled on by default, unless the env indicates otherwise.
ENABLE_OPENSEARCH_INDEXING_FOR_ORBYTE = (
    os.environ.get("ENABLE_OPENSEARCH_INDEXING_FOR_ORBYTE", "true").lower() == "true"
)
...
ENABLE_OPENSEARCH_RETRIEVAL_FOR_ORBYTE = (
    ENABLE_OPENSEARCH_INDEXING_FOR_ORBYTE
    and os.environ.get("ENABLE_OPENSEARCH_RETRIEVAL_FOR_ORBYTE", "").lower() == "true"
)
DISABLE_OPENSEARCH_MIGRATION_TASK = (
    os.environ.get("DISABLE_OPENSEARCH_MIGRATION_TASK", "").lower() == "true"
)
ORBYTE_DISABLE_VESPA = os.environ.get("ORBYTE_DISABLE_VESPA", "true").lower() == "true"
...
ORBYTE_SEARCH_UI_USES_OPENSEARCH_KEYWORD_SEARCH = (
    os.environ.get("ORBYTE_SEARCH_UI_USES_OPENSEARCH_KEYWORD_SEARCH", "").lower()
    == "true"
)
...
# When the OrbyteWebCrawler (open_url tool) hits a 403 / Cloudflare challenge,
...
CONFLUENCE_USE_ORBYTE_USERS_FOR_GROUP_SYNC = (
    os.environ.get("CONFLUENCE_USE_ORBYTE_USERS_FOR_GROUP_SYNC", "").lower() == "true"
)
...
# multi-tenant deployments. See orbyte/indexing/document_push.py.
...
# backend/orbyte/document_index/vespa/app_config/schemas/danswer_chunk.sd.jinja.
...
# Logs Orbyte only model interactions like prompts, responses, messages etc.
LOG_ORBYTE_MODEL_INTERACTIONS = (
    os.environ.get("LOG_ORBYTE_MODEL_INTERACTIONS", "").lower() == "true"
)
...
BRAINTRUST_PROJECT = os.environ.get("BRAINTRUST_PROJECT", "Orbyte")
...
API_SERVER_THREADPOOL_SIZE = int(os.environ.get("ORBYTE_API_THREADPOOL_SIZE") or "0")
...
# founders@onyx.app OR message Chris Weaver or Yuhong Sun in the Orbyte
...
S3_FILE_STORE_BUCKET_NAME = (
    os.environ.get("S3_FILE_STORE_BUCKET_NAME") or "orbyte-file-store-bucket"
)
S3_FILE_STORE_PREFIX = os.environ.get("S3_FILE_STORE_PREFIX") or "orbyte-files"
...
GCS_FILE_STORE_PREFIX = os.environ.get("GCS_FILE_STORE_PREFIX") or "orbyte-files"
...
# These are Orbyte-managed API keys provided to tenants by default
#####
...
# Auto-create Orbyte-managed built-in external app rows (disabled, Orbyte-owned creds)
...
# Orbyte-owned OAuth credentials for built-in external apps (managed cloud)
...
STRIPE_PUBLISHABLE_KEY_URL = (
    "https://orbyte-stripe-public.s3.amazonaws.com/publishable-key.txt"
)
