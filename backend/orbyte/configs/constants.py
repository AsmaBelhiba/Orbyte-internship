...
ORBYTE_DEFAULT_APPLICATION_NAME = "Orbyte"
ORBYTE_DISCORD_URL = "https://discord.gg/4NA5SbzrWb"
ORBYTE_UTM_SOURCE = "orbyte_app"
SLACK_USER_TOKEN_PREFIX = "xoxp-"
SLACK_BOT_TOKEN_PREFIX = "xoxb-"
ORBYTE_EMAILABLE_LOGO_MAX_DIM = 512
...
TENANT_ID_COOKIE_NAME = "orbyte_tid"  # tenant id - for workaround cases
ANONYMOUS_USER_COOKIE_NAME = "orbyte_anonymous_user"
...
ORBYTE_METADATA_FILENAME = ".orbyte_metadata.json"

# Messages
DISABLED_GEN_AI_MSG = (
    "Your System Admin has disabled the Generative AI functionalities of Orbyte.\n"
    "Please contact them if you wish to have this enabled.\n"
    "You can still use Orbyte as a search engine."
)
...
DANSWER_API_KEY_DUMMY_EMAIL_DOMAIN = "orbyteapikey.ai"
...
KV_SETTINGS_KEY = "orbyte_settings"
KV_CUSTOMER_UUID_KEY = "customer_uuid"
KV_INSTANCE_DOMAIN_KEY = "instance_domain"
KV_ENTERPRISE_SETTINGS_KEY = "orbyte_enterprise_settings"
...
class DocumentSource(str, Enum):
    # Special case, document passed in via Orbyte APIs without specifying a source type
...
class MessageType(str, Enum):
    ...
    USER_REMINDER = "user_reminder"  # Custom Orbyte message type which is translated into a USER message when passed to the LLM
...
class MilestoneRecordType(str, Enum):
    ...
    CREATED_ORBYTE_BOT = "created_orbyte_bot"
    REQUESTED_CONNECTOR = "requested_connector"


class OrbyteCeleryQueues:
...
class OrbyteRedisConstants:
    ACTIVE_FENCES = "active_fences"


class OrbyteCeleryPriority(int, Enum):
    HIGHEST = 0
    HIGH = auto()
    MEDIUM = auto()
    LOW = auto()
...
ORBYTE_CLOUD_CELERY_TASK_PREFIX = "cloud"

# the tenant id we use for system level redis operations
ORBYTE_CLOUD_TENANT_ID = "cloud"

# the redis namespace for runtime variables
ORBYTE_CLOUD_REDIS_RUNTIME = "runtime"
CLOUD_BUILD_FENCE_LOOKUP_TABLE_INTERVAL_DEFAULT = 600


class OrbyteCeleryTask:
    DEFAULT = "celery"

    CLOUD_BEAT_TASK_GENERATOR = f"{ORBYTE_CLOUD_CELERY_TASK_PREFIX}_generate_beat_tasks"
    CLOUD_MONITOR_ALEMBIC = f"{ORBYTE_CLOUD_CELERY_TASK_PREFIX}_monitor_alembic"
    CLOUD_MONITOR_CELERY_QUEUES = (
        f"{ORBYTE_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_queues"
    )
    CLOUD_CHECK_AVAILABLE_TENANTS = (
        f"{ORBYTE_CLOUD_CELERY_TASK_PREFIX}_check_available_tenants"
    )
    CLOUD_MONITOR_CELERY_PIDBOX = (
        f"{ORBYTE_CLOUD_CELERY_TASK_PREFIX}_monitor_celery_pidbox"
    )
...
ORBYTE_CELERY_BEAT_HEARTBEAT_KEY = "orbyte:celery:beat:heartbeat"
...
class OrbyteCallTypes(str, Enum):
    FIREFLIES = "FIREFLIES"
    GONG = "GONG"
