...
from orbyte.configs.app_configs import EGNYTE_CLIENT_ID
from orbyte.configs.app_configs import EGNYTE_CLIENT_SECRET
...
from orbyte.connectors.interfaces import OAuthConnector
...
from orbyte.file_processing.extract_file_text import detect_encoding
...
from orbyte.file_processing.extract_file_text import read_text_file
from orbyte.file_processing.file_types import OrbyteFileExtensions
from orbyte.utils.logger import setup_logger
from orbyte.utils.retry_wrapper import request_with_retries
...
def _process_egnyte_file(
    ...
    if extension not in OrbyteFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:
        logger.warning("Skipping file '%s' with extension '%s'", file_name, extension)
        return None

    # Extract text content based on file type
    # TODO @wenxi-orbyte: convert to extract_text_and_images
    if extension in OrbyteFileExtensions.PLAIN_TEXT_EXTENSIONS:
        encoding = detect_encoding(file_content)
        file_content_raw, file_metadata = read_text_file(
            file_content, encoding=encoding, ignore_orbyte_metadata=False
        )
    else:
