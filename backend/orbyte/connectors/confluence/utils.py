...
from orbyte.configs.app_configs import (
    CONFLUENCE_CONNECTOR_ATTACHMENT_CHAR_COUNT_THRESHOLD,
)
from orbyte.configs.app_configs import CONFLUENCE_CONNECTOR_ATTACHMENT_SIZE_THRESHOLD
...
def validate_attachment_filetype(
    ...
    if media_type.startswith("image/"):
        return media_type in OrbyteMimeTypes.IMAGE_MIME_TYPES

    # For non-image files, check if we support the extension
    title = attachment.get("title", "")
    extension = get_file_ext(title)

    return extension in OrbyteFileExtensions.ALL_ALLOWED_EXTENSIONS
...
def _process_image_attachment(
    confluence_client: "OrbyteConfluence",  # noqa: ARG001
    attachment: dict[str, Any],
    raw_bytes: bytes,
    media_type: str,
) -> AttachmentProcessingResult:
