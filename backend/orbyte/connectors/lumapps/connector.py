...
from orbyte.connectors.lumapps.client import LumAppsClientError
from orbyte.connectors.lumapps.client import OrbyteLumApps
from orbyte.connectors.lumapps.models import LumAppsCheckpoint
from orbyte.connectors.lumapps.utils import extract_body_text
from orbyte.connectors.lumapps.utils import pick_lang
from orbyte.connectors.lumapps.utils import resolve_metadata_labels
from orbyte.connectors.lumapps.utils import slugify_family_key
...
class LumAppsConnector(CheckpointedConnector[LumAppsCheckpoint], SlimConnector):
    """Ingests LumApps content (pages/news/custom) into Orbyte.

    Carries over **all** LumApps metadata families as Orbyte metadata
    ...
    visible to all Orbyte users regardless of the original LumApps audience. It is not a
    ...
    def __init__(
        ...
        self._client: OrbyteLumApps | None = None
    ...
    def _get_client(self) -> OrbyteLumApps:
    ...
    # --------------------------------------------------------------- label resolve
    def _family_name(self, family_id: str, client: OrbyteLumApps) -> str:
    ...
    def _resolve_labels(
        self, metadata_ids: list[str], client: OrbyteLumApps
    ) -> dict[str, list[str]]:
    ...
    def _content_to_document(
        self, content: dict[str, Any], client: OrbyteLumApps
    ) -> Document:
