...
class OrbyteRedisSlackRetryHandler(RetryHandler):
    """
    ...
    via an override of _perform_urllib_http_request in OrbyteSlackWebClient.
        ...
        *,
        ...
        try:
            if retry_after_header_name is None:
                ...
                raise ValueError(
                    "OrbyteRedisSlackRetryHandler.prepare_for_next_attempt: retry-after header name is None"
                )

            retry_after_header_value = response.headers.get(retry_after_header_name)
            if not retry_after_header_value:
                raise ValueError(
                    "OrbyteRedisSlackRetryHandler.prepare_for_next_attempt: retry-after header value is None"
                )
            ...
            if parsed_retry_after is None:
                raise ValueError(
                    "OrbyteRedisSlackRetryHandler.prepare_for_next_attempt: "
        ...
        logger.warning(
            "OrbyteRedisSlackRetryHandler.prepare_for_next_attempt setting delay: current_attempt=%s retry-after=%s ttl_ms_new=%r",
