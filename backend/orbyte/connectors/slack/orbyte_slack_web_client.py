...
from orbyte.connectors.slack.utils import ORBYTE_SLACK_LOCK_BLOCKING_TIMEOUT
from orbyte.connectors.slack.utils import ORBYTE_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT
from orbyte.connectors.slack.utils import ORBYTE_SLACK_LOCK_TTL
...
class OrbyteSlackWebClient(WebClient):
    """Use in combination with the Orbyte Retry Handler.
    ...
    def _perform_urllib_http_request(
        ...
        lock: RedisLock = self._redis.lock(
            self._delay_lock,
            timeout=ORBYTE_SLACK_LOCK_TTL,
        )

        # try to acquire the lock
        start = time.monotonic()
        while True:
            acquired = lock.acquire(blocking_timeout=ORBYTE_SLACK_LOCK_BLOCKING_TIMEOUT)
            if acquired:
                break
            ...
            if time.monotonic() - start > ORBYTE_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT:
                raise RuntimeError(
                    f"OrbyteSlackWebClient._perform_urllib_http_request - "
                    f"timed out waiting for lock: {ORBYTE_SLACK_LOCK_TOTAL_BLOCKING_TIMEOUT=}"
                )
        ...
        try:
            ...
            if lock.owned():
                ...
                logger.warning(
                    "OrbyteSlackWebClient._perform_urllib_http_request lock not owned on release"
                )

        time.monotonic() - start
        # logger.info(
        #     f"OrbyteSlackWebClient._perform_urllib_http_request: Releasing lock: {elapsed=}"
        # )
    ...
    def _perform_urllib_http_request_internal(
        ...
        if delay_ms > 0:
            logger.warning(
                "OrbyteSlackWebClient._perform_urllib_http_request_internal delay: delay_ms=%r self.num_requests=%r",
                delay_ms,
                self.num_requests,
            )
