...
from orbyte.file_processing.extract_file_text import extract_text_and_images
...
def _process_file(
    ...
    if extension not in OrbyteFileExtensions.ALL_ALLOWED_EXTENSIONS:
        logger.warning(
    ...
    orbyte_metadata, custom_tags = process_orbyte_metadata(metadata)
    file_display_name = orbyte_metadata.file_display_name or os.path.basename(file_name)
    time_updated = orbyte_metadata.doc_updated_at or datetime.now(timezone.utc)
    primary_owners = orbyte_metadata.primary_owners
    secondary_owners = orbyte_metadata.secondary_owners
    link = orbyte_metadata.link

    # These metadata items are not settable by the user
    source_type = orbyte_metadata.source_type or DocumentSource.FILE

    doc_id = orbyte_metadata.document_id or f"FILE_CONNECTOR__{file_id}"
    title = metadata.get("title") or file_display_name

    # 1) If the file itself is an image, handle that scenario quickly
    if extension in OrbyteFileExtensions.IMAGE_EXTENSIONS:
        # Read the image data
    ...
    # Each file may have file-specific ORBYTE_METADATA https://docs.onyx.app/admins/connectors/official/file
    ...
    if extraction_result.metadata:
        ...
        orbyte_metadata, more_custom_tags = process_orbyte_metadata(
            extraction_result.metadata
        )
        ...
        source_type = orbyte_metadata.source_type or source_type
        primary_owners = orbyte_metadata.primary_owners or primary_owners
        secondary_owners = orbyte_metadata.secondary_owners or secondary_owners
        time_updated = orbyte_metadata.doc_updated_at or time_updated
        file_display_name = orbyte_metadata.file_display_name or file_display_name
        title = orbyte_metadata.title or orbyte_metadata.file_display_name or title
        link = orbyte_metadata.link or link
    ...
    if is_tabular_file(file_name):
        ...
        if lowered_name.endswith(tuple(OrbyteFileExtensions.SPREADSHEET_EXTENSIONS)):
            file.seek(0)
