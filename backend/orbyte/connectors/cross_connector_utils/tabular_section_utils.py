...
from orbyte.file_processing.extract_file_text import stage_xlsx_sheets
...
def is_tabular_file(file_name: str) -> bool:
    ...
    return any(lowered.endswith(ext) for ext in OrbyteFileExtensions.TABULAR_EXTENSIONS)
...
def tabular_file_to_sections(
    ...
    if not lowered.endswith(tuple(OrbyteFileExtensions.TABULAR_EXTENSIONS)):
        raise ValueError(f"{file_name!r} is not a tabular file")

    if lowered.endswith(tuple(OrbyteFileExtensions.SPREADSHEET_EXTENSIONS)):
        return [
            TabularSection(
