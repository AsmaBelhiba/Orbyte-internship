...
from orbyte.configs.app_configs import GOOGLE_DRIVE_ADVANCED_PARSE_MAX_BYTES
from orbyte.configs.constants import DocumentSource
from orbyte.configs.constants import FileOrigin
from orbyte.connectors.cross_connector_utils.section_utils import cap_sections_text
...
from orbyte.connectors.google_drive.file_retrieval import DRIVE_RESOURCE_KEY_FIELD
from orbyte.connectors.google_drive.models import GDriveMimeType
from orbyte.connectors.google_drive.models import GoogleDriveFileType
from orbyte.connectors.google_drive.section_extraction import get_document_sections
from orbyte.connectors.google_drive.section_extraction import HEADING_DELIMITER
from orbyte.connectors.google_utils.resources import get_drive_service
from orbyte.connectors.google_utils.resources import get_google_authorized_session
...
from orbyte.file_processing.extract_file_text import read_docx_file
from orbyte.file_processing.extract_file_text import read_pdf_file
from orbyte.file_processing.extract_file_text import read_pptx_file
from orbyte.file_processing.file_types import OrbyteFileExtensions
from orbyte.file_processing.file_types import OrbyteMimeTypes
from orbyte.file_processing.file_types import PRESENTATION_MIME_TYPE
from orbyte.file_processing.file_types import SPREADSHEET_MIME_TYPE
from orbyte.file_processing.image_utils import make_image_callback
from orbyte.file_processing.image_utils import store_image_and_create_section
...
def orbyte_document_id_from_drive_file(file: GoogleDriveFileType) -> str:
...
def _download_and_extract_sections_basic(
    ...
    if mime_type in OrbyteMimeTypes.IMAGE_MIME_TYPES:
    ...
    if mime_type in GOOGLE_MIME_TYPES_TO_EXPORT:
        ...
        if export_mime_type in OrbyteMimeTypes.TABULAR_MIME_TYPES:
    ...
    if file_ext not in OrbyteFileExtensions.ALL_ALLOWED_EXTENSIONS:
...
def _get_external_access_for_raw_gdrive_file(
    ...
    external_access_fn = cast(
        ...
        fetch_versioned_implementation_with_fallback(
            "orbyte.external_permissions.google_drive.doc_sync",
...
def _convert_drive_item_to_document(
    ...
    try:
        ...
        doc_id = orbyte_document_id_from_drive_file(file)
        external_access = (
...
def build_slim_document(
    ...
    return SlimDocument(
        id=orbyte_document_id_from_drive_file(file),
        external_access=external_access,
        parent_hierarchy_raw_node_id=(file.get("parents") or [None])[0],
        doc_created_at=(
            datetime.fromisoformat(created_time.replace("Z", "+00:00"))
