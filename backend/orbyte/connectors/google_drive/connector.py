...
from orbyte.configs.app_configs import GOOGLE_DRIVE_CONNECTOR_SIZE_THRESHOLD
from orbyte.configs.app_configs import INDEX_BATCH_SIZE
from orbyte.configs.app_configs import MAX_DRIVE_WORKERS
...
from orbyte.connectors.google_drive.file_retrieval import DriveFileFieldType
from orbyte.connectors.google_drive.file_retrieval import get_all_files_for_oauth
from orbyte.connectors.google_drive.file_retrieval import (
    get_all_files_in_my_drive_and_shared,
)
from orbyte.connectors.google_drive.file_retrieval import get_external_access_for_folder
from orbyte.connectors.google_drive.file_retrieval import (
    get_files_by_web_view_links_batch,
)
from orbyte.connectors.google_drive.file_retrieval import get_files_in_shared_drive
from orbyte.connectors.google_drive.file_retrieval import get_folder_metadata
from orbyte.connectors.google_drive.file_retrieval import get_root_folder_id
from orbyte.connectors.google_drive.file_retrieval import get_shared_drive_name
from orbyte.connectors.google_drive.file_retrieval import has_link_only_permission
...
from orbyte.connectors.google_drive.models import StageCompletion
from orbyte.connectors.google_utils.google_auth import get_google_creds
from orbyte.connectors.google_utils.google_utils import execute_paginated_retrieval
from orbyte.connectors.google_utils.google_utils import get_file_owners
from orbyte.connectors.google_utils.google_utils import GoogleFields
from orbyte.connectors.google_utils.resources import get_admin_service
from orbyte.connectors.google_utils.resources import get_drive_service
from orbyte.connectors.google_utils.resources import GoogleDriveService
from orbyte.connectors.google_utils.resources import ImpersonationError
from orbyte.connectors.google_utils.resources import make_user_removal_checker
from orbyte.connectors.google_utils.shared_constants import (
    DB_CREDENTIALS_PRIMARY_ADMIN_KEY,
)
from orbyte.connectors.google_utils.shared_constants import MISSING_SCOPES_ERROR_STR
from orbyte.connectors.google_utils.shared_constants import ORBYTE_SCOPE_INSTRUCTIONS
from orbyte.connectors.google_utils.shared_constants import SLIM_BATCH_SIZE
from orbyte.connectors.google_utils.shared_constants import USER_FIELDS
...
from orbyte.connectors.interfaces import NormalizationResult
from orbyte.connectors.interfaces import Resolver
from orbyte.connectors.interfaces import SecondsSinceUnixEpoch
from orbyte.connectors.interfaces import SlimConnector
from orbyte.connectors.interfaces import SlimConnectorWithPermSync
...
from orbyte.utils.threadpool_concurrency import parallel_yield
...
class GoogleDriveConnector(
    ...
    def normalize_url(cls, url: str) -> NormalizationResult:
        ...
        if parse_qs(parsed.query).get("id"):
            normalized = orbyte_document_id_from_drive_file(
                {
    ...
    def _checkpointed_retrieval(
        ...
        for file in drive_files:
            ...
            try:
                document_id = orbyte_document_id_from_drive_file(drive_file)
    ...
    def _load_from_checkpoint(
        ...
        try:
            ...
            if MISSING_SCOPES_ERROR_STR in str(e):
                raise PermissionError(ORBYTE_SCOPE_INSTRUCTIONS) from e
            raise e
    ...
    def validate_connector_settings(self) -> None:
        ...
        try:
            ...
            if MISSING_SCOPES_ERROR_STR in str(e):
                raise InsufficientPermissionsError(
                    f"Google Drive credentials are missing required scopes. {ORBYTE_SCOPE_INSTRUCTIONS}"
                )
            raise ConnectorValidationError(
    ...
    def probe_directory_admin_permission(self) -> None:
        ...
        try:
            ...
            if MISSING_SCOPES_ERROR_STR in str(e):
                raise InsufficientPermissionsError(
                    f"Google Drive credentials are missing required scopes. {ORBYTE_SCOPE_INSTRUCTIONS} Full error: {e}"
                )
            raise ConnectorValidationError(
