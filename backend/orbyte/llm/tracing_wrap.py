...
Every concrete subclass of `orbyte.llm.interfaces.LLM` has its `invoke` and
...
Imports from `orbyte.tracing.llm_utils` stay lazy (inside the wrappers) because
it imports `orbyte.llm.interfaces`, which imports this module — loading it at
...
from orbyte.tracing.framework.create import get_current_span
from orbyte.tracing.framework.span_data import GenerationSpanData
...
_ALREADY_WRAPPED_ATTR = "_orbyte_tracing_wrapped"
...
def wrap_stream(
    ...
    def wrapper(
        ...
        from orbyte.tracing.flows import LLMFlow
        from orbyte.tracing.llm_utils import llm_generation_span
        from orbyte.tracing.llm_utils import record_llm_span_output
...
def _finalize_tool_calls(
    ...
    from orbyte.llm.models import FunctionCall as ModelFunctionCall
    from orbyte.llm.models import ToolCall
