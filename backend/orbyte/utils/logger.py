...
class OrbyteRequestIDFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        from shared_configs.contextvars import ORBYTE_REQUEST_ID_CONTEXTVAR

        record.request_id = ORBYTE_REQUEST_ID_CONTEXTVAR.get() or "-"
        return True


class OrbyteLoggingAdapter(logging.LoggerAdapter):
    def process(
        ...
        fastapi_request_id = ORBYTE_REQUEST_ID_CONTEXTVAR.get()
...
def _add_file_handlers(logger: logging.Logger, formatter: logging.Formatter) -> None:
    ...
    for level in ["debug", "info", "notice"]:
        file_name = (
            f"/var/log/orbyte/{LOG_FILE_NAME}_{level}.log"
            if is_containerized
        ...
        try:
            ...
            # container whose mounted /var/log/orbyte volume is owned by another UID.
...
def setup_uvicorn_logger(
    ...
    uvicorn_logger.addFilter(OrbyteRequestIDFilter())
