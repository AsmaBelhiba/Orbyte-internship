...
from shared_configs.contextvars import ORBYTE_REQUEST_ID_CONTEXTVAR


def add_orbyte_tenant_id_middleware(
    app: FastAPI,
    ...
    async def set_tenant_id(
        ...
        orbyte_tenant_id = request.headers.get("X-Orbyte-Tenant-ID")
        if orbyte_tenant_id:
            CURRENT_TENANT_ID_CONTEXTVAR.set(orbyte_tenant_id)
        return await call_next(request)


def add_orbyte_request_id_middleware(
    app: FastAPI,
    prefix: str,
    logger: logging.LoggerAdapter,  # noqa: ARG001
) -> None:
    ...
    async def set_request_id(
        ...
        orbyte_request_id = request.headers.get("X-Orbyte-Request-ID")
        if not orbyte_request_id:
            orbyte_request_id = make_randomized_orbyte_request_id(prefix)

        ORBYTE_REQUEST_ID_CONTEXTVAR.set(orbyte_request_id)
        return await call_next(request)


def make_randomized_orbyte_request_id(prefix: str) -> str:
    """generates a randomized request id"""

    hash_input = str(uuid.uuid4())
    return _make_orbyte_request_id(prefix, hash_input)


def make_structured_orbyte_request_id(prefix: str, request_url: str) -> str:
    """Not used yet, but could be in the future!"""
    hash_input = f"{request_url}:{datetime.now(timezone.utc)}"
    return _make_orbyte_request_id(prefix, hash_input)


def _make_orbyte_request_id(prefix: str, hash_input: str) -> str:
    ...
    orbyte_request_id = f"{prefix}:{hash_str}"
    return orbyte_request_id
...
def add_endpoint_context_middleware(app: FastAPI) -> None:
    ...
    Used by ``orbyte_db_connections_held_by_endpoint`` and
    ``orbyte_db_connection_hold_seconds`` in the pool event listeners.
