...
The emitted line goes to a dedicated logger named ``orbyte.audit.credential_access``
...
# object (the Orbyte standard formatter prepends a human-readable prefix, which
# would make machine parsing harder). The logger propagates to the root so it
# still reaches the configured handlers / log files.
AUDIT_LOGGER_NAME = "orbyte.audit.credential_access"
