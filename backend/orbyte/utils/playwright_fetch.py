...
logic that was originally embedded in `orbyte.connectors.web.connector`.

Two consumers:
- `WebConnector` (long-lived `BrowserContext` reused across many pages
  in a single crawl) uses `start_playwright()` directly.
- `OrbyteWebCrawler` (lazy fallback when its `requests`-based fetch hits
...
from orbyte.configs.app_configs import WEB_CONNECTOR_OAUTH_CLIENT_ID
from orbyte.configs.app_configs import WEB_CONNECTOR_OAUTH_CLIENT_SECRET
from orbyte.configs.app_configs import WEB_CONNECTOR_OAUTH_TOKEN_URL
...
def start_playwright() -> tuple[Playwright, BrowserContext]:
    ...
    OrbyteWebCrawler fallback. Caller owns lifecycle and must call
...
def looks_like_cloudflare_challenge(html: str) -> bool:
    ...
    Used by callers (e.g. `OrbyteWebCrawler`) to distinguish "we got real
