...
access-control change, credential access) on the ``orbyte.audit`` logger tree.
...
AUDIT_LOGGER_ROOT = "orbyte.audit"
AUDIT_HANDLER_NAME = "orbyte_audit_stdout"

# Diagnostics logger for the subsystem's own failures. Deliberately NOT under
# ``orbyte.audit`` so internal warnings never land in the parsed audit stream.
...
def _safe_get_request_id() -> str | None:
    try:
        ...
        return ORBYTE_REQUEST_ID_CONTEXTVAR.get()
    except Exception:
        return None
...
def _safe_get_client_ip() -> str | None:
    try:
        from orbyte.utils.client_ip import current_client_ip
...
def _configure_audit_logging() -> None:
    """Own an INFO stdout handler on ``orbyte.audit`` so events reach stdout
