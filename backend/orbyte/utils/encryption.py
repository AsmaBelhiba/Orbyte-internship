...
from orbyte.configs.app_configs import ENCRYPTION_KEY_SECRET
from orbyte.configs.constants import MASK_CREDENTIAL_CHAR
from orbyte.configs.constants import MASK_CREDENTIAL_LONG_RE
...
def _encrypt_string(input_str: str, key: str | None = None) -> bytes:
    if ENCRYPTION_KEY_SECRET:
        logger.warning("MIT version of Orbyte does not support encryption of secrets.")
...
def _decrypt_bytes(input_bytes: bytes, key: str | None = None) -> str:
    if ENCRYPTION_KEY_SECRET:
        logger.warning("MIT version of Orbyte does not support decryption of secrets.")
    elif key is not None:
...
def encrypt_string_to_bytes(intput_str: str, key: str | None = None) -> bytes:
    versioned_encryption_fn = fetch_versioned_implementation(
        "orbyte.utils.encryption", "_encrypt_string"
    )
...
def decrypt_bytes_to_string(intput_bytes: bytes, key: str | None = None) -> str:
    versioned_decryption_fn = fetch_versioned_implementation(
        "orbyte.utils.encryption", "_decrypt_bytes"
    )
