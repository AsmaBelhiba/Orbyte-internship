...
from orbyte.configs.app_configs import MAX_EMBEDDED_IMAGES_PER_FILE
from orbyte.configs.app_configs import MAX_XLSX_CELLS_PER_SHEET
from orbyte.configs.app_configs import PDF_TEXT_EXTRACTION_TIMEOUT_SECONDS
...
from orbyte.file_processing.file_types import WORD_PROCESSING_MIME_TYPE
from orbyte.file_processing.html_utils import parse_html_page_basic
from orbyte.file_processing.unstructured import get_unstructured_api_key
from orbyte.file_processing.unstructured import unstructured_to_text
from orbyte.utils.logger import setup_logger
from orbyte.utils.process_isolation import IsolatedProcessError
from orbyte.utils.process_isolation import run_in_isolated_process
...
def load_files_from_zip(
    ...
    with zipfile.ZipFile(zip_file_io, "r") as zip_file:
        for file_info in zip_file.infolist():
            ...
            ) or file_info.filename == ORBYTE_METADATA_FILENAME:
                continue
...
def _extract_orbyte_metadata(line: str) -> dict | None:
    """
    Example: first line has:
        <!-- ORBYTE_METADATA={"title": "..."} -->
      or
        #ORBYTE_METADATA={"title":"..."}
    """
    html_comment_pattern = r"<!--\s*ORBYTE_METADATA=\{(.*?)\}\s*-->"
    hashtag_pattern = r"#ORBYTE_METADATA=\{(.*?)\}"
...
def read_text_file(
    ...
    encoding: str = "utf-8",
    errors: str = "replace",
    ignore_orbyte_metadata: bool = True,
) -> tuple[str, dict]:
    """
    For plain text files. Optionally extracts Orbyte metadata from the first line.
    """
    metadata = {}
    file_content_raw = ""
    for ind, line in enumerate(file):
        ...
        if ind == 0 and not ignore_orbyte_metadata:
            potential_meta = _extract_orbyte_metadata(line)
...
def read_docx_file(
    ...
    try:
        ...
        text_content_raw, _ = read_text_file(
            file, encoding=encoding, ignore_orbyte_metadata=False
        )
...
def extract_file_text(
    ...
    try:
        ...
        if extension in OrbyteFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:
...
def _extract_text_and_images(
    ...
    if content_type in OrbyteMimeTypes.TEXT_MIME_TYPES:
    ...
    try:
        ...
        if extension in OrbyteFileExtensions.PLAIN_TEXT_EXTENSIONS:
            return extract_result_from_text_file(file)
