...
The Orbyte file store provides a unified interface for storing files and large binary objects. It supports three storage backends: S3-compatible storage (AWS S3, MinIO, Digital Ocean Spaces, etc.), Google Cloud Storage (GCS), and PostgreSQL Large Objects.
...
S3_FILE_STORE_BUCKET_NAME=your-bucket-name  # Defaults to 'orbyte-file-store-bucket'
S3_FILE_STORE_PREFIX=orbyte-files  # Optional, defaults to 'orbyte-files'
...
GCS_FILE_STORE_PREFIX=orbyte-files               # Optional, defaults to 'orbyte-files'
...
The blob storage connector (`backend/orbyte/connectors/blob/connector.py`) also supports native GCS authentication via the admin UI. When creating a Google Cloud Storage connector, three auth methods are available:
