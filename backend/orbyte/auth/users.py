...
from orbyte.auth.disposable_email_validator import is_disposable_email
from orbyte.auth.email_utils import send_forgot_password_email
from orbyte.auth.email_utils import send_user_verification_email
...
from orbyte.auth.jwt import verify_jwt_token
from orbyte.auth.pat import get_hashed_pat_from_request
from orbyte.auth.schemas import AuthBackend
from orbyte.auth.schemas import UserCreate
from orbyte.auth.schemas import UserRole
from orbyte.auth.signup_rate_limit import enforce_signup_rate_limit
from orbyte.configs.app_configs import AUTH_BACKEND
from orbyte.configs.app_configs import AUTH_COOKIE_EXPIRE_TIME_SECONDS
...
from orbyte.configs.app_configs import JWT_PUBLIC_KEY_URL
from orbyte.configs.app_configs import REDIS_AUTH_KEY_PREFIX
from orbyte.configs.app_configs import REQUIRE_EMAIL_VERIFICATION
...
from orbyte.configs.constants import ANONYMOUS_USER_COOKIE_NAME
...
from orbyte.configs.constants import DANSWER_API_KEY_DUMMY_EMAIL_DOMAIN
from orbyte.configs.constants import DANSWER_API_KEY_PREFIX
...
from orbyte.configs.constants import PASSWORD_SPECIAL_CHARS
from orbyte.configs.constants import UNNAMED_KEY_PLACEHOLDER
from orbyte.db.api_key import fetch_user_for_api_key
from orbyte.db.auth import get_access_token_db
from orbyte.db.auth import get_default_admin_user_emails
from orbyte.db.auth import get_user_count
from orbyte.db.auth import get_user_db
from orbyte.db.auth import SQLAlchemyUserAdminDB
from orbyte.db.engine.async_sql_engine import get_async_session
from orbyte.db.engine.async_sql_engine import get_async_session_context_manager
...
from orbyte.db.models import AccessToken
from orbyte.db.models import OAuthAccount
from orbyte.db.models import Persona
from orbyte.db.models import User
from orbyte.db.pat import resolve_pat
from orbyte.db.users import assign_user_to_default_groups__no_commit
from orbyte.db.users import get_user_by_email
from orbyte.db.users import is_limited_user
...
from orbyte.redis.redis_pool import retrieve_ws_token_data
...
from orbyte.utils.audit import AuditActor
...
from orbyte.utils.telemetry import mt_cloud_identify_user
...
def anonymous_user_enabled(*, tenant_id: str | None = None) -> bool:
    ...
    value = cache.get(OrbyteRedisLocks.ANONYMOUS_USER_ENABLED)
...
def verify_email_is_invited(email: str) -> None:
    ...
    if not email:
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, "Email must be specified")

    try:
        email_info = validate_email(email, check_deliverability=False)
    except EmailUndeliverableError:
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, "Email is not valid")
...
def enforce_seat_limit(
    ...
    """Raise ``OrbyteError(SEAT_LIMIT_EXCEEDED)`` if seats would be exceeded.
    ...
    result = fetch_ee_implementation_or_noop(
        "orbyte.db.license", "check_seat_availability", None
    )(db_session, seats_needed=seats_needed)

    if result is not None and not result.available:
        raise OrbyteError(OrbyteErrorCode.SEAT_LIMIT_EXCEEDED, result.error_message)
...
def enforce_seat_limit_locked(db_session: Session, seats_needed: int = 1) -> None:
    ...
    """
    fetch_ee_implementation_or_noop("orbyte.db.license", "acquire_seat_lock", None)(
...
def _user_currently_counts_toward_seats(user: User) -> bool:
    """Delegate to canonical ``ee/orbyte/db/license.py:user_counts_toward_seats``."""
    return bool(
        fetch_ee_implementation_or_noop(
            "orbyte.db.license", "user_counts_toward_seats", False
        )(user)
    )
...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    async def create(
        ...
        from orbyte.auth.captcha import CaptchaAction
        from orbyte.auth.captcha import CaptchaVerificationError
        from orbyte.auth.captcha import is_captcha_enabled
        from orbyte.auth.captcha import verify_captcha_token
        ...
        tenant_id = await fetch_ee_implementation_or_noop(
            "orbyte.server.tenants.provisioning",
    ...
    async def on_after_register(
        ...
        get_marketing_posthog_cookie_name = fetch_ee_implementation_or_noop(
            module="orbyte.utils.posthog_client",
        ...
        if (
            ...
            properties = {
                "email": user.email,
                "orbyte_cloud_user_id": str(user.id),
...
# Orbyte's own API-key/PAT handlers (see `optional_user`).
...
def get_default_admin_user_emails_() -> list[str]:
    # No default seeding available for Orbyte MIT
    return []
