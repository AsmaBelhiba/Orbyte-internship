...
from orbyte.external_apps.presentation.payload_decoders import PayloadDecoder
...
class OrbyteManagedExtApp(ExternalAppProvider, abstract=True):
    """Interface for a built-in provider whose OAuth client credentials Orbyte
    ...
    inherit this, so it carries no Orbyte-owned credentials and stays editable.
    ...
    # Orbyte-owned credential values, sourced from the ``EXT_APP_<APP_TYPE>_<FIELD>``
    ...
    def __init_subclass__(cls, *, abstract: bool = False, **kwargs: Any) -> None:
        ...
        if configured != required:
            raise TypeError(
                f"{cls.__name__} is an OrbyteManagedExtApp but its "
    ...
    def configured_managed_credentials(self) -> dict[str, str] | None:
        """This provider's Orbyte-owned credentials if fully configured, else None."""
...
class OAuthExternalAppProvider(ExternalAppProvider, abstract=True):
    ...
    def extract_credentials(self, response_data: dict[str, Any]) -> dict[str, Any]:
        ...
        of Slack's nested ``authed_user``). Raise ``OrbyteError`` if the expected
        token is absent."""
