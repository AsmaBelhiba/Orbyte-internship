...
from orbyte.external_apps.models import ActionPolicyView
from orbyte.external_apps.models import BuiltInExternalAppDescriptor
from orbyte.external_apps.models import EndpointDescriptor
from orbyte.external_apps.models import OrgCredentialFieldDescriptor
from orbyte.external_apps.providers.actions import EndpointSpec
from orbyte.external_apps.providers.base import ExternalAppProvider
from orbyte.external_apps.providers.base import OrbyteManagedExtApp
from orbyte.external_apps.providers.github import GitHubProvider
...
from orbyte.external_apps.providers.linear import LinearProvider
from orbyte.external_apps.providers.notion import NotionProvider
...
def get_orbyte_managed_provider(app_type: ExternalAppType) -> OrbyteManagedExtApp | None:
    ...
    None`` is the "is this app Orbyte-managed" check)."""
    provider = PROVIDERS.get(app_type)
    return provider if isinstance(provider, OrbyteManagedExtApp) else None
...
def fetch_orbyte_managed_built_in_apps() -> list[BuiltInExternalAppDescriptor]:
    """Built-in providers Orbyte owns the credentials for — the apps cloud
    provisioning seeds per tenant. Excludes non-managed built-ins."""
    return [
        _descriptor_for(cls)
        for cls in _PROVIDER_CLASSES
        if issubclass(cls, OrbyteManagedExtApp)
    ]
