...
from orbyte.configs.app_configs import DISABLE_TELEMETRY
from orbyte.configs.constants import ID_SEPARATOR
from orbyte.configs.constants import MessageType
from orbyte.configs.orbytebot_configs import ORBYTE_BOT_FEEDBACK_VISIBILITY
from orbyte.configs.orbytebot_configs import ORBYTE_BOT_MAX_QPM
...
_ORBYTE_BOT_MESSAGE_COUNT: int = 0
_ORBYTE_BOT_COUNT_START_TIME: float = time.time()


def get_orbyte_bot_auth_ids(
...
def check_message_limit() -> bool:
    ...
    """
    if ORBYTE_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD <= 0:
        return True
    global _ORBYTE_BOT_MESSAGE_COUNT
    global _ORBYTE_BOT_COUNT_START_TIME
    time_since_start = time.time() - _ORBYTE_BOT_COUNT_START_TIME
    if time_since_start > ORBYTE_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS:
        _ORBYTE_BOT_MESSAGE_COUNT = 0
        _ORBYTE_BOT_COUNT_START_TIME = time.time()
    if (_ORBYTE_BOT_MESSAGE_COUNT + 1) > ORBYTE_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD:
        logger.error(
            "OrbyteBot has reached the message limit %s"
            " for the time period %s seconds."
            " These limits are configurable in backend/orbyte/configs/orbytebot_configs.py",
            ORBYTE_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD,
            ORBYTE_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS,
        )
        return False
    _ORBYTE_BOT_MESSAGE_COUNT += 1
    return True
...
def _build_error_block(error_message: str) -> Block:
    ...
    """
    display_text = (
        "There was an error displaying all of the Orbyte answers."
        f" Please let an admin or an orbyte developer know. Error: {error_message}"
    )
    return SectionBlock(text=display_text)


@retry_builder(
    tries=ORBYTE_BOT_NUM_RETRIES,
    delay=0.25,
    backoff=2,
)
    ...
    for reply in replies:
        if "user" in reply and "bot_id" not in reply:
            ...
            if is_orbyte_bot_response:
                ...
                if not blocks:
                    logger.warning("OrbyteBot response has no blocks: %s", reply)
                    continue
                ...
                if message is not None and message.startswith("_Filters"):
                    if len(blocks) < 2:
                        ...
                        continue
                    # This is the OrbyteBot answer format, if there is a change to how we respond,
                    # this will need to be updated to get the correct "answer" portion
                    message = reply["blocks"][1].get("text", {}).get("text")
            else:
                # Other bots are not counted as the LLM response which only comes from Orbyte
                message_type = MessageType.USER
        ...
        message = remove_orbyte_bot_tag(tenant_id, slack_bot_id, message, client=client)
...
def slack_usage_report(action: str, sender_id: str | None, client: WebClient) -> None:
    ...
    if sender_email is not None:
        with get_session_with_current_tenant() as db_session:
            orbyte_user = get_user_by_email(email=sender_email, db_session=db_session)

    optional_telemetry(
        record_type=RecordType.USAGE,
        data={"action": action},
        user_id=str(orbyte_user.id) if orbyte_user else "Non-Orbyte-Or-No-Auth-User",
    )


class SlackRateLimiter:
    def __init__(self) -> None:
        self.max_qpm: int | None = ORBYTE_BOT_MAX_QPM
        self.max_wait_time = ORBYTE_BOT_MAX_WAIT_TIME
...
def get_feedback_visibility() -> FeedbackVisibility:
    try:
        return FeedbackVisibility(ORBYTE_BOT_FEEDBACK_VISIBILITY.lower())
    except ValueError:
...
class TenantSocketModeClient(SocketModeClient):
    def __init__(self, tenant_id: str, slack_bot_id: int, *args: Any, **kwargs: Any):
        ...
        # before these attributes exist (ORBYTE-BACKEND-1: AttributeError on
        # _tenant_id).
