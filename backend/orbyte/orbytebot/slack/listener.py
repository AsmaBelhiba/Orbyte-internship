...
from orbyte.configs.app_configs import POD_NAME
from orbyte.configs.app_configs import POD_NAMESPACE
from orbyte.configs.constants import MessageType
from orbyte.configs.constants import OrbyteRedisLocks
from orbyte.configs.orbytebot_configs import NOTIFY_SLACKBOT_NO_ANSWER
from orbyte.connectors.slack.utils import expert_info_from_slack_id
...
from orbyte.db.slack_bot import fetch_slack_bot
from orbyte.db.slack_bot import fetch_slack_bots
...
from orbyte.orbytebot.slack.config import get_slack_channel_config_for_bot_and_channel
from orbyte.orbytebot.slack.config import MAX_TENANTS_PER_POD
from orbyte.orbytebot.slack.config import TENANT_ACQUISITION_INTERVAL
from orbyte.orbytebot.slack.config import TENANT_HEARTBEAT_EXPIRATION
from orbyte.orbytebot.slack.config import TENANT_HEARTBEAT_INTERVAL
from orbyte.orbytebot.slack.config import TENANT_LOCK_EXPIRATION
from orbyte.orbytebot.slack.constants import DISLIKE_BLOCK_ACTION_ID
from orbyte.orbytebot.slack.constants import FEEDBACK_DOC_BUTTON_BLOCK_ACTION_ID
from orbyte.orbytebot.slack.constants import FOLLOWUP_BUTTON_ACTION_ID
from orbyte.orbytebot.slack.constants import FOLLOWUP_BUTTON_RESOLVED_ACTION_ID
from orbyte.orbytebot.slack.constants import GENERATE_ANSWER_BUTTON_ACTION_ID
from orbyte.orbytebot.slack.constants import IMMEDIATE_RESOLVED_BUTTON_ACTION_ID
from orbyte.orbytebot.slack.constants import KEEP_TO_YOURSELF_ACTION_ID
from orbyte.orbytebot.slack.constants import LIKE_BLOCK_ACTION_ID
from orbyte.orbytebot.slack.constants import SHOW_EVERYONE_ACTION_ID
...
from orbyte.orbytebot.slack.handlers.handle_buttons import handle_slack_feedback
from orbyte.orbytebot.slack.handlers.handle_message import handle_message
from orbyte.orbytebot.slack.handlers.handle_message import (
    remove_scheduled_feedback_reminder,
)
from orbyte.orbytebot.slack.handlers.handle_message import schedule_feedback_reminder
from orbyte.orbytebot.slack.models import SlackContext
from orbyte.orbytebot.slack.models import SlackMessageInfo
from orbyte.orbytebot.slack.models import ThreadMessage
from orbyte.orbytebot.slack.utils import check_message_limit
from orbyte.orbytebot.slack.utils import decompose_action_id
from orbyte.orbytebot.slack.utils import get_channel_name_from_id
from orbyte.orbytebot.slack.utils import get_channel_type_from_id
from orbyte.orbytebot.slack.utils import get_orbyte_bot_auth_ids
from orbyte.orbytebot.slack.utils import read_slack_thread
from orbyte.orbytebot.slack.utils import remove_orbyte_bot_tag
from orbyte.orbytebot.slack.utils import respond_in_thread_or_channel
from orbyte.orbytebot.slack.utils import TenantSocketModeClient
from orbyte.redis.redis_pool import get_redis_client
from orbyte.server.manage.models import SlackBotTokens
...
class SlackbotHandler:
    ...
    def acquire_tenants(self) -> None:
        ...
        gated_tenants = fetch_ee_implementation_or_noop(
            "orbyte.server.tenants.product_gating",
            "get_gated_tenants",
            set(),
        )()
        all_active_tenants = [
        ...
        for tenant_id in all_active_tenants:
            ...
            rlock: RedisLock = redis_client.lock(
                OrbyteRedisLocks.SLACK_BOT_LOCK,
    ...
    def send_heartbeats(pod_id: str, tenant_ids: set[str]) -> None:
        ...
        for tenant_id in tenant_ids:
            redis_client = get_redis_client(tenant_id=tenant_id)
            heartbeat_key = f"{OrbyteRedisLocks.SLACK_BOT_HEARTBEAT_PREFIX}:{pod_id}"
            redis_client.set(
    ...
    def start_socket_client(
        ...
        # Sentry as ORBYTE-BACKEND-4) and re-raises. The common case is
...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    bot_token_user_id, bot_token_bot_id = get_orbyte_bot_auth_ids(
    ...
    if req.type == "events_api":
        ...
        if event_type == "message":
            ...
            if bot_token_user_id and bot_token_user_id in event_user:
                is_orbyte_bot_msg = True

            if bot_token_bot_id and bot_token_bot_id in event_bot_id:
                is_orbyte_bot_msg = True

            # OrbyteBot should never respond to itself
            if is_orbyte_bot_msg:
                logger.info("Ignoring message from OrbyteBot (self-message)")
                return False

            # DMs with the bot don't pick up the @OrbyteBot so we have to keep the
            # caught events_api
        ...
        if is_bot_message:
            ...
            # If OrbyteBot is not specifically tagged and the channel is not set to respond to bots, ignore the message
        ...
        # Can respond in thread if it's an "im" directly to Orbyte or @OrbyteBot is tagged
        if (
            thread_ts
    ...
    if req.type == "slash_commands":
        ...
        if not channel:
            channel_specific_logger.error(
                "Received OrbyteBot command without channel - skipping"
            )
            return False
        ...
        if not sender:
            channel_specific_logger.error(
                "Cannot respond to OrbyteBot command without sender to respond to."
            )
            return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        bot_token_user_id, _ = get_orbyte_bot_auth_ids(
            tenant_id, client.slack_bot_id, client.web_client
        )
        message_ts = event.get("ts")
        thread_ts = event.get("thread_ts")
        ...
        if tagged:
            logger.debug("User tagged OrbyteBot")
    ...
    if not is_bot_event and not is_duplicate_mention:
        ...
        if channel:
            respond_in_thread_or_channel(
                ...
                thread_ts=thread_ts,
                text=(
                    "Your organization's subscription has expired. Please contact your Orbyte administrator to restore access."
                ),
            )
