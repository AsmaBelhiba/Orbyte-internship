...
from orbyte.chat.process_message import remove_answer_citations
from orbyte.configs.constants import MessageType
from orbyte.configs.constants import SearchFeedbackType
from orbyte.configs.orbytebot_configs import ORBYTE_BOT_FOLLOWUP_EMOJI
...
from orbyte.db.feedback import create_doc_retrieval_feedback
from orbyte.db.users import get_user_by_email
from orbyte.orbytebot.slack.blocks import build_follow_up_resolved_blocks
from orbyte.orbytebot.slack.blocks import build_slack_response_blocks
from orbyte.orbytebot.slack.blocks import get_document_feedback_blocks
...
from orbyte.orbytebot.slack.constants import FeedbackVisibility
...
from orbyte.orbytebot.slack.utils import build_feedback_id
from orbyte.orbytebot.slack.utils import decompose_action_id
from orbyte.orbytebot.slack.utils import fetch_group_ids_from_names
...
from orbyte.orbytebot.slack.utils import get_feedback_visibility
...
from orbyte.orbytebot.slack.utils import update_emote_react
from orbyte.server.query_and_chat.models import ChatMessageDetail
...
def handle_publish_ephemeral_message_button(
    ...
    # Obtain orbyte_user and chat_message information
    if not chat_message_id:
        raise ValueError("Missing chat_message_id in the payload")

    with get_session_with_current_tenant() as db_session:
        orbyte_user = get_user_by_email(user_email, db_session)
        if not orbyte_user:
            raise ValueError("Cannot determine orbyte_user_id from email in payload")
        try:
            chat_message = get_chat_message(chat_message_id, orbyte_user.id, db_session)
        except ValueError:
        ...
        orbyte_bot_answer = ChatBasicResponse(
    ...
    if action_id == SHOW_EVERYONE_ACTION_ID:
        ...
        try:
            ...
            respond_in_thread_or_channel(
                ...
                text="Hello! Orbyte has some results for you!",
                blocks=all_blocks,
...
def handle_followup_button(
    ...
    update_emote_react(
        emoji=ORBYTE_BOT_FOLLOWUP_EMOJI,
        channel=channel_id,
