...
from orbyte.configs.orbytebot_configs import ORBYTE_BOT_FEEDBACK_REMINDER
...
from orbyte.db.users import add_slack_user_if_not_exists
...
from orbyte.orbytebot.slack.utils import slack_usage_report
...
def schedule_feedback_reminder(
    ...
    if not ORBYTE_BOT_FEEDBACK_REMINDER:
    ...
    future = now + datetime.timedelta(minutes=ORBYTE_BOT_FEEDBACK_REMINDER)
...
def handle_message(
    ...
    Orbyte failing to answer/retrieve docs does count and should be notified
    """
    ...
    if missing_allowlist_entries:
        ...
        if allowed_user_ids is not None and not allowed_user_ids:
            ...
            logger.error(
                ...
                "OrbyteBot will be silent in this channel until lookups recover"
            )
    ...
    if slack_channel_config.channel_config.get("disabled"):
        logger.info("Skipping message: OrbyteBot is disabled for this channel")
        return False
    ...
    if respond_tag_only and not bypass_filters and not is_bot_dm:
        logger.info("Skipping message: OrbyteBot only responds to tags in this channel")
        return False

    # Reuses the resolved allowlist as the ephemeral response-visibility scope.
    send_to: list[str] | None = allowed_user_ids

    # If configured to respond to team members only, then cannot be used with a /OrbyteBot command
    ...
    if send_to and is_slash_command:
        if sender_id:
            respond_in_thread_or_channel(
                ...
                text="The OrbyteBot slash command is not enabled for this channel",
                thread_ts=None,
            )
        ...
        if message_info.email:
                ...
                if seat_result is not None and not seat_result.available:
                    ...
                    respond_in_thread_or_channel(
                        ...
                        text=(
                            ...
                            "Orbyte administrator to add more seats."
                        ),
                    )
                    return False
            ...
            def _slack_seat_enforcer(session: Session, seats_needed: int) -> None:
                ...
                if result is not None and not result.available:
                    raise OrbyteError(
                        OrbyteErrorCode.SEAT_LIMIT_EXCEEDED, result.error_message
                    )
            ...
            try:
                ...
                if e.error_code != OrbyteErrorCode.SEAT_LIMIT_EXCEEDED:
                    raise
                logger.info(
