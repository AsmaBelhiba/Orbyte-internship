"""Async HTTP client for communicating with Orbyte API pods."""

import aiohttp

from orbyte.chat.models import ChatFullResponse
from orbyte.orbytebot.discord.constants import API_REQUEST_TIMEOUT
from orbyte.orbytebot.discord.exceptions import APIConnectionError
from orbyte.orbytebot.discord.exceptions import APIResponseError
from orbyte.orbytebot.discord.exceptions import APITimeoutError
...
from orbyte.utils.variable_functionality import build_api_server_url_for_http_requests

logger = setup_logger()


class OrbyteAPIClient:
    """Async HTTP client for sending chat requests to Orbyte API pods.

    This client manages an aiohttp session for making non-blocking HTTP
    requests to the Orbyte API server. It handles authentication with per-tenant
    API keys and multi-tenant routing.

    Usage:
        client = OrbyteAPIClient()
    ...
    def __init__(
        ...
        # TODO: Ideally, this override is only used when someone is launching an Orbyte service independently
    ...
    async def send_chat_message(
        ...
        """Send a chat message to the Orbyte API server and get a response.
