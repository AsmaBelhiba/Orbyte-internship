...
The Discord bot uses a **single-client, multi-tenant** architecture where one `OrbyteDiscordClient` instance serves multiple tenants (organizations) simultaneously. Tenant isolation is achieved through:
...
│                                                                     │
│  ┌─────────────────────────┐    ┌─────────────────────────────┐    │
│  │   DiscordCacheManager   │    │      OrbyteAPIClient          │    │
│  │                         │    │                             │    │
...
### 1. Cache Manager (`backend/orbyte/orbytebot/discord/cache.py`)
...
### 2. API Client (`backend/orbyte/orbytebot/discord/api_client.py`)

The `OrbyteAPIClient` is a **stateless async HTTP client** that communicates with Orbyte API pods.

#### Key Design: Per-Request API Key Injection

```python
class OrbyteAPIClient:
...
```python
# backend/orbyte/db/discord_bot.py
...
1. **Single Client, Multiple Tenants**: One `OrbyteAPIClient` and one `DiscordCacheManager` instance serves all tenants via dynamic API key injection.
...
| Component | Path |
|-----------|------|
| Cache Manager | `backend/orbyte/orbytebot/discord/cache.py` |
| API Client | `backend/orbyte/orbytebot/discord/api_client.py` |
| Discord Client | `backend/orbyte/orbytebot/discord/client.py` |
| API Key DB Operations | `backend/orbyte/db/discord_bot.py` |
| Cache Manager Tests | `backend/tests/unit/orbyte/orbytebot/discord/test_cache_manager.py` |
| API Client Tests | `backend/tests/unit/orbyte/orbytebot/discord/test_api_client.py` |