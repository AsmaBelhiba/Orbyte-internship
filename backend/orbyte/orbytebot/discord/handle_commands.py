...
from orbyte.configs.app_configs import DISCORD_BOT_INVOKE_CHAR
from orbyte.configs.constants import ORBYTE_DISCORD_URL
from orbyte.db.discord_bot import bulk_create_channel_configs
from orbyte.db.discord_bot import get_guild_config_by_discord_id
...
from orbyte.orbytebot.discord.cache import DiscordCacheManager
from orbyte.orbytebot.discord.constants import REGISTER_COMMAND
from orbyte.orbytebot.discord.constants import SYNC_CHANNELS_COMMAND
from orbyte.orbytebot.discord.exceptions import RegistrationError
from orbyte.orbytebot.discord.exceptions import SyncChannelsError
from orbyte.server.manage.discord_bot.utils import parse_discord_registration_key
...
async def handle_dm(message: discord.Message) -> None:
    ...
    dm_response = (
        ...
        f"[Orbyte Discord]({ORBYTE_DISCORD_URL}) for help!"
    )
...
async def handle_registration_command(
    ...
    try:
        ...
        await message.reply(
            ...
            "This server is now connected to Orbyte. "
            "I'll respond to messages based on your server and channel settings set in Orbyte."
        )
...
async def _register_guild(
    ...
    if existing_tenant is not None:
        ...
        raise RegistrationError(
            "This server is already registered.\n\nOrbyteBot can only connect one Discord server to one Orbyte workspace."
        )
            ...
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                ...
                if not config:
                    raise RegistrationError(
                        ...
                        "Please generate a new one from the Orbyte admin panel."
                    )
                ...
                if config.guild_id is not None:
                    raise RegistrationError(
                        ...
                        "Please generate a new key from the Orbyte admin panel."
                    )
    ...
    try:
        ...
        def _get_guild_config_id() -> int | None:
            with get_session_with_tenant(tenant_id=tenant_id) as db:
                if not message.guild:
                    raise SyncChannelsError(
                        "Server not found. This shouldn't happen. Please contact Orbyte support."
                    )
        ...
        if not guild_config_id:
            raise SyncChannelsError(
                "Server config not found. This shouldn't happen. Please contact Orbyte support."
            )
        ...
        await message.reply(
            ...
            "New channels are disabled by default. Enable them in the Orbyte admin panel."
        )
