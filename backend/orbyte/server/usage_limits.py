...
from orbyte.db.usage import check_usage_limit
from orbyte.db.usage import UsageLimitExceededError
...
from orbyte.server.tenant_usage_limits import TenantUsageLimitKeys
from orbyte.server.tenant_usage_limits import TenantUsageLimitOverrides
...
# Collect all Orbyte-managed default API keys for comparison
_ORBYTE_MANAGED_API_KEYS: set[str] = set()
for key in [
    ...
    if key:
        _ORBYTE_MANAGED_API_KEYS.add(key)


def is_orbyte_managed_api_key(api_key: str | None) -> bool:
    """Check if the given API key is one of Orbyte's managed default keys."""
    return bool(api_key) and api_key in _ORBYTE_MANAGED_API_KEYS
...
def is_tenant_on_trial_fn(tenant_id: str) -> bool:
    ...
    fn: Callable[[str], bool] = fetch_versioned_implementation(
        "orbyte.server.usage_limits", "is_tenant_on_trial"
    )
    return fn(tenant_id)
...
def _get_tenant_override(tenant_id: str, field_name: str) -> int | None:
    ...
    try:
        ...
        get_overrides_fn = fetch_versioned_implementation(
            "orbyte.server.tenant_usage_limits", "get_tenant_usage_limit_overrides"
        )
...
def check_llm_cost_limit_for_provider(
    ...
    """
    Check if the LLM cost limit would be exceeded for a provider using Orbyte-managed keys.

    Only enforces limits when the provider uses Orbyte-managed API keys.
    ...
    # Only enforce limits for Orbyte-managed API keys
    if not is_orbyte_managed_api_key(llm_provider_api_key):
        return
...
def check_usage_and_raise(
    ...
    try:
        ...
        raise OrbyteError(OrbyteErrorCode.RATE_LIMITED, detail)
