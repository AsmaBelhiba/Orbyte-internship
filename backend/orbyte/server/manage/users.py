...
from orbyte.auth.anonymous_user import fetch_anonymous_user_info
from orbyte.auth.email_utils import send_user_email_invite
from orbyte.auth.invited_users import get_invited_users
from orbyte.auth.invited_users import remove_user_from_invited_users
from orbyte.auth.invited_users import write_invited_users
...
from orbyte.auth.users import anonymous_user_enabled
from orbyte.auth.users import current_curator_or_admin_user
from orbyte.auth.users import enforce_seat_limit_locked
from orbyte.auth.users import optional_user
from orbyte.auth.users import scope_exempt
from orbyte.configs.app_configs import AUTH_BACKEND
from orbyte.configs.app_configs import AUTH_TYPE
from orbyte.configs.app_configs import AuthBackend
from orbyte.configs.app_configs import DEV_MODE
from orbyte.configs.app_configs import EMAIL_CONFIGURED
from orbyte.configs.app_configs import ENABLE_EMAIL_INVITES
from orbyte.configs.app_configs import NUM_FREE_TRIAL_USER_INVITES
from orbyte.configs.app_configs import REDIS_AUTH_KEY_PREFIX
from orbyte.configs.app_configs import SESSION_EXPIRE_TIME_SECONDS
...
from orbyte.configs.constants import PUBLIC_API_TAGS
from orbyte.db.api_key import is_api_key_email_address
from orbyte.db.auth import get_live_users_count
from orbyte.db.engine.sql_engine import get_session
from orbyte.db.engine.sql_engine import get_session_with_shared_schema
...
from orbyte.db.tenant_invite_counter import release_trial_invites
from orbyte.db.tenant_invite_counter import reserve_trial_invites
from orbyte.db.user_preferences import activate_user
from orbyte.db.user_preferences import deactivate_user
from orbyte.db.user_preferences import get_all_user_assistant_specific_configs
from orbyte.db.user_preferences import get_latest_access_token_for_user
from orbyte.db.user_preferences import get_memories_for_user
from orbyte.db.user_preferences import update_assistant_preferences
from orbyte.db.user_preferences import update_user_assistant_visibility
from orbyte.db.user_preferences import update_user_auto_scroll
from orbyte.db.user_preferences import update_user_chat_background
from orbyte.db.user_preferences import update_user_default_app_mode
from orbyte.db.user_preferences import update_user_default_model
from orbyte.db.user_preferences import update_user_language
from orbyte.db.user_preferences import update_user_paste_as_tile
from orbyte.db.user_preferences import update_user_personalization
from orbyte.db.user_preferences import update_user_pinned_assistants
from orbyte.db.user_preferences import update_user_role
from orbyte.db.user_preferences import update_user_shortcut_enabled
from orbyte.db.user_preferences import update_user_temperature_override_enabled
from orbyte.db.user_preferences import update_user_theme_preference
from orbyte.db.user_preferences import update_users_craft_enabled
from orbyte.db.users import batch_get_user_groups
from orbyte.db.users import delete_user_from_db
from orbyte.db.users import get_all_accepted_users
from orbyte.db.users import get_all_users
from orbyte.db.users import get_page_of_filtered_users
from orbyte.db.users import get_total_filtered_users_count
from orbyte.db.users import get_user_by_email
from orbyte.db.users import get_user_counts_by_role_and_status
from orbyte.db.users import validate_user_role_update
from orbyte.error_handling.error_codes import OrbyteErrorCode
from orbyte.error_handling.exceptions import OrbyteError
from orbyte.key_value_store.factory import get_kv_store
...
from orbyte.server.features.projects.models import UserFileSnapshot
from orbyte.server.manage.invite_rate_limit import enforce_invite_rate_limit
from orbyte.server.manage.invite_rate_limit import enforce_remove_invited_rate_limit
from orbyte.server.manage.models import AllUsersResponse
from orbyte.server.manage.models import AutoScrollRequest
from orbyte.server.manage.models import BulkInviteResponse
from orbyte.server.manage.models import ChatBackgroundRequest
from orbyte.server.manage.models import DefaultAppModeRequest
from orbyte.server.manage.models import EmailInviteStatus
from orbyte.server.manage.models import LanguageRequest
from orbyte.server.manage.models import MemoryItem
from orbyte.server.manage.models import PersonalizationUpdateRequest
from orbyte.server.manage.models import TenantInfo
from orbyte.server.manage.models import TenantSnapshot
from orbyte.server.manage.models import ThemePreferenceRequest
from orbyte.server.manage.models import UserByEmail
from orbyte.server.manage.models import UserCraftAccessUpdateRequest
from orbyte.server.manage.models import UserInfo
from orbyte.server.manage.models import UserPreferences
from orbyte.server.manage.models import UserRoleResponse
from orbyte.server.manage.models import UserRoleUpdateRequest
from orbyte.server.manage.models import UserSpecificAssistantPreference
from orbyte.server.manage.models import UserSpecificAssistantPreferences
from orbyte.server.models import FullUserSnapshot
from orbyte.server.models import InvitedUserSnapshot
from orbyte.server.models import MinimalUserSnapshot
from orbyte.server.models import UserGroupInfo
from orbyte.server.security.store import get_security_settings
from orbyte.server.usage_limits import is_tenant_on_trial_fn
from orbyte.server.utils import BasicAuthenticationError
from orbyte.utils.audit import actor_from_user
from orbyte.utils.audit import AuditAction
from orbyte.utils.audit import AuditOutcome
from orbyte.utils.audit import emit_audit_event
from orbyte.utils.csv_utils import sanitize_csv_cell
...
from orbyte.utils.variable_functionality import (
...
async def test_upsert_user(
    ...
    user = await fetch_ee_implementation_or_noop(
        "orbyte.server.saml", "upsert_saml_user", None
    )(email=request.email)
...
def list_accepted_users(
    ...
    try:
        from orbyte.db.models import ScimUserMapping
...
def bulk_invite_users(
        ...
        if num_new_invites > 0:
            with get_session_with_shared_schema() as shared_session:
                ...
                if new_total > NUM_FREE_TRIAL_USER_INVITES:
                    ...
                    raise OrbyteError(
                        OrbyteErrorCode.TRIAL_INVITE_LIMIT_EXCEEDED,
    ...
    if MULTI_TENANT:
        try:
            fetch_ee_implementation_or_noop(
                "orbyte.server.tenants.provisioning", "add_users_to_tenant", None
    ...
    if MULTI_TENANT and not DEV_MODE:
        ...
        try:
            ...
            fetch_ee_implementation_or_noop(
                "orbyte.server.tenants.billing", "register_tenant_users", None
...
def deactivate_user_api(
    ...
    if not MULTI_TENANT:
        fetch_ee_implementation_or_noop(
            "orbyte.db.license", "invalidate_license_cache", None
        )()
...
def list_all_users_basic_info(
    ...
    if (
        ...
        raise OrbyteError(
            OrbyteErrorCode.INSUFFICIENT_PERMISSIONS,
...
def verify_user_logged_in(
    ...
    if MULTI_TENANT and user.account_type == AccountType.SERVICE_ACCOUNT:
        ...
        team_name = fetch_ee_implementation_or_noop(
            "orbyte.server.tenants.user_mapping", "get_tenant_id_for_email", None
        )(user.email)

        if MULTI_TENANT:
            if team_name != tenant_id:
                user_count = fetch_ee_implementation_or_noop(
                    "orbyte.server.tenants.user_mapping", "get_tenant_count", None
                )(team_name)
            ...
            tenant_invitation = fetch_ee_implementation_or_noop(
                "orbyte.server.tenants.user_mapping", "get_tenant_invitation", None
            )(user.email)

    super_users_list = cast(
        list[str],
        fetch_versioned_implementation_with_fallback(
            "orbyte.configs.app_configs",
            "SUPER_USERS",
            [],
        ),
    )
