from fastapi import APIRouter
from fastapi import Depends
from sqlalchemy.orm import Session

from orbyte.auth.permissions import require_permission
from orbyte.db.code_interpreter import fetch_code_interpreter_server
from orbyte.db.code_interpreter import update_code_interpreter_server_enabled
from orbyte.db.engine.sql_engine import get_session
from orbyte.db.enums import Permission
from orbyte.db.models import User
from orbyte.server.manage.code_interpreter.models import CodeInterpreterServer
from orbyte.server.manage.code_interpreter.models import CodeInterpreterServerHealth
from orbyte.tools.tool_implementations.python.code_interpreter_client import (
    CodeInterpreterClient,
)

admin_router = APIRouter(prefix="/admin/code-interpreter")


@admin_router.get("/health")
def get_code_interpreter_health(
    _: User = Depends(require_permission(Permission.FULL_ADMIN_PANEL_ACCESS)),
) -> CodeInterpreterServerHealth:
    try:
        client = CodeInterpreterClient()
        result = client.health()
        return CodeInterpreterServerHealth(
            connected=result.connected,
            error=result.error,
        )
    except ValueError as e:
        return CodeInterpreterServerHealth(connected=False, error=str(e))


@admin_router.get("")
def get_code_interpreter(
    _: User = Depends(require_permission(Permission.FULL_ADMIN_PANEL_ACCESS)),
    db_session: Session = Depends(get_session),
) -> CodeInterpreterServer:
    ci_server = fetch_code_interpreter_server(db_session)
    return CodeInterpreterServer(enabled=ci_server.server_enabled)


@admin_router.put("")
def update_code_interpreter(
    update: CodeInterpreterServer,
    _: User = Depends(require_permission(Permission.FULL_ADMIN_PANEL_ACCESS)),
    db_session: Session = Depends(get_session),
) -> None:
    update_code_interpreter_server_enabled(
        db_session=db_session,
        enabled=update.enabled,
    )
