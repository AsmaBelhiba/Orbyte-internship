...
past the limit. When Redis is unavailable (e.g. Orbyte Lite deployments
...
def _run_atomic(redis_client: TenantRedisClient, buckets: list[_Bucket]) -> None:
    """Run the check+increment Lua script. Raise OrbyteError on rejection.
    ...
    invite flow usable on Orbyte Lite deployments (Redis is opt-in there)
...
def enforce_invite_rate_limit(
    ...
    Raises OrbyteError(RATE_LIMITED) without recording if any tier would be
