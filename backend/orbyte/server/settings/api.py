...
from orbyte import __version__ as orbyte_version
from orbyte.auth.permissions import require_permission
from orbyte.auth.users import is_user_admin
from orbyte.configs.app_configs import DEFAULT_USER_FILE_MAX_UPLOAD_SIZE_MB
from orbyte.configs.app_configs import DISABLE_VECTOR_DB
from orbyte.configs.app_configs import MAX_ALLOWED_UPLOAD_SIZE_MB
from orbyte.configs.app_configs import POSTHOG_API_KEY
from orbyte.configs.app_configs import POSTHOG_HOST
...
from orbyte.db.notification import dismiss_all_notifications
from orbyte.db.notification import get_notifications
from orbyte.db.notification import update_notification_last_shown
...
from orbyte.server.features.build.utils import is_craft_available_for_deployment
from orbyte.server.features.build.utils import is_craft_enabled_for_user
from orbyte.server.features.notifications.models import NotificationResponse
from orbyte.server.settings.models import (
    DEFAULT_FILE_TOKEN_COUNT_THRESHOLD_K_NO_VECTOR_DB,
)
from orbyte.server.settings.models import DEFAULT_FILE_TOKEN_COUNT_THRESHOLD_K_VECTOR_DB
from orbyte.server.settings.models import Settings
from orbyte.server.settings.models import Tier
from orbyte.server.settings.models import UserSettings
from orbyte.server.settings.store import load_settings
from orbyte.server.settings.store import store_settings
from orbyte.server.settings.tier_order import tier_at_least
...
def admin_put_settings(
    ...
    if global_version.is_ee_version():
        from ee.orbyte.utils.tier import get_tier
    ...
    if settings.search_ui_enabled != existing.search_ui_enabled and not tier_at_least(
        ...
        raise OrbyteError(
            OrbyteErrorCode.FEATURE_NOT_AVAILABLE,
...
def fetch_settings(
    ...
    # Check if Orbyte Craft is enabled for this user (used for server-side
    # redirects). The deployment gate and already-loaded settings are shared.
    orbyte_craft_available = is_craft_available_for_deployment(user) if user else False
    orbyte_craft_enabled_for_user = (
        is_craft_enabled_for_user(
            user,
            deployment_available=orbyte_craft_available,
    ...
    from orbyte.server.features.build.configs import ENABLE_OPENCODE_DEBUGGING

    return UserSettings(
        **general_settings.model_dump(),
        notifications=settings_notifications,
        needs_reindexing=needs_reindexing,
        orbyte_craft_enabled=orbyte_craft_enabled_for_user,
        orbyte_craft_available=orbyte_craft_available,
        opencode_debugging_enabled=ENABLE_OPENCODE_DEBUGGING,
        vector_db_enabled=not DISABLE_VECTOR_DB,
        hooks_enabled=not MULTI_TENANT,
        version=orbyte_version,
