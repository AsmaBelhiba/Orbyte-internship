...
from orbyte.configs.app_configs import MOCK_CONNECTOR_FILE_PATH
...
from orbyte.configs.constants import ORBYTE_METADATA_FILENAME
...
from orbyte.connectors.factory import validate_ccpair_for_user
from orbyte.connectors.google_utils.google_auth import get_google_oauth_creds
from orbyte.connectors.google_utils.google_kv import build_service_account_creds
...
from orbyte.db.connector import create_connector
from orbyte.db.connector import delete_connector
from orbyte.db.connector import fetch_connector_by_id
from orbyte.db.connector import fetch_connectors
from orbyte.db.connector import fetch_unique_document_sources
from orbyte.db.connector import get_connector_credential_ids
from orbyte.db.connector import mark_ccpair_with_indexing_trigger
from orbyte.db.connector import update_connector
from orbyte.db.connector_credential_pair import add_credential_to_connector
from orbyte.db.connector_credential_pair import (
    fetch_connector_credential_pair_for_connector,
)
from orbyte.db.connector_credential_pair import get_cc_pair_groups_for_ids
from orbyte.db.connector_credential_pair import get_connector_credential_pair
from orbyte.db.connector_credential_pair import get_connector_credential_pairs_for_user
from orbyte.db.connector_credential_pair import (
    get_connector_credential_pairs_for_user_parallel,
)
from orbyte.db.connector_credential_pair import verify_user_has_access_to_cc_pair
from orbyte.db.credentials import create_credential
from orbyte.db.credentials import fetch_credential_by_id_for_user
from orbyte.db.deletion_attempt import check_deletion_attempt_is_allowed
from orbyte.db.document import get_document_counts_for_all_cc_pairs
...
from orbyte.db.enums import ProcessingMode
from orbyte.db.federated import fetch_all_federated_connectors_parallel
...
from orbyte.db.index_attempt import get_latest_index_attempts_parallel
...
from orbyte.server.documents.models import AuthStatus
from orbyte.server.documents.models import AuthUrl
...
from orbyte.server.documents.models import ConnectorFileInfo
from orbyte.server.documents.models import ConnectorFilesResponse
from orbyte.server.documents.models import ConnectorIndexingStatusLite
from orbyte.server.documents.models import ConnectorIndexingStatusLiteResponse
from orbyte.server.documents.models import ConnectorRequestSubmission
from orbyte.server.documents.models import ConnectorSnapshot
from orbyte.server.documents.models import ConnectorStatus
from orbyte.server.documents.models import ConnectorUpdateRequest
from orbyte.server.documents.models import CredentialBase
from orbyte.server.documents.models import CredentialSnapshot
from orbyte.server.documents.models import DocsCountOperator
from orbyte.server.documents.models import FailedConnectorIndexingStatus
from orbyte.server.documents.models import FileUploadResponse
from orbyte.server.documents.models import GDriveCallback
from orbyte.server.documents.models import GmailCallback
from orbyte.server.documents.models import GoogleServiceAccountCredentialRequest
from orbyte.server.documents.models import IndexedSourcesResponse
from orbyte.server.documents.models import IndexingStatusRequest
from orbyte.server.documents.models import ObjectCreationIdResponse
from orbyte.server.documents.models import RunConnectorRequest
from orbyte.server.documents.models import SourceSummary
from orbyte.server.federated.models import FederatedConnectorStatus
from orbyte.server.models import StatusResponse
from orbyte.server.security.store import get_security_settings
from orbyte.server.utils_vector_db import require_vector_db
...
from orbyte.utils.threadpool_concurrency import CallableProtocol
from orbyte.utils.threadpool_concurrency import run_functions_tuples_in_parallel
...
def save_zip_metadata_to_file_store(
    ...
    """
    Extract .orbyte_metadata.json from zip and save to file store.
    Returns the file_id or None if no metadata file exists.
    """
    try:
        metadata_file_info = zf.getinfo(ORBYTE_METADATA_FILENAME)
        with zf.open(metadata_file_info, "r") as metadata_file:
            ...
            try:
                ...
                logger.warning("Unable to load %s: %s", ORBYTE_METADATA_FILENAME, e)
                raise HTTPException(
                    status_code=400,
                    detail=f"Unable to load {ORBYTE_METADATA_FILENAME}: {e}",
                )

            # Save to file store
            file_id = file_store.save_file(
                content=BytesIO(metadata_bytes),
                display_name=ORBYTE_METADATA_FILENAME,
                file_origin=FileOrigin.CONNECTOR_METADATA,
                file_type="application/json",
            )
            return file_id
    except KeyError:
        logger.info("No %s file", ORBYTE_METADATA_FILENAME)
        return None
    ...
    try:
        ...
        if new_file_paths:
            ...
            client_app.send_task(
                OrbyteCeleryTask.CHECK_FOR_INDEXING,
                kwargs={"tenant_id": tenant_id},
                priority=OrbyteCeleryPriority.HIGH,
            )
            logger.info(
...
def create_connector_from_model(
    ...
    try:
        ...
        fetch_ee_implementation_or_noop(
            "orbyte.db.user_group", "validate_object_creation_for_user", None
        )(
            db_session=db_session,
            user=user,
...
def submit_connector_request(
    ...
    if EMAIL_CONFIGURED:
        try:
            subject = "Orbyte Craft Connector Request"
