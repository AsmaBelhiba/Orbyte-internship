...
from orbyte.auth.captcha import CAPTCHA_COOKIE_NAME
...
from orbyte.auth.captcha import issue_captcha_cookie_value
from orbyte.auth.captcha import validate_captcha_cookie_value
from orbyte.auth.captcha import verify_captcha_token
from orbyte.configs.app_configs import CAPTCHA_COOKIE_TTL_SECONDS
from orbyte.configs.app_configs import HEALTH_CHECK_BYPASS_TOKEN
from orbyte.configs.constants import PUBLIC_API_TAGS
from orbyte.error_handling.error_codes import OrbyteErrorCode
from orbyte.error_handling.exceptions import orbyte_error_to_json_response
...
async def verify_oauth_captcha(
    ...
    try:
        ...
        raise OrbyteError(OrbyteErrorCode.UNAUTHORIZED, str(exc))
    ...
    async def dispatch(
        ...
        if is_guarded_callback:
            ...
            if not validate_captcha_cookie_value(cookie_value):
                return orbyte_error_to_json_response(
                    OrbyteError(
        ...
        if (
            ...
            if _health_check_bypass_ok(request):
                ...
                try:
                    ...
                    return orbyte_error_to_json_response(
                        OrbyteError(OrbyteErrorCode.UNAUTHORIZED, str(exc))
                    )
