...
from orbyte.file_processing.extract_file_text import xlsx_sheet_extraction
from orbyte.file_processing.file_types import OrbyteMimeTypes
from orbyte.file_processing.file_types import SPREADSHEET_MIME_TYPES
...
def mime_type_to_chat_file_type(mime_type: str | None) -> ChatFileType:
    ...
    if normalized_mime_type in OrbyteMimeTypes.IMAGE_MIME_TYPES:
        return ChatFileType.IMAGE

    if normalized_mime_type in OrbyteMimeTypes.TABULAR_MIME_TYPES:
        return ChatFileType.TABULAR

    if normalized_mime_type in OrbyteMimeTypes.DOCUMENT_MIME_TYPES:
