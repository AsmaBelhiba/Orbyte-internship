...
from orbyte.auth.users import current_user_from_websocket
from orbyte.auth.users import current_user_with_expired_token
...
PUBLIC_ENDPOINT_SPECS = [
    ...
    ("/auth/type", {"GET"}),
    # just gets the version of Orbyte (e.g. 0.3.11)
    ("/version", {"GET"}),
    # Gets stable and beta versions for Orbyte docker images
    ("/versions", {"GET"}),
    ...
    ("/users/{id}", {"PATCH"}),
    ("/users/{id}", {"DELETE"}),
    # group join links — the token itself is the authorization, used by
    # not-yet-registered users to look up/redeem a link
    ("/join-link/{token}", {"GET"}),
    ("/join-link/{token}/redeem", {"POST"}),
...
def check_router_auth(
    ...
    control_plane_dep = fetch_ee_implementation_or_noop(
        "orbyte.server.tenants.access", "control_plane_dep"
    )
    current_cloud_superuser = fetch_ee_implementation_or_noop(
        "orbyte.auth.users", "current_cloud_superuser"
    )
    verify_scim_token = fetch_ee_implementation_or_noop(
        "orbyte.server.scim.auth", "verify_scim_token"
    )
