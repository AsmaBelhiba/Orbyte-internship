...
from orbyte.server.features.build.configs import CRAFT_PAID_USER_RATE_LIMIT
from orbyte.server.features.build.db.rate_limit import count_user_messages_in_window
from orbyte.server.features.build.db.rate_limit import count_user_messages_total
from orbyte.server.features.build.db.rate_limit import get_oldest_message_timestamp
from orbyte.server.features.build.subscription_check import is_user_subscribed
from orbyte.server.features.build.utils import CRAFT_HAS_USAGE_LIMITS
...
def _should_skip_rate_limiting(user: User) -> bool:
    ...
    PostHog flag is off (e.g. tenant_dev). Only reached on Orbyte Cloud, where
...
def get_user_rate_limit_status(
    ...
    Rate limits apply on Orbyte Cloud only:
    ...
    """
    # Rate limits apply only on Orbyte Cloud (multi-tenant); all other deployments
    # are unlimited.
