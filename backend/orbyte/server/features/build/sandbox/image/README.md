...
This directory contains the Dockerfile and resources for building the Orbyte Craft sandbox container image.
...
Built-in skill sources are **not** here — they live in `backend/orbyte/skills/builtin/`
...
```bash
cd backend/orbyte/server/features/build/sandbox/image
...
- **orbyte-cli**: `/usr/local/bin/orbyte-cli` — Orbyte CLI for search
- **Snapshot sidecar daemon**: Packages and restores session files; durable storage is handled by the api_server through the Orbyte FileStore
...
```bash
kubectl get pod <pod-name> -n orbyte-sandboxes -o jsonpath='{.spec.containers[?(@.name=="sandbox")].image}'
```
