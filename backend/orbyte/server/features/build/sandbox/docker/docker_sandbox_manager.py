...
- a fixed env allowlist (``ORBYTE_PAT``, ``ORBYTE_SERVER_URL``,
...
1. Public internet over HTTPS (the bridge has default internet egress; block at
   ...
   IMDS).
2. The Orbyte API via ``ORBYTE_SERVER_URL`` — which must be the *public* HTTPS URL
   the agent reaches just like any other orbyte-cli client.
...
from orbyte.server.features.build.configs import ATTACHMENTS_DIRECTORY
from orbyte.server.features.build.configs import OPENCODE_DISABLED_TOOLS
from orbyte.server.features.build.configs import OPENCODE_SERVE_PORT
from orbyte.server.features.build.configs import OPENCODE_SERVER_PASSWORD
from orbyte.server.features.build.configs import SANDBOX_API_SERVER_URL
from orbyte.server.features.build.configs import SANDBOX_CONTAINER_IMAGE
from orbyte.server.features.build.configs import SANDBOX_DOCKER_CPU_LIMIT
from orbyte.server.features.build.configs import SANDBOX_DOCKER_MEMORY_LIMIT
from orbyte.server.features.build.configs import SANDBOX_DOCKER_NETWORK
from orbyte.server.features.build.configs import SANDBOX_DOCKER_SOCKET
from orbyte.server.features.build.configs import SANDBOX_DOCKER_VOLUME_PREFIX
from orbyte.server.features.build.configs import SANDBOX_PROXY_CA_VOLUME_NAME
from orbyte.server.features.build.configs import SANDBOX_PROXY_HOST
from orbyte.server.features.build.configs import SANDBOX_PROXY_INJECTED_PLACEHOLDER
from orbyte.server.features.build.configs import SANDBOX_PROXY_PORT
from orbyte.server.features.build.sandbox.base import BUN_CACHE_DIR
from orbyte.server.features.build.sandbox.base import BUN_IMAGE_CACHE_DIR
from orbyte.server.features.build.sandbox.base import SandboxManager
from orbyte.server.features.build.sandbox.docker.dev_mode_serve import (
    opencode_serve_port_bindings,
)
from orbyte.server.features.build.sandbox.docker.dev_mode_serve import (
    published_opencode_serve_base_url,
)
from orbyte.server.features.build.sandbox.docker.internal.exec_helpers import ExecError
from orbyte.server.features.build.sandbox.docker.internal.exec_helpers import ExecResult
from orbyte.server.features.build.sandbox.docker.internal.exec_helpers import (
    run_in_container,
)
from orbyte.server.features.build.sandbox.docker.internal.exec_helpers import (
...
from orbyte.server.features.build.sandbox.labels import LABEL_K8S_MANAGED_BY
from orbyte.server.features.build.sandbox.labels import LABEL_K8S_MANAGED_BY_ORBYTE
from orbyte.server.features.build.sandbox.labels import LABEL_SANDBOX_ID
from orbyte.server.features.build.sandbox.labels import LABEL_TENANT_ID
...
from orbyte.server.features.build.sandbox.snapshot_manager import SnapshotManager
...
from orbyte.server.features.build.sandbox.util.opencode_config import (
...
LABEL_COMPONENT = "orbyte.app/component"
LABEL_COMPONENT_VALUE = "craft-sandbox"
LABEL_USER_ID = "orbyte.app/user-id"
...
def _looks_like_internal_compose_host(url: str) -> bool:
    ...
    the craft bridge network — so the URL must be the public Orbyte URL.
    """
    if not url:
        return False
    lowered = url.lower()
...
def build_sandbox_labels(
    ...
    Desktop groups sandbox containers under the same "orbyte" stack header as
    ...
    labels: dict[str, str] = {
        ...
        LABEL_K8S_MANAGED_BY: LABEL_K8S_MANAGED_BY_ORBYTE,
    }
...
def _proxy_env_vars(
    *,
    ...
    (``orbyte.sandboxProxyEnv``); here they're layered on the docker env dict.
...
def build_container_create_kwargs(
    *,
    ...
    - **Env is a fixed allowlist**: ORBYTE_PAT, ORBYTE_SERVER_URL, plus
    ...
    - **Single network**: joins only the caller-supplied ``network`` (the
      dedicated ``orbyte_craft_sandbox`` bridge). Does NOT join compose's default
    ...
    - Env layered with ``HTTPS_PROXY`` / SDK CA vars + the ``firewall-init.sh``
      ...
      are layered on top.
    - ``ORBYTE_PAT`` and the opencode ``api_key`` are replaced with
      ``SANDBOX_PROXY_INJECTED_PLACEHOLDER``; the proxy reads the real values
      from Postgres and injects them on the wire (OrbytePatResolver,
    ...
    ``ORBYTE_SERVER_URL`` must be the *public* Orbyte URL (the one orbyte-cli inside
    ...
    if _looks_like_internal_compose_host(api_server_url):
        logger.warning(
            ...
            "so this URL must resolve publicly (e.g. https://orbyte.your-org.com). Internal DNS will "
    ...
    env: dict[str, str] = {
        ...
        "ORBYTE_SERVER_URL": api_server_url,
...
class DockerSandboxManager(SandboxManager):
    ...
    def provision(
        ...
        *,
        ...
        if not orbyte_pat:
            raise ValueError("orbyte_pat is required for Docker sandbox provisioning.")
        ...
        if container is None:
            ...
            # (OrbytePatResolver, LLMProviderKeyResolver).
            container_orbyte_pat = (
                SANDBOX_PROXY_INJECTED_PLACEHOLDER if SANDBOX_PROXY_HOST else orbyte_pat
            )
            ...
            container, created_fresh = self._create_sandbox_container(
                ...
                user_id=user_id,
                tenant_id=tenant_id,
                orbyte_pat=container_orbyte_pat,
