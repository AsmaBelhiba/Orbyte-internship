# Orbyte Sandbox System

This directory contains the implementation of Orbyte's sandbox system for running OpenCode agents in isolated environments.
...
1. **Kubernetes Mode** (`SANDBOX_BACKEND=kubernetes`) — default
   ...
   - Used by Orbyte's Helm chart / cloud deployment
...
2. **Docker Mode** (`SANDBOX_BACKEND=docker`)
   ...
   - Sandboxes join only the dedicated `orbyte_craft_sandbox` bridge — `postgres` / `redis` / `minio` / model servers are not reachable by compose DNS
...
| `Service` + DNS for Next.js preview   | Container IP on `orbyte_craft_sandbox` bridge, proxied        |
...
`api_server` and `background` mount the host Docker socket so they can drive sandbox containers. Anything that can talk to that socket is effectively root on the host — only enable Craft on hosts you fully control. Sandbox containers themselves run unprivileged: `--security-opt no-new-privileges`, `--cap-drop ALL`, `user=1000:1000`, no Docker socket, and a fixed env allowlist (`ORBYTE_PAT` + `ORBYTE_SERVER_URL`).

`SANDBOX_API_SERVER_URL` must be the **public** HTTPS URL that the agent reaches Orbyte through (same way any orbyte-cli client would). Compose hostnames like `http://api_server:8080` do not resolve from inside the sandbox bridge.
...
Deploy the normal Orbyte application images with Craft enabled. The sandbox image
...
- Built-in skill sources live in `backend/orbyte/skills/builtin/` (pushed at session setup, not baked in)
...
SANDBOX_NAMESPACE=orbyte-sandboxes          # Default: orbyte-sandboxes

# Helm defaults the sandbox image to onyxdotapp/sandbox:${global.version}.
# SANDBOX_CONTAINER_IMAGE is an internal override.

# Snapshots use the normal Orbyte FileStore configuration
FILE_STORE_BACKEND=s3|gcs|postgres
S3_FILE_STORE_BUCKET_NAME=orbyte-file-store # when FILE_STORE_BACKEND=s3
...
# Public URL the sandbox agent uses to reach Orbyte (HTTPS, externally resolvable —
# compose hostnames like http://api_server:8080 will not resolve from inside the
# sandbox bridge).
SANDBOX_API_SERVER_URL=https://orbyte.your-org.example
...
# `docker network create orbyte_craft_sandbox` manually). Sandboxes join *only*
# this network — compose services are not reachable by DNS from inside.
SANDBOX_DOCKER_NETWORK=orbyte_craft_sandbox       # Default: orbyte_craft_sandbox

# Prefix for per-sandbox named volumes (mounted at /workspace/sessions).
SANDBOX_DOCKER_VOLUME_PREFIX=orbyte-sandbox       # Default: orbyte-sandbox
...
- Confirm the dedicated bridge exists: `docker network inspect orbyte_craft_sandbox` (created by `install.sh --include-craft`, or run `docker network create orbyte_craft_sandbox` manually)
...
- Check pod logs: `kubectl logs -n orbyte-sandboxes sandbox-{sandbox-id}`
- Check sidecar logs: `kubectl logs -n orbyte-sandboxes sandbox-{sandbox-id} -c sidecar`
...
- Check container logs: `kubectl logs -n orbyte-sandboxes sandbox-{sandbox-id}`
- Verify `bun install` succeeded (check entrypoint.sh logs)
- Check that web template was copied: `kubectl exec -n orbyte-sandboxes sandbox-{sandbox-id} -- ls /workspace/outputs/web`
...
- **Docker containers** run with `--security-opt no-new-privileges`, `--cap-drop ALL`, `user=1000:1000`, no Docker socket, and a fixed env allowlist (`ORBYTE_PAT` + `ORBYTE_SERVER_URL`)
- **Docker network isolation** is enforced by joining only the dedicated `orbyte_craft_sandbox` bridge — compose's default network (postgres/redis/minio/model servers) is unreachable by DNS from inside a sandbox
...
- Snapshots are stored through the normal Orbyte `FileStore` and isolated by tenant-scoped snapshot paths
...
The lightweight Next.js template (`backend/orbyte/server/features/build/sandbox/image/templates/outputs/web/`) includes:
...
This template provides a modern development environment without the complexity of the full Orbyte application, allowing agents to build custom UIs quickly.
