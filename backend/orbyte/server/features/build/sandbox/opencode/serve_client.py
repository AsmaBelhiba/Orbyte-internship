"""HTTP client for ``opencode serve`` — the only transport Orbyte Craft
...
Sandbox-event types come from :mod:`orbyte.server.features.build.sandbox.event_schema`.
"""
...
from orbyte.server.features.build.configs import (
    OPENCODE_PROMPT_INACTIVITY_TIMEOUT_SECONDS,
)
from orbyte.server.features.build.configs import OPENCODE_SERVE_CONNECT_TIMEOUT
from orbyte.server.features.build.configs import OPENCODE_SERVE_EVENT_READ_TIMEOUT
from orbyte.server.features.build.configs import OPENCODE_SERVE_REQUEST_TIMEOUT
from orbyte.server.features.build.configs import OPENCODE_SERVER_USERNAME
from orbyte.server.features.build.configs import SANDBOX_APPROVAL_WAIT_TIMEOUT_SECONDS
from orbyte.server.features.build.configs import SSE_KEEPALIVE_INTERVAL
from orbyte.server.features.build.packets import CompactionPacket
from orbyte.server.features.build.packets import ContextUsagePacket
from orbyte.server.features.build.packets import SubagentStartedPacket
from orbyte.server.features.build.sandbox.event_schema import AgentMessageChunk
from orbyte.server.features.build.sandbox.event_schema import AgentThoughtChunk
from orbyte.server.features.build.sandbox.event_schema import Error
from orbyte.server.features.build.sandbox.event_schema import PromptResponse
from orbyte.server.features.build.sandbox.event_schema import ToolCallProgress
from orbyte.server.features.build.sandbox.event_schema import ToolCallStart
from orbyte.server.features.build.sandbox.event_schema import TURN_ERROR_CODE_SESSION
...
from orbyte.server.features.build.sandbox.opencode.event_bus import _Subscription
...
# Orbyte schema: "pending" | "in_progress" | "completed" | "failed"
...
# array on tool parts — only ``state.{input,output,metadata}``. Orbyte's
...
class OpencodeServeClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._client_info = client_info or {
            "name": "orbyte-opencode-serve-client",
            "version": "1.0.0",
        }
    ...
    def delete_session(self, opencode_session_id: str, *, directory: str) -> bool:
        ...
        Product deletion is owned by Orbyte's BuildSession row. This cleanup is
        opportunistic: failure should not block deleting the Orbyte session.
        """
        try:
            r = self._request(
                "DELETE",
