...
from orbyte.server.features.build.configs import MAX_UPLOAD_FILE_SIZE_BYTES
...
# PostHog feature flag key for enabling Orbyte Craft (cloud rollout control)
# Flag logic: True = enabled, False/null/not found = disabled
ORBYTE_CRAFT_ENABLED_FLAG = "orbyte-craft-enabled"
...
def is_craft_available_for_deployment(user: User) -> bool:
    """
    Check whether Orbyte Craft (Build Mode) is available at the deployment level.

    Flag logic for "orbyte-craft-enabled":
    - Flag = True → enabled (Orbyte Craft is available)
    - Flag = False → disabled (Orbyte Craft is not available)
    - Flag = null/not found → disabled (Orbyte Craft is not available)
    ...
    is_enabled = feature_flag_provider.feature_enabled_for_user_tenant(
        ORBYTE_CRAFT_ENABLED_FLAG,
        user,
        get_current_tenant_id(),
    )

    if is_enabled:
        logger.debug("Orbyte Craft enabled via PostHog feature flag")
        return True
    else:
        logger.debug("Orbyte Craft disabled via PostHog feature flag")
        return False
...
def is_craft_enabled_for_user(
    ...
    """
    Check if Orbyte Craft (Build Mode) is enabled for the user: the deployment
...
def ensure_build_mode_intro_notification(user: User, db_session: Session) -> None:
    ...
    """
    # PostHog feature flag check - only show notification if Orbyte Craft is enabled
    ...
    create_notification(
        ...
        title="Introducing Orbyte Craft",
        description="Unleash Orbyte to create dashboards, slides, documents, and more with your connected data.",
