...
Thin HTTP layer over ``orbyte.db.scheduled_task``. Every handler:

1. Validates request payload via Pydantic.
2. Calls a DB op (which enforces ownership + raises ``OrbyteError``).
...
``backend/orbyte/server/features/build/api.py``), which provides the
``require_orbyte_craft_enabled`` gate.
"""
...
from orbyte.background.celery.versioned_apps.client import app as celery_app
...
from orbyte.db.models import ScheduledTaskRun
from orbyte.db.models import User
from orbyte.db.scheduled_task import create_scheduled_task
from orbyte.db.scheduled_task import get_scheduled_task
from orbyte.db.scheduled_task import insert_run
from orbyte.db.scheduled_task import list_runs_for_task
from orbyte.db.scheduled_task import list_scheduled_tasks_for_user
from orbyte.db.scheduled_task import soft_delete_scheduled_task
...
from orbyte.server.features.build.scheduled_tasks.schedule import compile_to_cron
from orbyte.server.features.build.scheduled_tasks.schedule import EDITOR_PAYLOAD_MODELS
from orbyte.server.features.build.scheduled_tasks.schedule import EditorMode
from orbyte.server.features.build.scheduled_tasks.schedule import EditorPayload
from orbyte.server.features.build.scheduled_tasks.schedule import human_readable
from orbyte.server.features.build.scheduled_tasks.schedule import next_n_fires
...
def _enqueue_executor(run_id: UUID) -> None:
    ...
    celery_app.send_task(
        OrbyteCeleryTask.SCHEDULED_TASKS_RUN,
        kwargs={"run_id": str(run_id), "tenant_id": tenant_id},
        queue=OrbyteCeleryQueues.SCHEDULED_TASKS,
...
def list_task_runs(
    ...
    Raises ``OrbyteError(NOT_FOUND)`` when the task is missing, soft-deleted,
