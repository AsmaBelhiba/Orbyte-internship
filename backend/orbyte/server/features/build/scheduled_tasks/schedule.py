...
them inside ``backend/orbyte/db/scheduled_task.py`` for persisted reads/writes.
"""
...
def _validate_cron(cron: str) -> None:
    """Raise ``OrbyteError(INVALID_INPUT)`` if ``cron`` is not a valid 5-field expression.
    ...
    cron = cron.strip()
    if not cron:
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, "Cron expression is empty")
    if len(cron.split()) != 5:
        raise OrbyteError(
    ...
    if not croniter.is_valid(cron):
        raise OrbyteError(
            OrbyteErrorCode.INVALID_INPUT, f"Invalid cron expression: {cron!r}"
        )
...
def compute_next_run_at(cron: str, after: datetime) -> datetime:
    ...
    Raises:
        OrbyteError(INVALID_INPUT): if ``cron`` is invalid, or if no future fire
...
def next_n_fires(
    ...
    Raises ``OrbyteError(INVALID_INPUT)`` if ``n`` is non-positive or the
    schedule is invalid.
    """
    if n <= 0:
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, "n must be positive")
    _validate_cron(cron)
