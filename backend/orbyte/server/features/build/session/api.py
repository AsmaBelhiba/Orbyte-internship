...
from orbyte.db.scheduled_task import get_scheduled_run_context
...
from orbyte.server.features.build.session.models import ArtifactResponse
from orbyte.server.features.build.session.models import DetailedSessionResponse
from orbyte.server.features.build.session.models import OpencodeHistorySnapshotResponse
from orbyte.server.features.build.session.models import PptxPreviewResponse
from orbyte.server.features.build.session.models import PreProvisionedCheckResponse
from orbyte.server.features.build.session.models import SandboxStatusResponse
from orbyte.server.features.build.session.models import SessionCreateRequest
from orbyte.server.features.build.session.models import SessionListResponse
from orbyte.server.features.build.session.models import SessionNameGenerateResponse
from orbyte.server.features.build.session.models import SessionResponse
from orbyte.server.features.build.session.models import SessionUpdateRequest
from orbyte.server.features.build.session.models import SetSessionSharingRequest
from orbyte.server.features.build.session.models import SetSessionSharingResponse
from orbyte.server.features.build.session.models import SnapshotResponse
from orbyte.server.features.build.session.models import WebappInfo
...
from orbyte.server.features.build.session.sandbox_lifecycle import provision_sandbox
from orbyte.server.features.build.session.sandbox_lifecycle import (
...
from orbyte.server.features.build.session.streaming import SSE_KEEPALIVE
from orbyte.server.features.build.utils import sanitize_filename
from orbyte.server.features.build.utils import validate_file
...
def _owned_session_sandbox(
    ...
    if get_build_session(session_id, user.id, db_session) is None:
        raise OrbyteError(OrbyteErrorCode.SESSION_NOT_FOUND, "Session not found")
    sandbox = get_sandbox_by_user_id(db_session, user.id)
    if sandbox is None:
        raise OrbyteError(OrbyteErrorCode.SESSION_NOT_FOUND, "Session not found")
    return sandbox
...
def create_session_snapshot(
    ...
    try:
        ...
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, str(e)) from e
    except RuntimeError as e:
        raise OrbyteError(OrbyteErrorCode.SERVICE_UNAVAILABLE, str(e)) from e
    if result is None:
        return Response(status_code=204)
