...
from orbyte.external_apps.providers.registry import get_provider_or_raise
from orbyte.external_apps.token_utils import stamp_expires_at
from orbyte.redis.redis_pool import get_redis_client
from orbyte.server.features.build.external_apps.models import OAuthCallbackRequest
from orbyte.server.features.build.external_apps.models import OAuthCallbackResponse
from orbyte.server.features.build.external_apps.models import OAuthStartResponse
...
def handle_external_app_oauth_callback(
    ...
    try:
        ...
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, "Malformed OAuth state.")
    ...
    if record.user_id != str(user.id):
        raise OrbyteError(
            OrbyteErrorCode.UNAUTHENTICATED,
