...
from orbyte.db.external_app import delete_external_app
from orbyte.db.external_app import get_external_app_by_id
from orbyte.db.external_app import get_external_apps
from orbyte.db.external_app import get_policies
from orbyte.db.external_app import get_user_credentials_by_app_id
from orbyte.db.external_app import required_user_credential_keys
from orbyte.db.external_app import update_external_app
from orbyte.db.external_app import upsert_external_app_user_credential
from orbyte.db.external_app import validate_auth_template
...
from orbyte.db.utils import none_as_unset
from orbyte.db.utils import UNSET
from orbyte.error_handling.error_codes import OrbyteErrorCode
from orbyte.error_handling.exceptions import OrbyteError
from orbyte.external_apps.models import BuiltInExternalAppDescriptor
from orbyte.external_apps.providers.base import OAuthExternalAppProvider
from orbyte.external_apps.providers.registry import action_policy_views
from orbyte.external_apps.providers.registry import fetch_available_built_in_apps
from orbyte.external_apps.providers.registry import get_orbyte_managed_provider
from orbyte.external_apps.providers.registry import get_provider_for_app
from orbyte.external_apps.providers.registry import resolve_action_overrides
...
from orbyte.server.features.build.external_apps.models import ConnectAppDecisionRequest
...
from orbyte.skills.ingest import ingested_skill_bundle
from orbyte.skills.push import push_skill_to_affected_sandboxes
from orbyte.skills.push import push_skills_for_users
from orbyte.utils.encryption import mask_string
from orbyte.utils.pydantic_util import parse_json_form_field
...
def _to_admin_response(app: ExternalApp) -> ExternalAppAdminResponse:
    ...
    managed = MULTI_TENANT and get_orbyte_managed_provider(app.app_type) is not None
    return ExternalAppAdminResponse(
        id=app.id,
        name=app.skill.name,
        description=app.skill.description,
        app_type=app.app_type,
        # Managed built-ins: hide Orbyte-owned config/creds. Else mask secrets — the
...
def create_built_in_external_app(
    ...
    Orbyte-managed built-ins are Orbyte-provisioned and can't be created here.
    """
    ...
    if MULTI_TENANT and get_orbyte_managed_provider(request.app_type) is not None:
        raise OrbyteError(
            OrbyteErrorCode.INVALID_INPUT,
            "Built-in apps are provided by Orbyte; use PATCH /admin/apps/{id} to "
...
def update_external_app_admin(
    ...
    untouched. For Orbyte-managed built-ins (cloud) the gateway-config fields
    are Orbyte-owned and ignored — only ``enabled`` + ``action_policies`` apply.
    ...
    app, _old = update_external_app(
        ...
        # Gateway config is Orbyte-owned for managed built-ins; leave it untouched.
...
def create_custom_external_app(
    ...
    if not name.strip():
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, "name is required.")
...
def delete_external_app_admin(
    ...
    app = _get_app_or_404(db_session, external_app_id)
    if MULTI_TENANT and get_orbyte_managed_provider(app.app_type) is not None:
        raise OrbyteError(
            OrbyteErrorCode.INVALID_INPUT,
            "Built-in apps are provided by Orbyte and cannot be deleted.",
        )
