...
from orbyte.server.features.build.connect_app import ConnectAppDecision
...
class UpdateExternalAppRequest(BaseModel):
    ...
    This is the single update path for built-in apps. For Orbyte-managed built-ins
    (cloud) the gateway-config fields (``upstream_url_patterns``,
    ``auth_template``, ``organization_credentials``) are Orbyte-owned and ignored —
...
class ExternalAppAdminResponse(BaseModel):
    ...
    # Orbyte-managed built-in (cloud): creds/config Orbyte-owned and blanked above;
    # admin may only enable/disable + set policies. UI hides the rest.
    is_orbyte_managed: bool = False
