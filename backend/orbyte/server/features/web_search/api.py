...
from orbyte.db.web_search import fetch_active_web_content_provider
from orbyte.db.web_search import fetch_active_web_search_provider
from orbyte.error_handling.error_codes import OrbyteErrorCode
from orbyte.error_handling.exceptions import OrbyteError
from orbyte.server.features.web_search.models import OpenUrlsToolRequest
from orbyte.server.features.web_search.models import OpenUrlsToolResponse
from orbyte.server.features.web_search.models import WebSearchToolRequest
from orbyte.server.features.web_search.models import WebSearchToolResponse
from orbyte.server.features.web_search.models import WebSearchWithContentResponse
...
from orbyte.tools.models import LlmOpenUrlResult
from orbyte.tools.models import LlmWebSearchResult
from orbyte.tools.tool_implementations.open_url.models import WebContentProvider
...
from orbyte.tools.tool_implementations.web_search.utils import (
...
def _get_active_search_provider(
    ...
    try:
        ...
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, str(exc)) from exc
...
def _get_active_content_provider(
    ...
    if provider_model is None:
        ...
        # NOTE: the OrbyteWebCrawler is not stored in the content provider table,
        # so we need to return it directly.

        return None, OrbyteWebCrawler(
...
def _open_urls(
    ...
    # SSRF protection is handled inside the content provider (OrbyteWebCrawler)
    ...
    provider_type = (
        ...
        if provider_view
        else WebContentProviderType.ORBYTE_WEB_CRAWLER
    )
