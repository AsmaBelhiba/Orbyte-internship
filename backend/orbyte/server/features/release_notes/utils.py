...
from orbyte.cache.factory import get_shared_cache_backend
...
from orbyte.server.features.release_notes.constants import REDIS_KEY_ETAG
from orbyte.server.features.release_notes.constants import REDIS_KEY_FETCHED_AT
from orbyte.server.features.release_notes.models import ReleaseNoteEntry
        ...
        if is_valid_version(version):
            all_entries.append(
                ReleaseNoteEntry(
                    version=version,
                    date=date,
                    title=f"Orbyte {version} is available!",
                )
            )
...
def ensure_release_notes_fresh_and_notify(db_session: Session) -> None:
    ...
    cache = get_shared_cache_backend()
    lock = cache.lock(
        OrbyteRedisLocks.RELEASE_NOTES_FETCH_LOCK,
        timeout=90,
    )
