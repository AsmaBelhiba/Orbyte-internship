...
from orbyte.configs import app_configs as _cfg
...
from orbyte.db.security_settings import load_overrides as _db_load_overrides
from orbyte.db.security_settings import upsert_overrides as _db_upsert_overrides
from orbyte.error_handling.error_codes import OrbyteErrorCode
from orbyte.error_handling.exceptions import OrbyteError
from orbyte.server.security.models import OPERATOR_LOCKED_FIELDS
...
def apply_patch(
    ...
    Raises ``OrbyteError(CONFLICT)`` if a competing writer holds the lock past
    the wait window, and ``OrbyteError(INVALID_INPUT)`` if the merged effective
    state would violate a model invariant.
    """
    cache = get_cache_backend()
    lock = cache.lock(
        OrbyteRedisLocks.SECURITY_SETTINGS, timeout=_WRITE_LOCK_LEASE_SECONDS
    )
