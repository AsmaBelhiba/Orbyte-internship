...
from orbyte.skills.built_in import SLUG_REGEX
from orbyte.skills.models import CustomSkillBundleContents
from orbyte.skills.models import SkillBundleFile
...
def check_slug(slug: str) -> None:
    if not SLUG_REGEX.match(slug):
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, f"invalid slug '{slug}'")
...
def read_bundle_file(bundle_file: BinaryIO) -> bytes:
    ...
    if len(data) > DEFAULT_TOTAL_MAX_BYTES:
        raise OrbyteError(
            OrbyteErrorCode.PAYLOAD_TOO_LARGE,
...
def build_skill_md(
    *,
    ...
    if not name:
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, "Skill name cannot be empty.")
    if not description:
        raise OrbyteError(
            OrbyteErrorCode.INVALID_INPUT,
...
def rewrite_custom_bundle_skill_md(
    ...
    *,
    ...
    if slug in BUILT_IN_SKILLS:
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, f"slug '{slug}' is reserved")
...
def validate_and_normalize_custom_bundle(
    ...
    *,
    ...
    try:
        ...
        raise OrbyteError(OrbyteErrorCode.INVALID_INPUT, "bundle is not a valid zip")
    ...
    with ExitStack() as archives:
        ...
        if upload_bytes is not None:
            ...
            try:
                ...
                raise OrbyteError(
                    OrbyteErrorCode.INVALID_INPUT, "upload is not a valid zip"
                ) from exc
            ...
            if normalized_remove_path == SKILL_MD_NAME:
                raise OrbyteError(
                    OrbyteErrorCode.INVALID_INPUT, "SKILL.md cannot be removed"
                )
        ...
        if remove_path is not None and not any(
            ...
            raise OrbyteError(OrbyteErrorCode.NOT_FOUND, "Skill file not found")
