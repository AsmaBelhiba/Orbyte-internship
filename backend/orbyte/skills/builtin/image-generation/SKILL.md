...
using `orbyte-cli image`. Generation runs server-side with whatever provider the
...
If `orbyte-cli image …` exits with "no image generation provider is configured",
...
```bash
orbyte-cli image generate -p "Abstract colorful album cover art" -n 3 -o art.png
```
...
4. Run `orbyte-cli image …`, saving project-bound assets into the workspace. Don't overwrite an existing asset unless asked — use a sibling version (e.g. `hero-v2.png`).
