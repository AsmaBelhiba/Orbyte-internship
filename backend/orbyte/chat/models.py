...
class ChatFullResponse(BaseModel):
    ...
    NOTE: This model is used for the core flow of the Orbyte application, any changes to it should be reviewed and approved by an
...
class ChatLoadedFile(InMemoryChatFile):
    ...
    def lazy_loaded(
        ...
        *,
        ...
        """
        from orbyte.file_store.models import install_lazy_content_loader
...
class LlmStepResult(BaseModel):
    ...
    # Set when _LeadingToolCallLeakFilter suppressed a tool-call-shaped payload
    # (JSON or code-fenced) that named a tool not offered this turn. Never shown
    # to the user; used by run_llm_loop to trigger a single corrective retry
    # instead of surfacing a blank answer.
    suppressed_tool_call_text: str | None = None
