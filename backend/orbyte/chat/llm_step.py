...
from orbyte.chat.chat_state import ChatStateContainer
...
from orbyte.chat.models import LlmStepResult
from orbyte.chat.tool_call_args_streaming import maybe_emit_argument_delta
from orbyte.configs.app_configs import ENABLE_AZURE_IMAGE_CAP
from orbyte.configs.app_configs import LOG_ORBYTE_MODEL_INTERACTIONS
from orbyte.configs.app_configs import PROMPT_CACHE_CHAT_HISTORY
...
from orbyte.llm.models import FunctionCall
from orbyte.llm.models import ImageContentPart
from orbyte.llm.models import ImageUrlDetail
from orbyte.llm.models import ReasoningEffort
from orbyte.llm.models import SystemMessage
from orbyte.llm.models import TextContentPart
from orbyte.llm.models import ToolCall
from orbyte.llm.models import ToolMessage
from orbyte.llm.models import UserMessage
from orbyte.llm.prompt_cache.processor import process_with_prompt_cache
from orbyte.llm.utils import model_needs_formatting_reenabled
from orbyte.prompts.chat_prompts import CODE_BLOCK_MARKDOWN
from orbyte.prompts.chat_prompts import IMAGE_DROP_REMINDER
from orbyte.prompts.constants import SYSTEM_REMINDER_TAG_CLOSE
from orbyte.prompts.constants import SYSTEM_REMINDER_TAG_OPEN
...
from orbyte.server.query_and_chat.streaming_models import CitationInfo
...
from orbyte.tools.models import ToolCallKickoff
from orbyte.tools.tool_name import sanitize_tool_name
from orbyte.tracing.flows import LLMFlow
from orbyte.tracing.framework.create import generation_span
from orbyte.utils.b64 import get_image_type_from_bytes
from orbyte.utils.jsonriver import Parser
from orbyte.utils.logger import setup_logger
from orbyte.utils.postgres_sanitization import sanitize_string
from orbyte.utils.text_processing import find_all_json_objects
...
_FENCE_MARKER = "```"
_LEADING_LEAK_FILTER_MAX_BUFFER = 20_000

_PRINT_CALL_RE = re.compile(
    r"""print\(\s*
        (
            '''(?:[^\\]|\\.)*?'''
            |\"\"\"(?:[^\\]|\\.)*?\"\"\"
...
def _looks_like_attempted_tool_call(obj: dict[str, Any]) -> bool:
    """Narrow structural check for "this JSON object looks like it was meant
    as a tool call" (name+parameters/arguments, or an OpenAI-style function
    wrapper) as opposed to ordinary JSON content the user asked to see.
    Deliberately conservative to avoid flagging legitimate JSON answers
    (e.g. a person's {"name": ..., "age": ...} record).
    """
    if (
        "name" in obj
...
def _looks_like_json_tool_call_fragment(text: str) -> bool:
    """Heuristic for a possibly-truncated JSON blob that still strongly
    resembles a tool-call attempt, used only when the stream ends mid-buffer.
    """
    if not text:
        return False
    lowered = text.lower()
    if '"name"' not in lowered:
        return False
    return '"parameters"' in lowered or '"arguments"' in lowered


def _extract_print_wrapped_prose(body: str) -> str | None:
    """If `body` consists entirely of one or more `print(<string literal>)`
    statements (and nothing else - no imports, defs, loops, real logic),
    return the concatenated string contents. Otherwise return None.

    This targets the common local-model failure mode of answering a plain
    question by "pretending" to run code that just prints the answer, instead
    of writing a normal paragraph.
    """
    ...
    pos = 0
    pieces: list[str] = []
    length = len(stripped_body)
    while pos < length:
        match = _PRINT_CALL_RE.match(stripped_body, pos)
        if not match:
            return None
        try:
        ...
        pos = match.end()
        while pos < length and stripped_body[pos] in " \t\n\r":
            pos += 1
...
class _LeadingToolCallLeakFilter:
    """Streaming guard against tool-call-shaped payloads (raw JSON, or a
    code-fenced "print the answer" block) that a model emits as ordinary
    content instead of a real tool call or a plain-language answer.

    Only inspects the very start of a step's output. The first time content
    is seen that isn't the opening of a JSON object or a fenced code block,
    this becomes a permanent no-op pass-through - it can never touch
    legitimate mid-answer content, including real code the user asked for.

    After the stream completes, `matched_tool_call` is set if a JSON blob
    was recovered as a real, offered tool call (execute it, nothing shown),
    and `suppressed_raw_text` is set if a tool-call-shaped payload was found
    but didn't match anything offered this turn (hallucinated tool - never
    shown, caller may retry once with a corrective reminder).
    """
    ...
    def __init__(self, tool_definitions: list[dict]) -> None:
        ...
        for tool_def in tool_definitions:
            if tool_def.get("type") == "function" and "function" in tool_def:
                ...
                if name:
                    self._tool_name_to_def[name] = func_def

        self._state = self._SNIFFING
        self._pending = ""

        # JSON-mode brace/string scanner state.
        self._scan_pos = 0
        self._depth = 0
        self._in_string = False
        self._escape = False

        self.matched_tool_call: tuple[str, dict[str, Any]] | None = None
        self.suppressed_raw_text: str | None = None
    ...
    def process(self, content: str) -> str:
        ...
        if self._state == self._SNIFFING:
            out = self._sniff()
            if self._state != self._SNIFFING:
                return out
            return out

        if self._state == self._JSON:
            return self._advance_json()

        if self._state == self._FENCE:
            return self._advance_fence()
    ...
    def flush(self) -> str:
        ...
        if self._state == self._SNIFFING:
            return self._bail_to_passthrough()

        # JSON or FENCE state: the stream ended before a block closed
        # (truncated response, finish_reason=length, etc). Fail open unless
        # the partial buffer already strongly resembles an unmatched tool
        # call, mirroring _looks_like_xml_tool_call_payload's role for XML.
        partial = self._pending
        self._pending = ""
        self._state = self._DONE
        if _looks_like_json_tool_call_fragment(partial):
            self.suppressed_raw_text = partial
            return ""
        return partial

    def _bail_to_passthrough(self) -> str:
        self._state = self._DONE
        out = self._pending
        self._pending = ""
        return out
    ...
    def _sniff(self) -> str:
        ...
        if stripped[0] == "{":
            self._state = self._JSON
            return self._advance_json()

        if stripped.startswith(_FENCE_MARKER):
            self._state = self._FENCE
            return self._advance_fence()

        if len(stripped) < len(_FENCE_MARKER) and _FENCE_MARKER.startswith(stripped):
            if len(self._pending) > 200:
                return self._bail_to_passthrough()
            return ""
    ...
    def _scan_json_object_end(self) -> int | None:
        text = self._pending
        i = self._scan_pos
        while i < len(text):
            ch = text[i]
    ...
    def _advance_json(self) -> str:
        end_idx = self._scan_json_object_end()
        if end_idx is None:
            if len(self._pending) > _LEADING_LEAK_FILTER_MAX_BUFFER:
                return self._bail_to_passthrough()
            return ""

        candidate = self._pending[:end_idx]
        remainder = self._pending[end_idx:]
        self._state = self._DONE
        self._pending = ""
        ...
        try:
            ...
            # Balanced braces but not actually valid JSON - this was never a
            # real tool call, e.g. prose that happens to start with '{'.
            return candidate + remainder

        if isinstance(parsed, dict):
            matched = _try_match_json_to_tool(parsed, self._tool_name_to_def)
            if matched:
                self.matched_tool_call = matched
                return remainder
            if _looks_like_attempted_tool_call(parsed):
                self.suppressed_raw_text = candidate
                return remainder

        # Valid JSON but not shaped like a tool call attempt (e.g. the user
        # asked for raw JSON output) - show it.
        return candidate + remainder

    def _advance_fence(self) -> str:
        text = self._pending
        first_newline = text.find("\n")
        if first_newline == -1:
            if len(text) > _LEADING_LEAK_FILTER_MAX_BUFFER:
                return self._bail_to_passthrough()
            return ""

        body_start = first_newline + 1
        close_idx = text.find(_FENCE_MARKER, body_start)
        if close_idx == -1:
            if len(text) > _LEADING_LEAK_FILTER_MAX_BUFFER:
                return self._bail_to_passthrough()
            return ""

        close_end = close_idx + len(_FENCE_MARKER)
        full_block = text[:close_end]
        body = text[body_start:close_idx]
        if body.endswith("\n"):
            body = body[:-1]
        remainder = text[close_end:]

        self._state = self._DONE
        self._pending = ""

        resolved = self._resolve_fence_body(body=body, full_block=full_block)
        return resolved + remainder

    def _resolve_fence_body(self, body: str, full_block: str) -> str:
        stripped = body.strip()
        ...
        if stripped.startswith("{") and stripped.endswith("}"):
            ...
            if isinstance(parsed, dict):
                ...
                if matched:
                    ...
                    return ""
                if _looks_like_attempted_tool_call(parsed):
                    self.suppressed_raw_text = body
                    return ""
                # Legitimate JSON content the user asked to see - show as-is.
                return full_block

        unwrapped = _extract_print_wrapped_prose(body)
        if unwrapped is not None:
            return unwrapped

        # Ordinary/legitimate code answer - show unchanged.
        return full_block
...
def run_llm_step_pkt_generator(
    ...
    if LOG_ORBYTE_MODEL_INTERACTIONS:
        logger.debug(
            "Message history:\n%s",
    ...
    leading_leak_filter = _LeadingToolCallLeakFilter(tool_definitions)
    ...
    with generation_span(
        ...
        for packet in llm.stream(
            ...
            if delta.content:
                ...
                leak_guarded_content = leading_leak_filter.process(delta.content)
                filtered_content = xml_tool_call_content_filter.process(
                    leak_guarded_content
                )
                if filtered_content:
        ...
        # Flush any tail text buffered while checking for a leading JSON/fenced
        ...
        # A JSON tool call can arrive in the content channel instead of the
        # native tool-call channel (common with local models). If no native
        # tool call was found but the leak filter recovered a real, offered
        # tool call from leading content, use that instead of losing it.
        if not tool_calls and leading_leak_filter.matched_tool_call:
            leak_tool_name, leak_tool_args = leading_leak_filter.matched_tool_call
            tool_calls = [
                ToolCallKickoff(
                    tool_call_id=f"leak_recovered_{uuid.uuid4().hex[:8]}",
                    tool_name=leak_tool_name,
                    tool_args=leak_tool_args,
                    placement=Placement(
                        turn_index=turn_index,
                        tab_index=tab_index if use_existing_tab_index else 0,
                        sub_turn_index=sub_turn_index,
                    ),
                )
            ]
            logger.info(
                "Recovered tool call %r from JSON emitted in content channel "
        ...
        # Also skipped when the leak filter suppressed a hallucinated tool-call
        # payload (JSON or fenced): re-surfacing accumulated_raw_answer here
        # would leak exactly the payload the filter just hid from the user.
        if (
            ...
            and not leading_leak_filter.suppressed_raw_text
        ):
            logger.warning(
    ...
    return (
        LlmStepResult(
            ...
            suppressed_tool_call_text=leading_leak_filter.suppressed_raw_text,
        ),
        has_reasoned,
    )
