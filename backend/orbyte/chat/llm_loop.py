...
from orbyte.chat.chat_utils import build_python_chat_files_from_search_docs
...
from orbyte.chat.llm_step import _looks_like_xml_tool_call_payload
...
from orbyte.chat.prompt_utils import build_reminder_message
from orbyte.chat.prompt_utils import build_system_prompt
...
from orbyte.configs.chat_configs import MAX_LLM_CYCLES
from orbyte.configs.constants import DocumentSource
from orbyte.configs.constants import MessageType
from orbyte.configs.model_configs import GEN_AI_INPUT_TOKEN_SAFETY_MARGIN
...
from orbyte.db.memory import UserMemoryContext
...
from orbyte.llm.model_capabilities import is_true_openai_model
from orbyte.prompts.chat_prompts import IMAGE_GEN_REMINDER
from orbyte.prompts.chat_prompts import OPEN_URL_REMINDER
...
from orbyte.server.query_and_chat.streaming_models import ToolCallDebug
...
from orbyte.tools.built_in_tools import STOPPING_TOOLS_NAMES
from orbyte.tools.interface import Tool
from orbyte.tools.models import ChatFile
from orbyte.tools.models import CustomToolCallSummary
from orbyte.tools.models import MemoryToolResponseSnapshot
...
from orbyte.tracing.framework.create import trace
...
def run_llm_loop(
    ...
    with trace(
        ...
        from orbyte.llm.litellm_singleton.config import (
            initialize_litellm,
        ...
        hallucinated_tool_retry_attempted: bool = False
        ...
        for llm_cycle_count in range(MAX_LLM_CYCLES):
            ...
            # The model attempted a tool call (JSON or code-fenced) that named a
            # tool not offered this turn (e.g. a hallucinated open_url/python
            # call) - it was never shown to the user. Give the model one
            # corrective nudge instead of surfacing a blank answer.
            if (
                not llm_step_result.tool_calls
                and llm_step_result.suppressed_tool_call_text
                and not hallucinated_tool_retry_attempted
            ):
                hallucinated_tool_retry_attempted = True
                corrective_reminder_text = (
                    "Your previous response attempted to call a tool that is "
                    "not available in this deployment. Do not attempt to call "
                    "any tools other than the ones explicitly provided to you. "
                    "Answer the user's question directly, in plain natural "
                    "language, using only the retrieved context and your own "
                    "knowledge."
                )
                simple_chat_history.append(
                    ChatMessageSimple(
                        message=corrective_reminder_text,
                        token_count=token_counter(corrective_reminder_text),
                        message_type=MessageType.USER_REMINDER,
                    )
                )
                continue
