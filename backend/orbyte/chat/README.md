...
This document reviews some design decisions around the main agent-loop powering Orbyte's chat flow.
It is highly recommended for all engineers contributing to this flow to be familiar with the concepts here.

> Note: it is assumed the reader is familiar with the Orbyte product and features such as Projects, User files, Citations, etc. 
...
- How messages, files, images are stored can be found in backend/orbyte/db/models.py, there is also a README.md under that directory that may be helpful.
