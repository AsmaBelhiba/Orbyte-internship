...
# document as represented in Orbyte. What is actually stored in the index is the
...
class IndexRetrievalFilters(BaseModel):
    ...
    Used to filter on permissions and other Orbyte-specific metadata rather than
...
class DocumentIndex(
    ...
    """
    A valid document index that can plug into all Orbyte flows must implement all
