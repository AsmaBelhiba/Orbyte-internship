...
from orbyte.configs.app_configs import VERIFY_CREATE_OPENSEARCH_INDEX_ON_INIT_MT
...
from orbyte.document_index.opensearch.schema import GLOBAL_BOOST_FIELD_NAME
...
from orbyte.indexing.models import Document
...
def set_cluster_state(client: OpenSearchClient) -> None:
    if not client.put_cluster_settings(settings=OPENSEARCH_CLUSTER_SETTINGS):
        logger.error(
            ...
            "is not the first time running Orbyte against this instance of OpenSearch, these "
...
def convert_retrieved_opensearch_chunk_to_inference_chunk_uncleaned(
    ...
    Returns:
        An Orbyte inference chunk representation.
    """
...
def _convert_orbyte_chunk_to_opensearch_document(
...
class OpenSearchDocumentIndex(DocumentIndex):
    ...
    def verify_and_create_index_if_necessary(
        ...
        with redis_shared_lock(
            lock_name=f"{OrbyteRedisLocks.OPENSEARCH_VERIFY_INDEX_LOCK_PREFIX}:{self._index_name}",
    ...
    def index(
        ...
        def _flush_chunks(doc_chunks: list[DocMetadataAwareIndexChunk]) -> None:
            ...
            chunk_batch: list[DocumentChunk] = [
                _convert_orbyte_chunk_to_opensearch_document(chunk)
                for chunk in doc_chunks
            ]
            orbyte_document: Document = doc_chunks[0].source_document
            ...
            if orbyte_document.id not in deleted_doc_ids:
                num_chunks_deleted = self.delete(
                    orbyte_document.id, orbyte_document.chunk_count
                )
                deleted_doc_ids.add(orbyte_document.id)
                ...
                document_indexing_results.append(
                    DocumentInsertionRecord(
                        document_id=orbyte_document.id,
    ...
    def delete(
        ...
        Args:
            document_id: The unique identifier for the document as represented
                in Orbyte, not necessarily in the document index.
