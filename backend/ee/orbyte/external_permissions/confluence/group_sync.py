...
def _build_group_member_email_map(
    confluence_client: OrbyteConfluence, cc_pair_id: int
) -> dict[str, set[str]]:
...
def _build_group_member_email_map_from_orbyte_users(
...
def _build_final_group_to_member_email_map(
    confluence_client: OrbyteConfluence,
    cc_pair_id: int,
    # if set, will infer confluence usernames from orbyte users in addition to using the
    ...
    use_orbyte_users: bool = CONFLUENCE_USE_ORBYTE_USERS_FOR_GROUP_SYNC,
) -> dict[str, set[str]]:
    group_to_member_email_map = _build_group_member_email_map(
        confluence_client=confluence_client,
        cc_pair_id=cc_pair_id,
    )
    group_to_member_email_map_from_orbyte_users = (
        (
            _build_group_member_email_map_from_orbyte_users(
                confluence_client=confluence_client,
            )
        )
        if use_orbyte_users
    ...
    all_group_ids = set(group_to_member_email_map.keys()) | set(
        group_to_member_email_map_from_orbyte_users.keys()
    )
    final_group_to_member_email_map = {}
    for group_id in all_group_ids:
        group_member_emails = group_to_member_email_map.get(
            group_id, set()
        ) | group_to_member_email_map_from_orbyte_users.get(group_id, set())
