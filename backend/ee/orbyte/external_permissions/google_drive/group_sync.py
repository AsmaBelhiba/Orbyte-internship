...
from ee.orbyte.external_permissions.google_drive.folder_retrieval import (
    get_folder_permissions_by_ids,
)
from ee.orbyte.external_permissions.google_drive.folder_retrieval import (
    get_modified_folders,
)
from ee.orbyte.external_permissions.google_drive.models import GoogleDrivePermission
from ee.orbyte.external_permissions.google_drive.models import PermissionType
from ee.orbyte.external_permissions.utils import credential_json
from orbyte.access.utils import build_domain_group_id
...
from orbyte.connectors.google_utils.resources import AdminService
...
def _drive_folder_to_orbyte_group(
    folder: FolderInfo,
    group_email_to_member_emails_map: dict[str, list[str]],
) -> ExternalUserGroup:
    """
    Converts a folder into an Orbyte group.
    """
...
def _drive_member_map_to_orbyte_groups(
    drive_id_to_members_map: dict[str, tuple[set[str], set[str]]],
    group_email_to_member_emails_map: dict[str, list[str]],
...
def _google_group_to_orbyte_group(
    admin_service: AdminService,
    group_email: str,
) -> ExternalUserGroup:
    """
...
def _build_orbyte_groups(
    drive_id_to_members_map: dict[str, tuple[set[str], set[str]]],
    group_email_to_member_emails_map: dict[str, set[str]],
    folder_info: list[FolderInfo],
) -> list[ExternalUserGroup]:
    orbyte_groups: list[ExternalUserGroup] = []

    # Convert all drive member definitions to orbyte groups
    ...
    # Convert all folder permissions to orbyte groups
    ...
    # Convert all group member definitions to orbyte groups
...
def gdrive_group_sync(
    ...
    # Each google group is an Orbyte group, yield those
    group_email_to_member_emails_map: dict[str, list[str]] = {}
    for group_email in all_group_emails:
        orbyte_group = _google_group_to_orbyte_group(admin_service, group_email)
        group_email_to_member_emails_map[group_email] = orbyte_group.user_emails
        yield orbyte_group

    # Each drive is a group, yield those
    for orbyte_group in _drive_member_map_to_orbyte_groups(
    ...
    for folder in folder_info:
        yield _drive_folder_to_orbyte_group(folder, group_email_to_member_emails_map)
