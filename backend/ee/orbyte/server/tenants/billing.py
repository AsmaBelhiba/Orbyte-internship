...
from ee.orbyte.configs.app_configs import STRIPE_SECRET_KEY
from ee.orbyte.db.license import acquire_seat_lock
...
def enforce_cloud_seat_limit(
    ...
    ``OrbyteError(SEAT_LIMIT_EXCEEDED)`` on decline.
    ...
    from ee.orbyte.server.tenants.user_mapping import get_tenant_count
    ...
    if reason == SeatBillingDeclineReason.CARD_DECLINED:
        ...
        message = (
            ...
            "subscription. Please contact your Orbyte administrator."
        )
    else:
        message = "Could not add a new seat (billing declined)."

    raise OrbyteError(OrbyteErrorCode.SEAT_LIMIT_EXCEEDED, message)
