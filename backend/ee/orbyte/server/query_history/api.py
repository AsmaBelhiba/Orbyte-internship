...
from ee.orbyte.background.task_name_builders import query_history_task_name
from ee.orbyte.db.query_history import get_all_query_history_export_tasks
from ee.orbyte.db.query_history import get_page_of_chat_sessions
from ee.orbyte.db.query_history import get_total_filtered_chat_sessions_count
...
from ee.orbyte.server.query_history.models import MessageSnapshot
from ee.orbyte.server.query_history.models import QueryHistoryExport
from orbyte.auth.permissions import require_permission
from orbyte.auth.users import get_display_email
...
from orbyte.configs.constants import SessionType
...
from orbyte.db.file_record import get_query_history_export_files
from orbyte.db.models import ChatSession
from orbyte.db.models import User
from orbyte.db.tasks import get_task_with_id
from orbyte.db.tasks import register_task
...
ORBYTE_ANONYMIZED_EMAIL = "anonymous@anonymous.invalid"
...
def snapshot_from_chat_session(
    ...
    flow_type = SessionType.SLACK if chat_session.orbytebot_flow else SessionType.CHAT
...
def get_chat_session_history(
    ...
    for chat_session in page_of_chat_sessions:
        ...
        if query_history_type == QueryHistoryType.ANONYMIZED:
            minimal_chat_session.user_email = ORBYTE_ANONYMIZED_EMAIL
...
def start_query_history_export(
    ...
    client_app.send_task(
        OrbyteCeleryTask.EXPORT_QUERY_HISTORY_TASK,
        task_id=task_id,
        priority=OrbyteCeleryPriority.MEDIUM,
        queue=OrbyteCeleryQueues.CSV_GENERATION,
        kwargs={
            "start": start,
            "end": end,
            "start_time": start_time,
