...
from ee.orbyte.db.analytics import fetch_assistant_message_analytics
from ee.orbyte.db.analytics import fetch_assistant_unique_users
from ee.orbyte.db.analytics import fetch_assistant_unique_users_total
from ee.orbyte.db.analytics import fetch_orbytebot_analytics
from ee.orbyte.db.analytics import fetch_per_user_query_analytics
from ee.orbyte.db.analytics import fetch_persona_message_analytics
from ee.orbyte.db.analytics import fetch_persona_unique_users
from ee.orbyte.db.analytics import fetch_query_analytics
from ee.orbyte.db.analytics import user_can_view_assistant_stats
...
class OrbytebotAnalyticsResponse(BaseModel):
    total_queries: int
    auto_resolved: int
    date: datetime.date


@router.get("/admin/orbytebot")
def get_orbytebot_analytics(
    start: datetime.datetime | None = None,
    end: datetime.datetime | None = None,
    _: User = Depends(require_permission(Permission.FULL_ADMIN_PANEL_ACCESS)),
    db_session: Session = Depends(get_session),
) -> list[OrbytebotAnalyticsResponse]:
    daily_orbytebot_info = fetch_orbytebot_analytics(
        start=start
        or (
            datetime.datetime.now(tz=datetime.timezone.utc)
            - datetime.timedelta(days=_DEFAULT_LOOKBACK_DAYS)
