...
Wired in `ee/orbyte/main.py` for both self-hosted AND multi-tenant. The
...
from ee.orbyte.configs.license_enforcement_config import (
    LICENSE_ENFORCEMENT_ALLOWED_PREFIXES,
)
from ee.orbyte.configs.license_enforcement_config import PATH_PREFIX_MIN_TIER
...
def add_tier_gate_middleware(app: FastAPI, logger: logging.LoggerAdapter) -> None:
    ...
    async def enforce_tier_gate(
        ...
        payload = OrbyteErrorCode.FEATURE_NOT_AVAILABLE.detail(
            f"This feature requires the {required.value.title()} plan."
        )
        payload["required_tier"] = required.value
        return JSONResponse(
            status_code=OrbyteErrorCode.FEATURE_NOT_AVAILABLE.status_code,
            content=payload,
        )
