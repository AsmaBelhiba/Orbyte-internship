...
from ee.orbyte.auth.users import decode_anonymous_user_jwt_token
from ee.orbyte.configs.multi_tenant_gating_config import (
    MULTI_TENANT_GATING_ALLOWED_PREFIXES,
)
from ee.orbyte.server.tenants.product_gating import is_tenant_gated
from orbyte.auth.utils import extract_tenant_from_auth_header
from orbyte.configs.constants import ANONYMOUS_USER_COOKIE_NAME
from orbyte.configs.constants import TENANT_ID_COOKIE_NAME
from orbyte.db.engine.sql_engine import is_valid_schema_name
from orbyte.error_handling.error_codes import OrbyteErrorCode
from orbyte.redis.redis_pool import retrieve_auth_token_data_from_bearer
from orbyte.redis.redis_pool import retrieve_auth_token_data_from_redis
        ...
        try:
            ...
            if (
                ...
                if gated:
                    ...
                    code, status = OrbyteErrorCode.SUBSCRIPTION_INACTIVE.value
                    return JSONResponse(
                        status_code=status,
                        content={
                            "error_code": code,
