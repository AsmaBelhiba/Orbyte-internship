...
from orbyte.db.models import ChatMessageFeedback
...
def fetch_orbytebot_analytics(
    ...
    Number of OrbyteBot Queries (Chat Sessions)
    Number of instances of Negative feedback OR Needing additional help
        (only counting the last feedback)
    """
    # Get every chat session in the time range which is a Orbytebot flow
    ...
    subquery_first_ai_response = (
        ...
        .where(
            ...
            ChatSession.orbytebot_flow.is_(True),
        )
        .where(
    ...
    results = (
        db_session.query(
            ...
            # feedback aren't counted against Orbytebot
            func.sum(
                case(
                    (
                        or_(
        ...
        # message of the chat session where the chat session is Orbytebot type and within the time
