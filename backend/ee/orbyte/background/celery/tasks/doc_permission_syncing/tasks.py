...
from ee.orbyte.db.document import upsert_document_external_perms
...
from orbyte.configs.constants import CELERY_PERMISSIONS_SYNC_LOCK_TIMEOUT
...
from orbyte.db.utils import is_retryable_sqlalchemy_error
...
from orbyte.server.metrics.perm_sync_metrics import inc_doc_perm_sync_docs_processed
from orbyte.server.metrics.perm_sync_metrics import inc_doc_perm_sync_errors
from orbyte.server.metrics.perm_sync_metrics import observe_doc_perm_sync_duration
from orbyte.server.runtime.orbyte_runtime import OrbyteRuntime
from orbyte.server.utils import make_short_id
from orbyte.utils.logger import doc_permission_sync_ctx
...
def _is_external_doc_permissions_sync_due(cc_pair: ConnectorCredentialPair) -> bool:
    ...
    source_sync_period *= int(OrbyteRuntime.get_doc_permission_sync_multiplier())
...
@shared_task(
    name=OrbyteCeleryTask.CHECK_FOR_DOC_PERMISSIONS_SYNC,
    ignore_result=True,
...
def check_for_doc_permissions_sync(self: Task, *, tenant_id: str) -> bool | None:
    ...
    lock_beat: RedisLock = r.lock(
        OrbyteRedisLocks.CHECK_CONNECTOR_DOC_PERMISSIONS_SYNC_BEAT_LOCK,
    ...
    try:
        ...
        lock_beat.reacquire()
        if not r.exists(OrbyteRedisSignals.BLOCK_VALIDATE_PERMISSION_SYNC_FENCES):
            ...
            r.set(
                OrbyteRedisSignals.BLOCK_VALIDATE_PERMISSION_SYNC_FENCES,
                1,
...
def try_creating_permissions_sync_task(
    ...
    try:
        ...
        result = app.send_task(
            OrbyteCeleryTask.CONNECTOR_PERMISSION_SYNC_GENERATOR_TASK,
            kwargs=dict(
                cc_pair_id=cc_pair_id,
                tenant_id=tenant_id,
            ),
            queue=OrbyteCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
...
@shared_task(
    name=OrbyteCeleryTask.CONNECTOR_PERMISSION_SYNC_GENERATOR_TASK,
    acks_late=False,
    soft_time_limit=JOB_TIMEOUT,
    track_started=True,
    trail=False,
...
def connector_permission_sync_generator_task(
    ...
    lock: RedisLock = r.lock(
        OrbyteRedisLocks.CONNECTOR_DOC_PERMISSIONS_SYNC_LOCK_PREFIX
...
def validate_permission_sync_fence(
    ...
    found = celery_find_task(
        ...
        OrbyteCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC,
        r_celery,
    )
    if found:
