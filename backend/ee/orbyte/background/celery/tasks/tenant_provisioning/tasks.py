...
from ee.orbyte.server.tenants.provisioning import setup_tenant
from ee.orbyte.server.tenants.schema_management import create_schema_if_not_exists
from ee.orbyte.server.tenants.schema_management import get_current_alembic_version
...
from orbyte.configs.app_configs import TARGET_AVAILABLE_TENANTS
...
@shared_task(
    name=OrbyteCeleryTask.CLOUD_CHECK_AVAILABLE_TENANTS,
...
def check_available_tenants(self: Task) -> None:  # noqa: ARG001
    ...
    r = get_redis_client(tenant_id=ORBYTE_CLOUD_TENANT_ID)
    lock_check: RedisLock = r.lock(
        OrbyteRedisLocks.CHECK_AVAILABLE_TENANTS_LOCK,
...
def pre_provision_tenant() -> bool:
    ...
    lock_provision: RedisLock = r.lock(
        OrbyteRedisLocks.CLOUD_PRE_PROVISION_TENANT_LOCK,
        timeout=_TENANT_PROVISIONING_TIME_LIMIT,
    )
