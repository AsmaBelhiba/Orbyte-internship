...
from ee.orbyte.utils.license_expiry import get_grace_days_remaining
from orbyte.auth.email_utils import build_html_email
...
def _build_copy(
    ...
    if stage == ExpiryWarningStage.T_30D:
        return (
            f"Orbyte license expires {expires_str}",
            "Your license will expire in approximately 30 days. Contact your "
            "Orbyte representative to renew.",
            "Action required: Orbyte license expires in ~30 days",
        )
    if stage == ExpiryWarningStage.T_14D:
        return (
            ...
            "Action required: Orbyte license expires in ~2 weeks",
        )
    if stage == ExpiryWarningStage.T_1D:
        return (
            f"Orbyte license expires tomorrow ({expires_str})",
            "Your license expires within 24 hours. Renew immediately to avoid "
            "service interruption.",
            "URGENT: Orbyte license expires within 24 hours",
        )
    if stage == ExpiryWarningStage.GRACE:
        return (
            f"Orbyte license expired — {grace_days_remaining} grace days remaining",
