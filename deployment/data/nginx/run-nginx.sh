...
export ORBYTE_BACKEND_API_HOST="${ORBYTE_BACKEND_API_HOST:-api_server}"
export ORBYTE_WEB_SERVER_HOST="${ORBYTE_WEB_SERVER_HOST:-web_server}"
export ORBYTE_MCP_SERVER_HOST="${ORBYTE_MCP_SERVER_HOST:-mcp_server}"
...
echo "Using API server host: $ORBYTE_BACKEND_API_HOST"
echo "Using web server host: $ORBYTE_WEB_SERVER_HOST"
echo "Using MCP server host: $ORBYTE_MCP_SERVER_HOST"
...
envsubst '$DOMAIN $SSL_CERT_FILE_NAME $SSL_CERT_KEY_FILE_NAME $ORBYTE_BACKEND_API_HOST $ORBYTE_WEB_SERVER_HOST $ORBYTE_MCP_SERVER_HOST $NGINX_PROXY_CONNECT_TIMEOUT $NGINX_PROXY_SEND_TIMEOUT $NGINX_PROXY_READ_TIMEOUT' < "/etc/nginx/conf.d/$1" > /etc/nginx/conf.d/app.conf
...
if [ "${MCP_SERVER_ENABLED}" = "True" ] || [ "${MCP_SERVER_ENABLED}" = "true" ]; then
  ...
  # shellcheck disable=SC2016
  envsubst '$ORBYTE_MCP_SERVER_HOST' < "/etc/nginx/conf.d/mcp_upstream.conf.inc.template" > /etc/nginx/conf.d/mcp_upstream.conf.inc
  # shellcheck disable=SC2016
  envsubst '$ORBYTE_MCP_SERVER_HOST' < "/etc/nginx/conf.d/mcp.conf.inc.template" > /etc/nginx/conf.d/mcp.conf.inc
else
