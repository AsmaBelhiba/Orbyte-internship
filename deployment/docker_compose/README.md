...
To set up Orbyte there are several options, Orbyte supports the following for deployment:
...
This provides a guided installation of Orbyte via Docker Compose. It will deploy the latest version of Orbyte
and set up the volumes to ensure data is persisted across deployments or upgrades.

The script will create an orbyte_data directory, all necessary files for the deployment will be stored in
...
Orbyte maintains backwards compatibility across all minor versions following SemVer. If following the install.sh script (or through Docker Compose), you can
...
IMAGE_TAG is the version of Orbyte to run. It is recommended to leave it as latest to get all updates with each redeployment.
