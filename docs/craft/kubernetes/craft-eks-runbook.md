# Orbyte Craft on EKS: infrastructure and migration runbook

How to stand up Orbyte + Craft on a brand-new EKS cluster, and how to migrate existing
...
- Managed data stores: **RDS** (Postgres), **ElastiCache** (Redis), **OpenSearch** domain; the main Orbyte **S3 file-store bucket**; **WAF**.
- IAM/IRSA **roles**: the workload role for Orbyte application pods.
...
**Helm** (`deployment/helm/charts/orbyte`) creates ONLY Kubernetes objects:
- All in-cluster workloads (api, web, nginx, celery, model servers, code-interpreter, **sandbox-proxy**).
- The **`orbyte-sandboxes`** namespace (`templates/sandbox-namespace.yaml`) + **`sandbox` SA** + sandbox RBAC (`<release>-<release-namespace>-sandbox-manager`, `<release>-<release-namespace>-proxy-resolve`) (`templates/sandbox-rbac.yaml`).
...
Craft snapshots now use the normal Orbyte FileStore from the API server/Celery
path. Do not configure `SANDBOX_S3_BUCKET`, `craft.sandboxFileSyncRoleArn`, or
a Craft-only snapshot bucket.

> ⚠️ The one currently-blurred boundary: today the **eks module also creates the `orbyte` namespace + the
> `orbyte-workload-access` SA** (so terraform reaches into k8s). The recommended end state (see §6) is to move
...
Craft needs infrastructure that a normal Orbyte deployment does not need because it
...
|---|---:|---|
| Main Orbyte FileStore bucket/access | Yes | Stores all Orbyte files, including Craft snapshot archives written by API server/Celery through `SnapshotManager(get_default_file_store())`. |
...
|---|---|---:|---|
| `Namespace/orbyte-sandboxes` | cluster | Yes | Keeps runtime sandbox pods/services/secrets separate from app workloads. |
| `ServiceAccount/sandbox` | `orbyte-sandboxes` | Yes | Runtime identity for sandbox pods. It has no storage credentials and does not automount a Kubernetes API token. |
| `Role/<release>-<release-namespace>-sandbox-manager` | `orbyte-sandboxes` | Yes | Allows the Orbyte workload SA to manage sandbox pods, services, secrets, exec, and logs. |
| `RoleBinding/<release>-<release-namespace>-sandbox-manager` | `orbyte-sandboxes` | Yes | Binds sandbox-management permissions to `orbyte.serviceAccountName` and `craft.extraBoundServiceAccounts`. |
...
| `Deployment/orbyte-sandbox-proxy` | release namespace | Yes for proxied egress | Runs the egress proxy/gate. |
| `Service/orbyte-sandbox-proxy` | release namespace | Yes for proxied egress | Stable in-cluster address for sandbox traffic. |
| `ServiceAccount/orbyte-sandbox-proxy` | release namespace | Yes for proxy | Identity used by the proxy deployment. |
| Proxy CA `Role`/`RoleBinding` | release namespace | Yes for proxy CA | Lets the proxy read/create its CA Secret. |
| Proxy sandbox `Role`/`RoleBinding` | `orbyte-sandboxes` | Yes for proxy | Lets the proxy watch sandbox pods and write the CA ConfigMap. |
| `NetworkPolicy/orbyte-sandbox-proxy` | release namespace | Strongly recommended | Allows only the sandbox namespace to reach the proxy port. |
| `NetworkPolicy/orbyte-sandbox-push` | `orbyte-sandboxes` | Strongly recommended | Allows only API server and scheduled-task worker pods to reach sandbox push/opencode/Next.js ports. |
| `PodDisruptionBudget/orbyte-sandbox-proxy` | release namespace | Availability only | Keeps multi-replica proxy deployments from voluntary disruption all at once. |
...
| `configMap.SANDBOX_SERVICE_ACCOUNT_NAME` | Optional | Defaults to `sandbox`; the chart renders the matching ServiceAccount in `orbyte-sandboxes`. |
...
- normal Orbyte pods cannot schedule onto the sandbox nodes because they do not
...
```bash
# 1. Infra (root module instantiates modules/aws/orbyte)
...
cd ../../helm/charts/orbyte && helm dependency build
helm upgrade --install orbyte . -n orbyte -f <values> \
...
# 5. kubectl port-forward -n orbyte svc/orbyte-nginx-controller 8080:80  → http://localhost:8080
```

There is no Craft-specific Terraform object-store module anymore. Snapshot
archives are persisted by API server/Celery through the normal Orbyte FileStore,
...
- `postgresql.enabled/redis.enabled/opensearch.enabled/minio.enabled: false`; `serviceAccount.name: orbyte-workload-access` (IRSA), `auth.objectstorage.enabled: false`.
- RDS: `POSTGRES_HOST`, `PGSSLMODE=require`. If `USE_IAM_AUTH=true`, the sandbox-proxy ServiceAccount must also be trusted by the workload IRSA role and annotated with that same role ARN. Set Terraform `irsa_additional_service_account_names` to the rendered sandbox-proxy ServiceAccount name (`orbyte-sandbox-proxy` for release `orbyte`; verify with `helm template` for other release names). In Helm values, set the proxy annotation:
...
- OpenSearch (v4.0 search backend): `ORBYTE_DISABLE_VESPA=true`, `ENABLE_OPENSEARCH_INDEXING/RETRIEVAL_FOR_ORBYTE=true`, `USING_AWS_MANAGED_OPENSEARCH=true`, `OPENSEARCH_REST_API_PORT=443`, `OPENSEARCH_USE_SSL=true`, `OPENSEARCH_ADMIN_USERNAME=admin`.
- S3: `S3_FILE_STORE_BUCKET_NAME`, `S3_ENDPOINT_URL=""`, `AWS_REGION_NAME`.
- Craft: `ENABLE_CRAFT=true`, `SANDBOX_API_SERVER_URL=http://orbyte-api-service.orbyte.svc.cluster.local:8080`, `auth.sandboxPushSecret.enabled=true`. (`SANDBOX_SERVICE_ACCOUNT_NAME`/`SANDBOX_CONTAINER_IMAGE` default correctly.)
...
remove the unused sandbox storage layer, keep normal Orbyte FileStore access on
...
snapshots are stored through the normal Orbyte FileStore, so the API server and
...
(`deployment/helm/charts/orbyte`). Do not edit chart templates manually; set values.
...
```bash
kubectl -n orbyte-sandboxes get pods,svc,secret
kubectl -n orbyte-sandboxes delete role orbyte-sandbox-manager orbyte-orbyte-sandbox-manager --ignore-not-found
kubectl -n orbyte-sandboxes delete rolebinding orbyte-sandbox-manager orbyte-orbyte-sandbox-manager --ignore-not-found
kubectl -n orbyte-sandboxes delete serviceaccount sandbox --ignore-not-found
kubectl -n orbyte-sandboxes delete serviceaccount sandbox-file-sync --ignore-not-found
kubectl -n orbyte delete role orbyte-proxy-resolve orbyte-orbyte-proxy-resolve --ignore-not-found
kubectl -n orbyte delete rolebinding orbyte-proxy-resolve orbyte-orbyte-proxy-resolve --ignore-not-found
helm upgrade --install orbyte deployment/helm/charts/orbyte -n orbyte -f your-values.yaml ...
```

The example commands include legacy unqualified names and assume release `orbyte`
in namespace `orbyte`. For other release namespaces, use the rendered
...
the normal Orbyte FileStore.

### Existing published-chart install

If the published `orbyte/orbyte` Helm chart does not yet include this PR, install or
upgrade from the local chart in this branch:

```bash
helm dependency build deployment/helm/charts/orbyte
helm upgrade --install orbyte deployment/helm/charts/orbyte -n orbyte -f your-values.yaml ...
```
...
helm template orbyte deployment/helm/charts/orbyte -n orbyte -f your-values.yaml \
  --show-only templates/sandbox-namespace.yaml
helm template orbyte deployment/helm/charts/orbyte -n orbyte -f your-values.yaml \
  --show-only templates/sandbox-rbac.yaml

# Workload SA can manage sandboxes.
kubectl auth can-i create pods -n orbyte-sandboxes \
  --as system:serviceaccount:orbyte:orbyte-workload-access
kubectl auth can-i create pods/exec -n orbyte-sandboxes \
  --as system:serviceaccount:orbyte:orbyte-workload-access
kubectl auth can-i get services -n orbyte \
  --as system:serviceaccount:orbyte:orbyte-workload-access
...
kubectl -n orbyte-sandboxes get sa sandbox -o yaml
...
kubectl -n orbyte get sa orbyte-sandbox-proxy -o yaml
...
|---|---|---|
| Helm sandbox namespace RBAC + ServiceAccount | Covered | Helm owns `orbyte-sandboxes` via `sandbox-namespace.yaml`, then owns `ServiceAccount/sandbox`, sandbox manager RBAC, and proxy service lookup RBAC via `sandbox-rbac.yaml` when Craft is enabled. |
| Terraform sandbox object store + workload identity | Removed | Craft snapshots now use the normal Orbyte FileStore from the API server/Celery path. There is no sandbox-specific bucket, S3 policy, or sandbox IRSA role to create. |
...
- **Workload IRSA SA → chart-owned (refactor).** The `eks` module creates the `orbyte-workload-access`
...
- Codified chart/terraform changes live in the **local** chart, not the published `orbyte/orbyte` — install from `.` until released.
...
- `sandbox-proxy` is a DB/Redis client, not just a forward proxy: its `gate.py` resolves tenant / sandbox /
  ...
  `ServiceAccount/orbyte-sandbox-proxy` and the matching role trust policy subject. Verified: proxy node SG →
  RDS:5432 path open and an authenticated query succeeds; the gate logs `tenant_id=…/sandbox_id=…` resolved
  per request.
- Teardown order/gotchas: `helm uninstall` before `terraform destroy` (else the `orbyte` namespace hangs on
...
**Validated:** snapshot create → main Orbyte FileStore S3 bucket (`sandbox-snapshots/{tenant}/{sandbox}/{id}.tar.gz`,
...
end-to-end — the worker SA (`orbyte-workload-access`, bound to `orbyte-orbyte-sandbox-manager`) snapshots through
