...
- Sandboxes receive only placeholder credentials and a trusted proxy CA. Real
  Orbyte PATs, LLM provider keys, and external-app tokens are resolved by the
...
- `backend/orbyte/sandbox_proxy/server.py` starts mitmproxy, health checks, CA
  bootstrap, identity lookup, request evaluator, credential resolvers, and the
  gate addon.
- `backend/orbyte/sandbox_proxy/addons/gate.py` owns request classification,
  approval parking, grant resolution, credential injection dispatch, internal
  destination blocking, and SIGTERM drain cleanup.
- `backend/orbyte/sandbox_proxy/identity.py`,
  `identity_k8s.py`, and `identity_docker.py` resolve source IPs to sandbox,
  tenant, and user identity.
- `backend/orbyte/sandbox_proxy/request_evaluator.py` turns mitmproxy requests
  into external-app action matches.
- `backend/orbyte/sandbox_proxy/credential_injection.py` and
  `backend/orbyte/sandbox_proxy/resolvers/` inject Orbyte PATs, LLM provider keys,
  and external-app credentials.
- `backend/orbyte/sandbox_proxy/approval_cache.py` defines Redis announce, wake,
  and session-grant cache keys.
- `backend/orbyte/sandbox_proxy/ca.py`, `ca_k8s.py`, and `ca_docker.py` manage
  the proxy CA.

Approval persistence and API:

- `backend/orbyte/db/models.py` defines `ActionApproval`,
  `ExternalAppPolicy`, and `ScheduledTaskPreApprovedApp`.
- `backend/orbyte/db/enums.py` defines `EndpointPolicy`,
  `ApprovalDecision`, and `ApprovalDecidedVia`.
- `backend/orbyte/server/features/build/db/action_approval.py` owns approval DB
  operations. `try_record_decision` is the race arbiter.
- `backend/orbyte/server/features/build/approvals/api.py` exposes live approval
  listing, approve/reject, and approve-for-session.
- `backend/orbyte/db/scheduled_task.py` contains scheduled-task pre-approval
  lookups.

External-app policy and matching:

- `backend/orbyte/external_apps/providers/` defines provider specs, OAuth flows,
  URL patterns, auth templates, action catalogs, and payload decoders.
- `backend/orbyte/external_apps/matching/engine.py` recognizes actions and
  resolves effective policy.
- `backend/orbyte/external_apps/matching/rules.py` implements REST route and
  GraphQL operation matchers.
- `backend/orbyte/external_apps/credentials.py` renders injected auth headers.
- `backend/orbyte/external_apps/token_refresh.py` lazily refreshes OAuth access
  tokens at injection time.

Sandbox and deployment wiring:

- `backend/orbyte/server/features/build/sandbox/image/firewall-init.sh` installs
  the proxy CA and applies in-sandbox egress lockdown.
- `backend/orbyte/server/features/build/sandbox/image/opencode-plugins/session-proxy-tag.ts`
  tags proxied requests with the originating `BuildSession` id.
- `deployment/helm/charts/orbyte/templates/sandbox-proxy/` deploys the Kubernetes
  proxy.
- `deployment/helm/charts/orbyte/templates/sandbox-podtemplate.yaml` wires
  Kubernetes sandboxes to the proxy.
- `deployment/helm/charts/orbyte/templates/network-policy-sandbox-egress.yaml`
...
`sandbox-proxy` runs `python -m orbyte.sandbox_proxy.server`. At startup it:
...
6. Registers credential resolvers in this order:
   `OrbytePatResolver`, `LLMProviderKeyResolver`, `ExternalAppResolver`.
...
- `ORBYTE_PAT=replaced_by_egress_proxy`
...
namespace and has broad egress because it must reach public APIs and Orbyte
datastores.
...
This is fail-closed. A pod or container not known as an Orbyte sandbox cannot use
the proxy.
...
- Allows the configured Orbyte API server host and port.
...
still runs host-level credential injection; this is how Orbyte API PATs and LLM
...
`OrbytePatResolver` claims requests whose host and port match
`SANDBOX_API_SERVER_URL`.

It loads the sandbox row and decrypts `Sandbox.encrypted_pat`, then injects both
Orbyte API key header names as bearer tokens. The tenant is embedded in the PAT,
