# Orbyte Prometheus Metrics Reference

## Adding New Metrics

All Prometheus metrics live in the `backend/orbyte/server/metrics/` package. Follow these steps to add a new metric.
...
_my_counter = Counter(
    "orbyte_my_counter_total",          # Always prefix with orbyte_
...
- Prefix all metric names with `orbyte_`
- Counters: `_total` suffix (e.g. `orbyte_api_slow_requests_total`)
...
from orbyte.server.metrics.my_metric import my_metric_callback
...
from orbyte.server.metrics.my_metric import setup_my_metrics
...
All metrics initialization is funneled through the single `setup_prometheus_metrics()` call in `orbyte/main.py:lifespan()`. Do not add separate setup calls to `main.py`.

### 5. Write tests

Add tests in `backend/tests/unit/orbyte/server/`. Use `unittest.mock.patch` to mock the prometheus objects — don't increment real global counters in tests.
...
1. Open Grafana and navigate to the Orbyte dashboard (or create a new one)
2. Add a new panel — choose the appropriate visualization:
   - **Counters** → use `rate()` in a time series panel (e.g. `rate(orbyte_my_counter_total[5m])`)
...
### Custom (via `orbyte.server.metrics`)

| Metric                         | Type    | Labels                        | Description                                                      |
| ------------------------------ | ------- | ----------------------------- | ---------------------------------------------------------------- |
| `orbyte_api_slow_requests_total` | Counter | `method`, `handler`, `status` | Requests exceeding `SLOW_REQUEST_THRESHOLD_SECONDS` (default 1s) |
...
These metrics provide visibility into SQLAlchemy connection pool state across all three engines (`sync`, `async`, `readonly`). Collected via `orbyte.server.metrics.postgres_connection_pool`.

### Pool State (via custom Prometheus collector — snapshot on each scrape)

| Metric                     | Type  | Labels   | Description                                     |
| -------------------------- | ----- | -------- | ----------------------------------------------- |
| `orbyte_db_pool_checked_out` | Gauge | `engine` | Currently checked-out connections               |
| `orbyte_db_pool_checked_in`  | Gauge | `engine` | Idle connections available in the pool          |
| `orbyte_db_pool_overflow`    | Gauge | `engine` | Current overflow connections beyond `pool_size` |
| `orbyte_db_pool_size`        | Gauge | `engine` | Configured pool size (constant)                 |

### Pool Lifecycle (via SQLAlchemy pool event listeners)

| Metric                                   | Type    | Labels   | Description                              |
| ---------------------------------------- | ------- | -------- | ---------------------------------------- |
| `orbyte_db_pool_checkout_total`            | Counter | `engine` | Total connection checkouts from the pool |
| `orbyte_db_pool_checkin_total`             | Counter | `engine` | Total connection checkins to the pool    |
| `orbyte_db_pool_connections_created_total` | Counter | `engine` | Total new database connections created   |
| `orbyte_db_pool_invalidations_total`       | Counter | `engine` | Total connection invalidations           |
| `orbyte_db_pool_checkout_timeout_total`    | Counter | `engine` | Total connection checkout timeouts       |

### Per-Endpoint Attribution (via pool events + endpoint context middleware)

| Metric                                 | Type      | Labels              | Description                                     |
| -------------------------------------- | --------- | ------------------- | ----------------------------------------------- |
| `orbyte_db_connections_held_by_endpoint` | Gauge     | `handler`, `engine` | DB connections currently held, by endpoint      |
| `orbyte_db_connection_hold_seconds`      | Histogram | `handler`, `engine` | Duration a DB connection is held by an endpoint |
...
### Metrics Server (`orbyte.server.metrics.metrics_server`)
...
### Generic Task Lifecycle Metrics (`orbyte.server.metrics.celery_task_metrics`)

Push-based metrics that fire on Celery signals for all tasks on the worker.

| Metric                              | Type      | Labels                          | Description                                                                   |
| ----------------------------------- | --------- | ------------------------------- | ----------------------------------------------------------------------------- |
| `orbyte_celery_task_started_total`    | Counter   | `task_name`, `queue`            | Total tasks started                                                           |
| `orbyte_celery_task_completed_total`  | Counter   | `task_name`, `queue`, `outcome` | Total tasks completed (`outcome`: `success` or `failure`)                     |
| `orbyte_celery_task_duration_seconds` | Histogram | `task_name`, `queue`            | Task execution duration. Buckets: 1, 5, 15, 30, 60, 120, 300, 600, 1800, 3600 |
| `orbyte_celery_tasks_active`          | Gauge     | `task_name`, `queue`            | Currently executing tasks                                                     |
| `orbyte_celery_task_retried_total`    | Counter   | `task_name`, `queue`            | Total task retries                                                            |
| `orbyte_celery_task_revoked_total`    | Counter   | `task_name`                     | Total tasks revoked (cancelled)                                               |
| `orbyte_celery_task_rejected_total`   | Counter   | `task_name`                     | Total tasks rejected by worker                                                |

Stale start-time entries (tasks killed via SIGTERM/OOM where `task_postrun` never fires) are evicted after 1 hour.

### Per-Connector Indexing Metrics (`orbyte.server.metrics.indexing_task_metrics`)

Enriches docfetching and docprocessing tasks with connector-level labels. Silently no-ops for all other tasks.

| Metric                                | Type      | Labels                                                      | Description                              |
| ------------------------------------- | --------- | ----------------------------------------------------------- | ---------------------------------------- |
| `orbyte_indexing_task_started_total`    | Counter   | `task_name`, `source`, `tenant_id`, `cc_pair_id`            | Indexing tasks started per connector     |
| `orbyte_indexing_task_completed_total`  | Counter   | `task_name`, `source`, `tenant_id`, `cc_pair_id`, `outcome` | Indexing tasks completed per connector   |
| `orbyte_indexing_task_duration_seconds` | Histogram | `task_name`, `source`, `tenant_id`                          | Indexing task duration by connector type |
...
### Connector Health Metrics (`orbyte.server.metrics.connector_health_metrics`)
...
| Metric                                          | Type    | Labels                                                          | Description                                                   |
| ----------------------------------------------- | ------- | --------------------------------------------------------------- | ------------------------------------------------------------- |
| `orbyte_index_attempt_transitions_total`          | Counter | `tenant_id`, `source`, `cc_pair_id`, `connector_name`, `status` | Index attempt status transitions (in_progress, success, etc.) |
| `orbyte_connector_in_error_state`                 | Gauge   | `tenant_id`, `source`, `cc_pair_id`, `connector_name`           | Whether connector is in repeated error state (1=yes, 0=no)    |
| `orbyte_connector_last_success_timestamp_seconds` | Gauge   | `tenant_id`, `source`, `cc_pair_id`, `connector_name`           | Unix timestamp of last successful indexing                    |
| `orbyte_connector_docs_indexed_total`             | Counter | `tenant_id`, `source`, `cc_pair_id`, `connector_name`           | Total documents indexed per connector (monotonic)             |
| `orbyte_connector_indexing_errors_total`          | Counter | `tenant_id`, `source`, `cc_pair_id`, `connector_name`           | Total failed index attempts per connector (monotonic)         |

### Pull-Based Collectors (`orbyte.server.metrics.indexing_pipeline`)
...
| Metric                               | Type  | Labels  | Description                         |
| ------------------------------------ | ----- | ------- | ----------------------------------- |
| `orbyte_queue_depth`                   | Gauge | `queue` | Celery queue length                 |
| `orbyte_queue_unacked`                 | Gauge | `queue` | Unacknowledged messages per queue   |
| `orbyte_queue_oldest_task_age_seconds` | Gauge | `queue` | Age of the oldest task in the queue |
...
```python
from orbyte.server.metrics.celery_task_metrics import (
...
```python
from orbyte.server.metrics.metrics_server import start_metrics_server
...
sum by (queue) (rate(orbyte_celery_task_completed_total[5m]))

# P95 task duration for pruning tasks
histogram_quantile(0.95,
  sum by (le) (rate(orbyte_celery_task_duration_seconds_bucket{task_name=~".*pruning.*"}[5m])))

# Task failure rate
sum by (task_name) (rate(orbyte_celery_task_completed_total{outcome="failure"}[5m]))
  / sum by (task_name) (rate(orbyte_celery_task_completed_total[5m]))

# Active tasks per queue
sum by (queue) (orbyte_celery_tasks_active)

# Indexing throughput by source type
sum by (source) (rate(orbyte_indexing_task_completed_total{outcome="success"}[5m]))

# Queue depth — are tasks backing up?
orbyte_queue_depth > 100
```

## OpenSearch Search Metrics

These metrics track OpenSearch search latency and throughput. Collected via `orbyte.server.metrics.opensearch_search`.

| Metric                                           | Type      | Labels        | Description                                                                 |
| ------------------------------------------------ | --------- | ------------- | --------------------------------------------------------------------------- |
| `orbyte_opensearch_search_client_duration_seconds` | Histogram | `search_type` | Client-side end-to-end latency (network + serialization + server execution) |
| `orbyte_opensearch_search_server_duration_seconds` | Histogram | `search_type` | Server-side execution time from OpenSearch `took` field                     |
| `orbyte_opensearch_search_total`                   | Counter   | `search_type` | Total search requests sent to OpenSearch                                    |
| `orbyte_opensearch_searches_in_progress`           | Gauge     | `search_type` | Currently in-flight OpenSearch searches                                     |
...
sum by (handler) (rate(orbyte_api_slow_requests_total[5m])) * 60
```
...
orbyte_db_pool_checked_out{engine="sync"} / (orbyte_db_pool_size{engine="sync"} + 10) * 100
```
...
# NOTE: Replace 10 with your actual POSTGRES_API_SERVER_POOL_OVERFLOW value.
orbyte_db_pool_checked_out{engine="sync"} > 0.8 * (orbyte_db_pool_size{engine="sync"} + 10)
```
...
topk(10, orbyte_db_connections_held_by_endpoint{engine="sync"})
```
...
histogram_quantile(0.99, sum by (handler, le) (rate(orbyte_db_connection_hold_seconds_bucket{engine="sync"}[5m])))
```
...
sum by (engine) (rate(orbyte_db_pool_checkout_total[5m]))
```
...
histogram_quantile(0.99, sum by (search_type, le) (rate(orbyte_opensearch_search_client_duration_seconds_bucket[5m])))
```
...
sum by (search_type) (rate(orbyte_opensearch_search_total[5m]))
```
...
sum(orbyte_opensearch_searches_in_progress)
```
...
histogram_quantile(0.5, sum by (le) (rate(orbyte_opensearch_search_client_duration_seconds_bucket[5m])))
  -
histogram_quantile(0.5, sum by (le) (rate(orbyte_opensearch_search_server_duration_seconds_bucket[5m])))
```
