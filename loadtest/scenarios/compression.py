...
sends large messages (default ORBYTE_MSG_CHARS), so the history quickly crosses
the model's input-token limit and Orbyte summarizes/recompresses it every turn.
...
ORBYTE_LONGCONV_MODEL — otherwise the default 200k window needs a very long
...
class CompressionUser(LongConversationUser):
    ...
    # Large messages by default so history grows fast (ORBYTE_MSG_CHARS overrides).
    default_msg_chars: int = 8000
    # Long sessions so the recompression path runs many times per session.
    max_session_turns: int = max(2, env_int("ORBYTE_SESSION_TURNS", 60))
