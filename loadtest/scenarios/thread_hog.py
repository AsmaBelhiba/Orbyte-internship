...
thread hold per turn. Override with ORBYTE_HOG_MODEL (mock knobs ride in the
...
class ThreadHogUser(OrbyteChatUser):
    abstract = False
    weight = 1

    scenario_prefix: str = "hog"
    mock_model: str | None = os.environ.get(
        "ORBYTE_HOG_MODEL", "mock-ttft1000-itl200-len600"
    )
    ...
    wait_time = constant(env_float("ORBYTE_HOG_WAIT_SECONDS", 1.0))
    stream_read_timeout: float = env_float("ORBYTE_HOG_STREAM_READ_TIMEOUT", 600.0)
