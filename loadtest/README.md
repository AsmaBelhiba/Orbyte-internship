...
**Guiding principle:** these tests measure Orbyte's application code and
...
enters the environment when you opt in. Code here must never import `orbyte.*`
...
Register it in Orbyte (Admin Panel → LLM, or `PUT /api/admin/llm/provider`) as
...
The mock understands Orbyte's LLM-loop contract: `tool_choice` none/auto/
...
a model configuration and select per scenario to test how Orbyte behaves when
...
```bash
ORBYTE_API_KEY=<key> uv run --group loadtest locust --headless -u 5 -r 1 -t 5m -H https://<your-orbyte-url>
```
...
... uv run --group loadtest locust --headless -u 50 -r 5 -t 15m -H https://<your-orbyte-url>
# or pin to specific classes:
... uv run --group loadtest locust --headless -u 10 -r 2 -t 10m -H https://<your-orbyte-url> BasicChatUser ChatWithSearchUser
```
...
- **LongConversationUser** (`longconv:*`) — keeps one chat session alive for
  `ORBYTE_SESSION_TURNS` turns (default 20), chaining `parent_message_id` so the
...
- **DisconnectUser** (`disconnect:*`) — drops the connection mid-stream the
  moment `ORBYTE_DISCONNECT_AFTER` (default `first_answer_token`) arrives, then
...
- **CompressionUser** (`compress:*`) — long session (default 60 turns) of
  large messages (`ORBYTE_MSG_CHARS`, default 8000) so the history crosses the
  model's input-token limit and Orbyte summarizes/recompresses it every turn —
  ...
  `ORBYTE_LONGCONV_MODEL`**, otherwise the default 200k window needs an
  impractically long history before compression triggers.

```bash
ORBYTE_LONGCONV_MODEL=mock-smallctx \
... uv run --group loadtest locust --headless -u 25 -r 5 -t 20m -H https://<your-orbyte-url> CompressionUser
```
- **FileAttachmentUser** (`fileattach:*`) — uploads one file (`ORBYTE_FILE_KB`,
  ...
  scenarios never touch. Set `ORBYTE_SESSION_TURNS > 1` to accumulate files
  across a growing history (a file-heavy long chat).

```bash
... uv run --group loadtest locust --headless -u 50 -r 5 -t 15m -H https://<your-orbyte-url> LongConversationUser
... uv run --group loadtest locust --headless -u 50 -r 5 -t 15m -H https://<your-orbyte-url> DisconnectUser
```

### Collapse-point ramp (`ORBYTE_SHAPE=stepramp`)
...
slow-provider profile (`ORBYTE_LLM_MODEL=mock-ttft8000-itl40-len600`) to hold
streams open and surface connection/memory exhaustion sooner. The shape
overrides `-u/-r` and is only active when `ORBYTE_SHAPE=stepramp` is set.

```bash
ORBYTE_SHAPE=stepramp ORBYTE_RAMP_STAGES=25,50,100,200 ORBYTE_RAMP_DWELL=300 \
ORBYTE_LLM_MODEL=mock-ttft8000-itl40-len600 \
ORBYTE_API_KEY=<key> uv run --group loadtest locust --headless -t 25m -H https://<your-orbyte-url>
```
...
- **ThreadHogUser** (`hog:*`) — each turn streams a deliberately slow mock
  ...
  pool pressure.
- **HealthProbeUser** (`HEALTH:probe`) — pins `ORBYTE_HEALTH_PROBES` (default 1)
  users that GET `/health` once per `ORBYTE_HEALTH_INTERVAL` (default 1s),
  **independent of `-u`**, and fail any probe slower than `ORBYTE_HEALTH_SLA_MS`
...
ORBYTE_API_KEY=<key> ORBYTE_HEALTH_PATH=/health \
  uv run --group loadtest locust --headless -u 50 -r 5 -t 15m \
  -H https://<your-orbyte-url> ThreadHogUser HealthProbeUser
```

Sweep `concurrent-long-requests ∈ {10,20,40,80}` (use `ORBYTE_SHAPE=stepramp`)
...
> When `LOCUST_HOST` is the web/nginx host, set `ORBYTE_HEALTH_PATH=/api/health`;
...
|---|---|---|
| `ORBYTE_API_KEY` | required | Bearer token for all requests |
| `ORBYTE_LLM_PROVIDER` | unset | Provider name for `llm_override` (needed when the mock isn't the deployment default) |
| `ORBYTE_LLM_MODEL` | unset | Model for BasicChatUser (unset = persona default) |
| `ORBYTE_SEARCH_MODEL` | `mock-tools1` | Model for ChatWithSearchUser |
| `ORBYTE_MULTITOOL_MODEL` | `mock-tools3` | Model for MultiToolUser |
| `ORBYTE_DR_MODEL` | `mock-agents2` | Model for DeepResearchUser |
| `ORBYTE_HOG_MODEL` | `mock-ttft1000-itl200-len600` | Slow model for ThreadHogUser (sets per-turn thread-hold time) |
| `ORBYTE_HOG_WAIT_SECONDS` / `ORBYTE_HOG_STREAM_READ_TIMEOUT` | 1 / 600 | ThreadHogUser think time / max inter-chunk wait |
| `ORBYTE_HEALTH_PATH` | `/health` | Health-probe path (`/api/health` via web/nginx host) |
| `ORBYTE_HEALTH_PROBES` | 1 | Number of HealthProbeUser instances to pin (independent of `-u`) |
| `ORBYTE_HEALTH_INTERVAL` | 1.0 | Seconds between health probes |
| `ORBYTE_HEALTH_SLA_MS` | 10000 | Fail a probe slower than this (liveness `timeoutSeconds`) |
| `ORBYTE_LONGCONV_MODEL` | unset | Model for LongConversationUser (unset = persona default) |
| `ORBYTE_SESSION_TURNS` | 1 | Turns to keep one session alive (LongConversationUser 20, CompressionUser 60) |
| `ORBYTE_MSG_CHARS` | 0 | Per-message size in chars (CompressionUser defaults to 8000; 0 = short questions) |
| `ORBYTE_DISCONNECT_AFTER` | `first_answer_token` | Milestone after which DisconnectUser drops the stream |
| `ORBYTE_FILE_KB` | 512 | Uploaded file size (KB) for FileAttachmentUser |
| `ORBYTE_HOST_HEADER` | unset | `Host` header to send (set when `LOCUST_HOST` targets an internal Service to bypass an external ALB/WAF for high-rps runs) |
| `ORBYTE_SHAPE` | unset | `stepramp` activates the staged ramp shape |
| `ORBYTE_RAMP_STAGES` / `ORBYTE_RAMP_DWELL` / `ORBYTE_RAMP_SPAWN` | `25,50,100,200` / 300 / 5 | Ramp user plateaus, dwell seconds, spawn rate |
| `ORBYTE_WAIT_SECONDS` | 15 | Think time between turns per user |
| `ORBYTE_DR_WAIT_SECONDS` | 30 | Think time for DR users |
| `ORBYTE_STREAM_READ_TIMEOUT` | 180 | Max seconds between stream chunks |
| `ORBYTE_DR_STREAM_READ_TIMEOUT` | 300 | Same, for DR turns |
...
```bash
cd loadtest && docker build -f mock_llm/Dockerfile -t orbyte-mock-llm .
docker run -p 8001:8000 orbyte-mock-llm

# Locust harness image (locustfile + scenarios baked in, for k8s/)
docker build -t orbyte-loadtest .
```
...
1. `kubectl apply -n <orbyte-namespace> -f k8s/mock-llm.yaml`, then register
   `http://orbyte-mock-llm:8000` as an `openai_compatible` provider (see Mock
   LLM server above; keep it `is_public=false` and persona-scoped so real
   users never see it).
2. `kubectl create secret generic orbyte-loadtest --from-literal=ORBYTE_API_KEY=...`
3. `kubectl apply -n <orbyte-namespace> -f k8s/locust.yaml`, then
   `kubectl port-forward svc/orbyte-loadtest-master 8089:8089` and drive runs
   from the web UI. Scale `orbyte-loadtest-worker` replicas for bigger runs,
