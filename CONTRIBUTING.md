# Contributing to Orbyte

This is an internal guide for setting up a local development environment and
the engineering conventions this codebase follows.
...
Orbyte relies on some external software, specifically:
...
> **Note:**
> This guide provides instructions to build and run Orbyte locally from source
> with Docker containers providing the above external software. If you prefer
> to use pre-built container images, see [Running in Docker](#running-in-docker)
> below.

### Prerequisites

- **Python 3.13** — Other versions may require code or dependency changes.
...
We also use `ty` for static type checking. To run the checks manually:
...
We use `oxfmt` for formatting. The desired version will be installed via `bun install` from the `web` directory. To run the formatter:
...
We use `oxlint` for linting. The desired version will be installed via `bun install` from the `web` directory. To run the linter:

```bash
bunx oxlint  # from web
...
Navigate to `deployment/docker_compose`, then start up Postgres/OpenSearch/Redis/MinIO with:
...
To start the frontend, navigate to `web` and run:

```bash
bun run dev
```

Next, start the model server which runs the local NLP models. Navigate to `backend` and run:
...
The first time running the app, you will need to run the DB migrations for Postgres. After the first time, this is no longer required unless the DB models change.

Navigate to `backend` and with the venv active, run:

```bash
alembic upgrade head
```

Next, start the task queue which orchestrates the background jobs. Still in `backend`, run:
...
Now, visit http://localhost:3000 in your browser. The first account you register becomes the admin.

### Running in Docker

You can run the full application stack from pre-built images including all external software dependencies.
...
If you want to make changes and run those changes in Docker, build a local version of the container images that incorporates your changes:
...
- Connector code (data to documents):
