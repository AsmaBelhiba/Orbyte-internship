...
async function waitForServer(baseURL: string): Promise<void> {
  ...
  throw new Error(
    `Orbyte is not running at ${baseURL}. ` +
      `Timed out after ${
 ...
 * first-time "What should Orbyte call you?" prompt that otherwise covers the
...
async function globalSetup(config: FullConfig) {
  ...
  try {
    const client = new OrbyteApiClient(adminCtx, baseURL);
