...
async function uploadFilesToChat(
  ...
  const inputWrapper = page.locator("#orbyte-chat-input");
...
test.describe("Chat File Uploads", () => {
  ...
  test.describe("User-Uploaded Files", () => {
    test("uploaded text file renders in user message", async ({ page }) => {
      ...
      const fileDisplay = userMessage.locator("#orbyte-file");
    ...
    test("uploaded image renders in user message", async ({ page }) => {
      ...
      const userMessage = page.locator("#orbyte-human-message").first();
      const imageDisplay = userMessage.locator("#orbyte-image");
    ...
    test("multiple uploaded files render together", async ({ page }) => {
      ...
      const userMessage = page.locator("#orbyte-human-message").first();
      await expect(
        userMessage.locator("#orbyte-file").getByText(textName)
      ).toBeVisible();
      await expect(
        userMessage.locator("#orbyte-image").locator("img").first()
      ).toBeVisible();
  ...
  test.describe("LLM-Generated Images", () => {
    test("AI response with generated image renders correctly", async ({
      ...
      await mockChatEndpoint(
        ...
        buildMockImageGenStream({
          fileId,
          revisedPrompt: "A friendly cartoon orbyte stone with a smile",
      ...
      await sendMessage(
        page,
        "Generate an image of a friendly cartoon orbyte stone"
      );
