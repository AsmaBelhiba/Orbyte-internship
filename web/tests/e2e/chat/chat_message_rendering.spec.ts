...
const SHORT_USER_MESSAGE = "What is Orbyte?";
...
const LONG_USER_MESSAGE = `I've been evaluating several enterprise search and AI platforms for our organization, and I have a number of detailed questions about Orbyte that I'd like to understand before we make a decision.

First, can you explain how Orbyte handles document indexing across multiple data sources? We currently use Confluence, Google Drive, Slack, and GitHub, and we need to ensure that all of these can be indexed simultaneously without performance degradation.

Second, I'm interested in understanding the security model. Specifically, how does Orbyte handle document-level permissions when syncing from sources that have their own ACL systems? Does it respect the original source permissions, or does it create its own permission layer?

Third, we have a requirement for real-time or near-real-time indexing. What is the typical latency between a document being updated in a source system and it becoming searchable in Orbyte?
...
const SHORT_AI_RESPONSE =
  "Orbyte is an open-source AI-powered enterprise search platform that connects to your company's documents, apps, and people.";

const LONG_AI_RESPONSE = `Orbyte is an open-source Gen-AI and Enterprise Search platform designed to connect to your company's documents, applications, and people. Let me address each of your questions in detail.

## Document Indexing

Orbyte uses a **connector-based architecture** where each data source has a dedicated connector. These connectors run as background workers and can index simultaneously without interfering with each other. The supported connectors include:
...
Orbyte implements a **document-level permission system** that syncs with source ACLs. When documents are indexed, their permissions are preserved:

\`\`\`
Source Permission → Orbyte ACL Sync → Query-time Filtering
\`\`\`
...
\`\`\`python
from orbyte import OrbyteClient

client = OrbyteClient(api_key="your-key")
...
> **Note**: Orbyte supports both cloud and self-hosted deployments. The self-hosted option gives you full control over your data.
  ...
  test.describe(`Chat Message Rendering (${theme} mode)`, () => {
    ...
    test.describe("Short Messages", () => {
      test("short user message with short AI response renders correctly", async ({
        ...
        const userMessage = page.locator("#orbyte-human-message").first();
        await expect(userMessage).toContainText(SHORT_USER_MESSAGE);

        const aiMessage = page.getByTestId("orbyte-ai-message").first();
    ...
    test.describe("Markdown and Code Rendering", () => {
      test("AI response with tables and code blocks renders correctly", async ({
        ...
        await sendMessage(page, "Give me an overview of Orbyte features");

        const aiMessage = page.getByTestId("orbyte-ai-message").first();
        await expect(aiMessage).toContainText("Key Features");
        await expect(aiMessage).toContainText("OrbyteClient");
    ...
    test.describe("Multi-Turn Conversation", () => {
      test("multi-turn conversation renders all messages correctly", async ({
        ...
        const responses = [
          SHORT_AI_RESPONSE,
          "Yes, Orbyte supports over 30 data source connectors including Confluence, Google Drive, Slack, GitHub, Jira, Notion, and many more.",
          "To get started, you can deploy Orbyte using Docker Compose with a single command. The setup takes about 5 minutes.",
        ];

        await mockChatEndpointSequence(page, responses);

        await sendMessage(page, SHORT_USER_MESSAGE);
        await expect(page.getByTestId("orbyte-ai-message").first()).toContainText(
          "open-source AI-powered"
        );

        await sendMessage(page, "What connectors does it support?");
        await expect(page.getByTestId("orbyte-ai-message")).toHaveCount(2, {
          timeout: 30000,
        });

        await sendMessage(page, "How do I get started?");
        await expect(page.getByTestId("orbyte-ai-message")).toHaveCount(3, {
          timeout: 30000,
        });

        const userMessages = page.locator("#orbyte-human-message");
      ...
      const WEB_SEARCH_DOCUMENTS: MockDocument[] = [
        {
          ...
          semantic_identifier: "Orbyte Documentation - Getting Started",
          link: "https://docs.onyx.app/getting-started",
          source_type: "web",
          blurb:
            "Orbyte is an open-source enterprise search and AI platform. Deploy in minutes with Docker Compose.",
          is_internet: true,
        },
        {
          document_id: "web-doc-2",
          semantic_identifier: "Orbyte GitHub Repository",
        ...
        },
        {
          ...
          source_type: "web",
          blurb:
            "Comparing top enterprise search platforms including Orbyte, Glean, and Coveo.",
          is_internet: true,
        },
      ];

      const WEB_SEARCH_RESPONSE = `Based on my web search, here's what I found about Orbyte:

Orbyte is an open-source enterprise search and AI platform that can be deployed in minutes using Docker Compose [[D1]](https://docs.onyx.app/getting-started). The project is hosted on GitHub and is MIT licensed for the community edition, with over 30 connectors available [[D2]](https://github.com/onyx-dot-app/onyx).

In comparisons with other enterprise search platforms, Orbyte stands out for its open-source nature and self-hosted deployment option [[D3]](https://example.com/enterprise-search-comparison). Unlike proprietary alternatives, you maintain full control over your data and infrastructure.
...
- **Extensible**: 30+ pre-built connectors with custom connector support`;
      ...
      test("web search response with citations renders correctly", async ({
        ...
        await page.route("**/api/chat/send-chat-message", async (route) => {
          await route.fulfill({
            ...
            body: buildMockSearchStream({
              ...
              queries: ["Orbyte enterprise search platform overview"],
        ...
        await sendMessage(page, "Search the web for information about Orbyte");
      ...
      test("internal document search response renders correctly", async ({
        ...
        const internalDocs: MockDocument[] = [
          ...
          },
          {
            ...
            blurb:
              "Orbyte platform architecture document covering microservices, data flow, and deployment topology.",
            is_internet: false,
          },
        ];
