...
Orbyte's TypeScript component library and design system.

## Install

```sh
bun add @orbyte-ai/opal
```
...
```tsx
import "@orbyte-ai/opal/styles.css";
```
...
`colors.css` or import a copy from Orbyte. The preset references them but does not define them —
they live with the consumer so the consumer controls the palette.

## Usage

```tsx
import { Button, Text } from "@orbyte-ai/opal/components";
import { Content } from "@orbyte-ai/opal/layouts";
import SvgPlus from "@orbyte-ai/opal/icons/plus";
...
| Subpath                         | Contents                                             |
| ------------------------------- | ---------------------------------------------------- |
| `@orbyte-ai/opal/components`      | Buttons, Text, Tag, Tooltip, Popover, Table, etc.    |
| `@orbyte-ai/opal/layouts`         | Content, ContentAction, IllustrationContent, Section |
| `@orbyte-ai/opal/core`            | Interactive primitives, Hoverable, Disabled          |
| `@orbyte-ai/opal/icons`           | SVG icon components                                  |
| `@orbyte-ai/opal/illustrations`   | Larger SVG illustrations                             |
| `@orbyte-ai/opal/types`           | Shared types (`RichStr`, `IconProps`, etc.)          |
| `@orbyte-ai/opal/utils`           | `cn`, `markdown` helpers                             |
| `@orbyte-ai/opal/styles.css`      | Bundled component CSS                                |
| `@orbyte-ai/opal/tailwind-preset` | Tailwind preset with tokens                          |
...
## Local development (inside the Orbyte repo)
...
so Orbyte's web app keeps building.

The package is consumed by `web/` as a workspace via `web/package.json`'s `"@orbyte-ai/opal":
"./lib/opal"`. During Orbyte development, `web/` resolves Opal source through the `@opal/*`
...
4. The workflow runs automatically on tag push. It builds (`tsup` + CSS barrel) and runs
   ...
   the new version on https://www.npmjs.com/package/@orbyte-ai/opal.
...
The `@orbyte-ai/opal/logos` subpath ships brand marks of third-party
products Orbyte integrates with. Marks remain the property of their
respective owners; Orbyte claims no trademark over them. See
